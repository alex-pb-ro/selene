"""
Coordinates the launching of JetBrains IDE instances triggered by Selene's own
`jetbrains_launch_command`, so that concurrent Selene sessions activating different
...
from selene.jetbrains import jetbrains_plugin_client
...
def _lock_path_for_launch_command(launch_command: str) -> Path:
    ...
    :return: a stable, cross-process lock file path for the given launch command, so that
        concurrent Selene sessions configured with the same launch command serialize on the
    ...
    return Path(tempfile.gettempdir()) / f"selene-jetbrains-launch-{digest}.lock"
...
def launch_and_wait_for_plugin_server(
    ...
    Selene sessions that share the same launch command, and waits for the plugin server to
    ...
    try:
        with lock:
            ...
            while True:
                ...
                if time.monotonic() >= deadline:
                    log.warning(
                        "JetBrains IDE launch command exited but no Selene plugin server became reachable "
        ...
        log.warning(
            "Timed out after %.0fs waiting for another Selene session's JetBrains IDE launch to finish "
