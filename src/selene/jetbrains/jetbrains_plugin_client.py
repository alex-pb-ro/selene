"""
Client for the Selene JetBrains Plugin
"""
...
import selene.jetbrains.jetbrains_types as jb
from selene.config.selene_config import RegisteredProject
...
from selene.util.version import Version

T = TypeVar("T")
log = logging.getLogger(__name__)


class SeleneClientError(Exception):
    """Base exception for Selene client errors."""


class ConnectionError(SeleneClientError):
    """Raised when connection to the service fails."""


class APIError(SeleneClientError):
...
class PluginServerError(SeleneClientError):
    ...
    def __init__(self, message: str):
        unrecoverable_error_prefix = (
            "The request failed in the local JetBrains plugin. Review the error locally with the user. "
            "Do not send project data or diagnostics to an external service unless the user explicitly requests it.\n"
        )
...
class JetBrainsPluginClientManager:
    ...
    def find_client(self, project_root: Path, log_warning: bool = True) -> "JetBrainsPluginClient":
        ...
        if log_warning:
            log.warning(
                "Searched for Selene JetBrains plugin service for project at %s but found no matching service. "
                "Found plugin instances for the following project paths: %s",
                project_root,
                plugin_paths_found,
            )
        raise ServerNotFoundError(
            f"Found no Selene service in a JetBrains IDE instance for the project at {project_root}. "
            "STOP. Do not attempt any other tools or workarounds. Ask the user to open this folder as a project in a JetBrains IDE "
            "with the Selene plugin installed and running!"
        )
...
class JetBrainsPluginClient(ToStringMixin):
    """
    Python client for the Selene Backend Service.

    Provides simple methods to interact with all available endpoints.
    """

    BASE_PORT = SelenePorts.JETBRAINS_PLUGIN_SERVER_BASE_PORT
    ...
    @staticmethod
    def _paths_match(resolved_selene_path: str, plugin_path: str) -> bool:
        """
        Checks whether the resolved Selene path matches the plugin path, accounting for possible prefixes
        ...
        - The plugin path may contain prefixes:
          - The plugin path may be a WSL UNC path, e.g. `//wsl.localhost/Ubuntu-24.04/home/user/project`
            or `//wsl$/Ubuntu/home/user/project` while Selene will just have `/home/user/project`
          - Other prefixes like `/workspaces/selene/C:/Users/user/projects/my-app`
        ...
        :param resolved_selene_path: The resolved project root path from Selene's perspective
        ...
        try:
            ...
            if resolved_plugin_path == resolved_selene_path:
                return True
        except:
            pass
        ...
        std_selene_path = normalise_wsl_mnt(str(resolved_selene_path)).lower()
        std_plugin_path = normalise_wsl_mnt(str(plugin_path)).lower()

        # At this point, the plugin path may still contain prefixes, so we check if the Selene path is a suffix of the plugin path
        return std_plugin_path.endswith(std_selene_path)

    def matches(self, resolved_path: Path) -> bool:
        """
        :param resolved_path: the resolved project root path from Selene's perspective
    ...
    def _require_version_at_least(self, *version_parts: int) -> None:
        ...
        if not self.is_version_at_least(*version_parts):
            raise SeleneClientError(
                f"This operation requires Selene JetBrains plugin version "
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[dict] = None, pythonify: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            raise ConnectionError(f"Failed to connect to Selene service at {url}: {e}")
            ...
            raise SeleneClientError(f"Request failed: {e}")
