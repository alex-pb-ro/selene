...
class Project(ToStringMixin):
    def __init__(
        ...
        *,
        ...
        assert selene_config is not None
        self.project_root = project_root
        self.project_config = project_config
        self.selene_config = selene_config
        self._selene_data_folder = selene_config.get_project_selene_folder(self.project_root)
        log.info("Selene project data folder: %s", self._selene_data_folder)

        read_only_memory_patterns = selene_config.read_only_memory_patterns + project_config.read_only_memory_patterns
        ignored_memory_patterns = selene_config.ignored_memory_patterns + project_config.ignored_memory_patterns
        ...
        self.line_ending = project_config.line_ending or selene_config.line_ending
        ...
        if not os.path.exists(selene_data_gitignore_path):
            os.makedirs(os.path.dirname(selene_data_gitignore_path), exist_ok=True)
            log.info(f"Creating .gitignore file in {selene_data_gitignore_path}")
            with open(selene_data_gitignore_path, "w", encoding="utf-8") as f:
                f.write(f"/{SolidLanguageServer.CACHE_FOLDER_NAME}\n")
                f.write(f"/{ProjectConfig.SELENE_LOCAL_PROJECT_FILE}\n")
    ...
    def _gather_ignorespec(self, *, strict: bool = False) -> None:
        with LogTime(f"Gathering ignore spec for project {self.project_config.project_name}", logger=log):
            try:
                ...
                global_ignored_paths = self.selene_config.ignored_paths
    ...
    def set_agent(self, agent: "SeleneAgent") -> None:
        self._agent = agent
    ...
    def language_backend(self) -> LanguageBackend:
        ...
        if self._agent is not None:
            ...
            return self.selene_config.determine_language_backend(self.project_config)
    ...
    def load(
        ...
        return Project(project_root=str(project_root), project_config=project_config, selene_config=selene_config)
    ...
    def path_to_selene_data_folder(self) -> str:
        return self._selene_data_folder

    def path_to_project_yml(self) -> str:
        return self.selene_config.get_project_yml_location(self.project_root)
    ...
    def is_trusted(self) -> bool:
        ...
        """
        return self.selene_config.is_trusted_project_path(self.project_root)
    ...
    def _ignore_spec(self) -> pathspec.PathSpec:
        ...
        if self.__ignore_spec is None:
            raise ValueError(
                "The ignore spec could not be computed; check the local log for errors. Share diagnostics only when the user explicitly requests it."
            )
    ...
    def _ignored_patterns(self) -> list[str]:
        ...
        if self.__ignored_patterns is None:
            raise ValueError(
                "The ignored patterns could not be computed; check the local log for errors. Share diagnostics only when the user explicitly requests it."
            )
    ...
    def create_language_server_manager(self) -> LanguageServerManager:
        ...
        try:
            ...
            tool_timeout = self.selene_config.tool_timeout
            ...
            ls_specific_settings = dict(self.selene_config.ls_specific_settings)
