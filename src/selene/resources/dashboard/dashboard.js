...
class Dashboard {
    constructor() {
        ...
        this.originalSeleneConfigContent = null;
        this.seleneConfigContentDirty = false;
        ...
        this.$editSeleneConfigModal = $('#edit-selene-config-modal');
        this.$editSeleneConfigContent = $('#edit-selene-config-content');
        this.$editSeleneConfigSaveBtn = $('#edit-selene-config-save-btn');
        this.$editSeleneConfigCancelBtn = $('#edit-selene-config-cancel-btn');
        this.$modalCloseEditSeleneConfig = $('.modal-close-edit-selene-config');
        ...
        this.$editSeleneConfigSaveBtn.click(this.saveSeleneConfigFromModal.bind(this));
        this.$editSeleneConfigCancelBtn.click(this.closeEditSeleneConfigModal.bind(this));
        this.$modalCloseEditSeleneConfig.click(this.closeEditSeleneConfigModal.bind(this));
        ...
        this.$editSeleneConfigModal.click(function (e) {
            if ($(e.target).hasClass('modal')) {
                self.closeEditSeleneConfigModal();
            }
        });
    ...
    displayConfig(config) {
        try {
            ...
            html += '<div class="config-label">Version:</div><div class="config-value">' + config.selene_version;
            html += '</div>';

            // Project info
            html += '<div class="config-label">Active Project:</div>';
            if (config.active_project.name && config.active_project.path) {
                const configPath = config.active_project.path + '/.selene/project.yml';
            ...
            html += '<span>Configuration guide: docs/02-usage/050_configuration.md</span>';
            html += '</div>';
            html += '<button id="edit-selene-config-btn" class="btn language-add-btn" style="white-space: nowrap; padding: 10px; ">Edit Global Selene Config</button>';
            html += '</div>';
            ...
            // Attach event handler for edit selene config button
            $('#edit-selene-config-btn').click(this.openEditSeleneConfigModal.bind(this));
    ...
    updateTitle(activeProject) {
        document.title = activeProject ? `${activeProject} – Selene Dashboard` : 'Selene Dashboard';
    }
    ...
    saveLogs() {
        ...
        a.href = url;
        a.download = `selene-logs-${timestamp}.txt`;
    ...
    toggleTheme() {
        ...
        localStorage.setItem('selene-theme', newTheme);
        this.setTheme(newTheme);
    }
    ...
    setTheme(theme) {
        ...
        localStorage.setItem('selene-theme', theme);
    ...
    // ===== Selene Config Editing Methods =====

    openEditSeleneConfigModal() {
        const self = this;
        this.seleneConfigContentDirty = false;

        // Load selene config content
        $.ajax({
            url: '/get_selene_config', type: 'GET', success: function (response) {
                if (response.status === 'error') {
                    alert('Error: ' + response.message);
                    return;
                }
                self.originalSeleneConfigContent = response.content;
                self.$editSeleneConfigContent.val(response.content);
                self.seleneConfigContentDirty = false;
                self.$editSeleneConfigModal.fadeIn(200);
            }, error: function (xhr, status, error) {
                console.error('Error loading selene config:', error);
                alert('Error loading selene config: ' + (xhr.responseJSON ? xhr.responseJSON.message : error));
            }
        });

        // Track changes to config content
        this.$editSeleneConfigContent.off('input').on('input', function () {
            const currentContent = self.$editSeleneConfigContent.val();
            self.seleneConfigContentDirty = (currentContent !== self.originalSeleneConfigContent);
        });
    }

    closeEditSeleneConfigModal() {
        // Check if there are unsaved changes
        if (this.seleneConfigContentDirty) {
            if (!confirm('You have unsaved changes. Are you sure you want to close?')) {
                return;
            }
        }

        this.$editSeleneConfigModal.fadeOut(200);
        this.originalSeleneConfigContent = null;
        this.seleneConfigContentDirty = false;
    }

    saveSeleneConfigFromModal() {
        const self = this;
        const content = this.$editSeleneConfigContent.val();

        // Disable button during request
        self.$editSeleneConfigSaveBtn.prop('disabled', true).text('Saving...');

        $.ajax({
            url: '/save_selene_config', type: 'POST', contentType: 'application/json', data: JSON.stringify({
                content: content
            }), success: function (response) {
                if (response.status === 'success') {
                    // Update original content and reset dirty flag
                    self.originalSeleneConfigContent = content;
                    self.seleneConfigContentDirty = false;
                    // Close modal
                    self.$editSeleneConfigModal.fadeOut(200);
                    alert('Configuration saved successfully. Please restart Selene for changes to take effect.');
                } else {
                    alert('Error: ' + response.message);
                }
            }, error: function (xhr, status, error) {
                console.error('Error saving selene config:', error);
                alert('Error saving selene config: ' + (xhr.responseJSON ? xhr.responseJSON.message : error));
            }, complete: function () {
                // Re-enable button
                self.$editSeleneConfigSaveBtn.prop('disabled', false).text('Save');
            }
        });
    }
    ...
    shutdown() {
        ...
        if (confirm("This will fully terminate the Selene server.")) {
            _shutdown();
        } else {
