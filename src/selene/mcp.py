...
def configure_logging(*args, **kwargs) -> None:
    ...
    if not logging.is_enabled():
        logging.basicConfig(level=logging.INFO, stream=sys.stderr, format=SELENE_LOG_FORMAT)
...
class SeleneMCPFactory:
    """
    Factory for the creation of the Selene MCP server with an associated SeleneAgent.
    """
    ...
    def __init__(
        ...
        self.context = SeleneAgentContext.load(context)
        self.project = project
        self.agent: SeleneAgent | None = None
    ...
    @staticmethod
    def make_mcp_tool(tool: Tool, openai_tool_compatible: bool = True, structured_output: bool | None = None) -> SeleneFastMCPTool:
        """
        Creates an MCP tool from a Selene Tool instance.

        :param tool: the Selene Tool instance to convert.
        :param openai_tool_compatible: whether to process the tool schema to be compatible with OpenAI tools
            (doesn't accept integer, needs number instead, etc.). This allows using Selene MCP within codex.
        :param structured_output: whether to use structured output for the tool (None = auto)
        """
        return SeleneFastMCPTool(tool, openai_tool_compatible=openai_tool_compatible, structured_output=structured_output)
    ...
    def _create_selene_agent(
        self, selene_config: SeleneConfig, modes: ModeSelectionDefinition | None = None, project_activation_error: str | None = None
    ) -> SeleneAgent:
        return SeleneAgent(
    ...
    def create_mcp_server(
        ...
        """
        Create an MCP server with process-isolated SeleneAgent to prevent asyncio contamination.
        ...
        :param enable_web_dashboard: Whether to enable the web dashboard. If not specified, will take the value from the selene configuration.
        :param enable_gui_log_window: Whether to enable the GUI log window. It currently does not work on macOS, and setting this to True will be ignored then.
            If not specified, will take the value from the selene configuration.
        :param open_web_dashboard: Whether to open the web dashboard on launch.
            If not specified, will take the value from the selene configuration.
        :param log_level: Log level. If not specified, will take the value from the selene configuration.
        :param trace_lsp_communication: Whether to trace the communication between Selene and the language servers.
            This is useful for debugging language server issues.
        :param tool_timeout: Timeout in seconds for tool execution. If not specified, will take the value from the selene configuration.
        :param project_activation_error: an initial project activation error to report back to the client
        """
        try:
            config = SeleneConfig.from_config_file()
            ...
            self.agent = self._create_selene_agent(config, modes=mode_selection_def, project_activation_error=project_activation_error)
        ...
        # Set the low-level server value explicitly so MCP clients identify Selene correctly.
