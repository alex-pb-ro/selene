...
from selene.jetbrains.jetbrains_plugin_client import JetBrainsPluginClientManager
from selene.project_server import ProjectServerClient
...
class ListQueryableProjectsTool(Tool, ToolMarkerOptional, ToolMarkerDoesNotRequireActiveProject):
    ...
    def apply(self, symbol_access: bool = True) -> str:
        ...
        registered_projects = self.agent.selene_config.projects
        if symbol_access:
...
class QueryProjectTool(Tool, ToolMarkerOptional, ToolMarkerDoesNotRequireActiveProject):
    ...
    def apply(self, project_name: str, tool_name: str, tool_params_json: str) -> str:
        """
        Queries a project by executing a read-only Selene tool. The tool will be executed in the context of the project.
        ...
        if self._is_project_server_required(tool):
            ...
            registered_project = self.agent.selene_config.get_registered_project(project_name)
            assert registered_project is not None, f"Project {project_name} is not registered and cannot be queried."
            project = registered_project.get_project_instance(self.agent.selene_config)
