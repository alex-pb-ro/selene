...
from selene.tools import Tool, ToolMarkerDoesNotRequireActiveProject, ToolMarkerOptional


class OpenDashboardTool(Tool, ToolMarkerOptional, ToolMarkerDoesNotRequireActiveProject):
    """
    Opens the Selene web dashboard in the default web browser.
    ...
    def apply(self) -> str:
        ...
        if self.agent.open_dashboard():
            return f"Selene web dashboard has been opened in the user's default web browser: {self.agent.get_dashboard_url()}"
        else:
            return f"Selene web dashboard could not be opened automatically; tell the user to open it via {self.agent.get_dashboard_url()}"
...
class ActivateProjectTool(Tool, ToolMarkerDoesNotRequireActiveProject):
    ...
    def apply(self, project: str, session_id: str) -> str:
        ...
        result += "\nIMPORTANT: If you have not yet read the 'Selene Instructions Manual', do it now before continuing!"
        return result


class RemoveProjectTool(Tool, ToolMarkerDoesNotRequireActiveProject, ToolMarkerOptional):
    """
    Removes a project from the Selene configuration.
    """
    ...
    def apply(self, project_name: str) -> str:
        ...
        """
        self.agent.selene_config.remove_project(project_name)
