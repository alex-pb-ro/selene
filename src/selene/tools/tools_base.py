...
if TYPE_CHECKING:
    ...
    from selene.code_editor import CodeEditor, LanguageServerCodeEditor
...
class Component(ABC):
    def __init__(self, agent: "SeleneAgent"):
        self.agent = agent
...
class Tool(Component):
    ...
    def _limit_length(
        ...
        if max_answer_chars == -1:
            max_answer_chars = self.agent.selene_config.default_max_tool_answer_chars
                ...
                if not isinstance(self, ToolMarkerDoesNotRequireActiveProject):
                    if self.agent.get_active_project() is None:
                        raise ToolCallError(
                            ...
                            + f"{self.agent.selene_config.project_names}"
                        )
