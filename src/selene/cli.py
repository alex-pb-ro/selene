...
from selene import selene_version
from selene.config.client_setup import client_setup_handlers
...
from selene.constants import (
    ...
    SELENE_LOG_FORMAT,
    SELENES_OWN_CONTEXT_YAMLS_DIR,
    SELENES_OWN_MODE_YAMLS_DIR,
)
from selene.prompt_factory import SelenePromptFactory
from selene.tools import ActivateProjectTool
from selene.util.cli_util import AutoRegisteringGroup
...
override the default_modes defined in the global Selene configuration or
the active project.
For details on mode configuration, see
  docs/02-usage/050_configuration.md#modes.
"""
...
def find_project_root(root: str | Path | None = None) -> str | None:
    ...
    Returns the nearest ancestor that is either an explicit Selene project
    (contains .selene/project.yml) or a git root (contains .git, which may be a
    directory or, for git worktrees and submodules, a pointer file). The nearest
    such directory wins; a .selene/project.yml at the same level takes priority
    ...
    .selene/project.yml first and only then for .git) ensures a git worktree
    nested under another Selene project resolves to the worktree itself, instead
    of being hijacked by the ancestor project's .selene/project.yml.
    ...
    # Selene project (.selene/project.yml) or a git root (.git, a dir or a worktree
    # pointer file). This keeps a nested git worktree from being hijacked by an
    # ancestor's .selene/project.yml.
    for directory in ancestors():
        if (directory / ".selene" / "project.yml").is_file() or (directory / ".git").exists():
...
class TopLevelCommands(AutoRegisteringGroup):
    """Root CLI group containing the core Selene commands."""

    def __init__(self) -> None:
        super().__init__(
            name="selene",
            help="Main selene CLI commands. "
            "Note that you also have access to `selene-hooks` CLI commands which are kept under "
            "that separate entrypoint for performance reasons, see `selene-hooks --help`. You can run `<command> --help` for more info on each command.",
        )
    ...
    def _print_version(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
        ...
        if not value:
            return
        click.echo(f"Selene {selene_version()}")
        ctx.exit()

    @staticmethod
    @click.command(
        "init",
        help="Initialize Selene by creating a global config file with the specified default language backend.",
    ...
    def init(language_backend: Literal["LSP", "JetBrains"] = "LSP") -> None:
        click.echo(f"\nSelene version: {selene_version()}\n")
        selene_config = SeleneConfig.init(language_backend=LanguageBackend(language_backend))
        click.echo(f"Configuration file: {selene_config.config_file_path}")
        ...
        if len(applicable_setup_handlers) > 0:
            click.echo(
                "\nAuto-configurable clients detected.\nApply the following commands to configure the Selene MCP server (in a default configuration):"
            )
            for setup_handler in applicable_setup_handlers:
                click.echo(f"  selene setup {setup_handler.name}")

        click.echo("\nSelene has been initialised successfully.\n")

    @staticmethod
    @click.command(
        "setup",
        help="Set up Selene for use with a specific client by registering it as an MCP server.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument(
        "client",
    ...
    def setup(client: str) -> None:
        ...
        # apply the setup
        if handler.apply():
            click.echo(f"\nSelene has been successfully set up for {client}.\n")
        else:
            click.echo(f"\nFailed to set up Selene for {client}.\n")
            raise SystemExit(1)

    @staticmethod
    @click.command("start-mcp-server", help="Starts the Selene MCP server.", context_settings={"max_content_width": _MAX_CONTENT_WIDTH})
    ...
    @click.option(
        ...
        is_flag=False,
        default=None,
        help="Enable the web dashboard (overriding the setting in Selene's config). "
    ...
    @click.option(
        ...
        is_flag=False,
        default=None,
        help="Enable the gui log window (currently only displays logs; overriding the setting in Selene's config).",
    )
    @click.option(
        "--open-web-dashboard",
        type=bool,
        is_flag=False,
        default=None,
        help="Open Selene's dashboard in your browser after MCP server startup (overriding the setting in Selene's config).",
    )
    @click.option(
        "--log-level",
    ...
    @click.option(
        ...
        is_flag=True,
        default=False,
        help="Auto-detect project from current working directory (nearest ancestor containing .selene/project.yml or .git). If none is found, no project is activated. Intended for CLI-based agents like Claude Code, Gemini and Codex.",
    )
    def start_mcp_server(
        ...
        # initialize logging, using INFO level initially (will later be adjusted by SeleneAgent according to the config)
        ...
        formatter = logging.Formatter(SELENE_LOG_FORMAT)
        memory_log_handler = MemoryLogHandler()
        Logger.root.addHandler(memory_log_handler)
        stderr_handler = logging.StreamHandler(stream=sys.stderr)
        stderr_handler.formatter = formatter
        Logger.root.addHandler(stderr_handler)
        log_path = SelenePaths().get_next_log_file_path("mcp")
        file_handler = logging.FileHandler(log_path, mode="w")
        file_handler.formatter = formatter
        Logger.root.addHandler(file_handler)

        log.info("Initializing Selene MCP server")
        ...
        if project_from_cwd:
            ...
            if project is not None:
                ...
                project_activation_error = (
                    f"No project root found from cwd={os.getcwd()} (no .selene/project.yml or .git found); "
        ...
        factory = SeleneMCPFactory(transport=transport, context=context, project=project_file, memory_log_handler=memory_log_handler)
    ...
    def print_system_prompt(project: str | None, only_instructions: bool, context: str, modes: Sequence[str] | None = None) -> None:
        ...
        prefix = "You will receive access to Selene's symbolic tools. Below are instructions for using them, take them into account."
        postfix = "You begin by acknowledging that you understood the above instructions and are ready to receive tasks."

        context_instance = SeleneAgentContext.load(context)
    ...
    @click.command(
        ...
        help="Starts the Selene project server, which exposes project querying capabilities via HTTP.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.option(
        "--host",
    ...
    def start_project_server(
        ...
        from selene.project_server import ProjectServer
        ...
        stderr_handler.formatter = formatter
        Logger.root.addHandler(stderr_handler)
        log_path = SelenePaths().get_next_log_file_path("project-server")
        file_handler = logging.FileHandler(log_path, mode="w")
        file_handler.formatter = formatter
        Logger.root.addHandler(file_handler)
        ...
        log.info("Starting Selene project server")
        log.info("Storing logs in %s", log_path)
    ...
    @click.command(
        "dashboard-viewer",
        help="Open the Selene dashboard viewer for a given URL.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    ...
    def dashboard_viewer(url: str, width: int, height: int) -> None:
        from selene.dashboard import SeleneDashboardViewer

        viewer = SeleneDashboardViewer(url, width=width, height=height)
        viewer.run()
...
class ModeCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(name="mode", help="Manage Selene modes. You can run `mode <command> --help` for more info on each command.")

    @staticmethod
    @click.command("list", help="List available modes.", context_settings={"max_content_width": _MAX_CONTENT_WIDTH})
    def list() -> None:
        mode_names = SeleneAgentMode.list_registered_mode_names()
        max_len_name = max(len(name) for name in mode_names) if mode_names else 20
        for name in mode_names:
            mode_yml_path = SeleneAgentMode.get_path(name)
            is_internal = Path(mode_yml_path).is_relative_to(SELENES_OWN_MODE_YAMLS_DIR)
    ...
    def create(name: str, from_internal: str) -> None:
        ...
        dest = os.path.join(SelenePaths().user_modes_dir, f"{mode_name}.yml")
        src = (
            os.path.join(SELENES_OWN_MODE_YAMLS_DIR, f"{from_internal}.yml")
            if from_internal
            else os.path.join(SELENES_OWN_MODE_YAMLS_DIR, "mode.template.yml")
        )
        if not os.path.exists(src):
            raise FileNotFoundError(
                f"Internal mode '{from_internal}' not found in {SELENES_OWN_MODE_YAMLS_DIR}. Available modes: {SeleneAgentMode.list_registered_mode_names()}"
            )
    ...
    def edit(mode_name: str) -> None:
        path = os.path.join(SelenePaths().user_modes_dir, f"{mode_name}.yml")
        if not os.path.exists(path):
            if mode_name in SeleneAgentMode.list_registered_mode_names(include_user_modes=False):
                click.echo(
...
class ContextCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="context", help="Manage Selene contexts. You can run `context <command> --help` for more info on each command."
        )

    @staticmethod
    @click.command("list", help="List available contexts.", context_settings={"max_content_width": _MAX_CONTENT_WIDTH})
    def list() -> None:
        context_names = SeleneAgentContext.list_registered_context_names()
        max_len_name = max(len(name) for name in context_names) if context_names else 20
        for name in context_names:
            context_yml_path = SeleneAgentContext.get_path(name)
            is_internal = Path(context_yml_path).is_relative_to(SELENES_OWN_CONTEXT_YAMLS_DIR)
    ...
    def create(name: str, from_internal: str) -> None:
        ...
        dest = os.path.join(SelenePaths().user_contexts_dir, f"{ctx_name}.yml")
        src = (
            os.path.join(SELENES_OWN_CONTEXT_YAMLS_DIR, f"{from_internal}.yml")
            if from_internal
            else os.path.join(SELENES_OWN_CONTEXT_YAMLS_DIR, "context.template.yml")
        )
        if not os.path.exists(src):
            raise FileNotFoundError(
                f"Internal context '{from_internal}' not found in {SELENES_OWN_CONTEXT_YAMLS_DIR}. Available contexts: {SeleneAgentContext.list_registered_context_names()}"
            )
        os.makedirs(os.path.dirname(dest), exist_ok=True)
        shutil.copyfile(src, dest)
    ...
    def edit(context_name: str) -> None:
        path = os.path.join(SelenePaths().user_contexts_dir, f"{context_name}.yml")
        if not os.path.exists(path):
            if context_name in SeleneAgentContext.list_registered_context_names(include_user_contexts=False):
                click.echo(
...
class SeleneConfigCommands(AutoRegisteringGroup):
    """Group for 'config' subcommands."""

    def __init__(self) -> None:
        super().__init__(name="config", help="Manage Selene configuration.")

    @staticmethod
    @click.command(
        "edit",
        help="Edit selene_config.yml in your default editor. Will create a config file from the template if no config is found.",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    def edit() -> None:
        selene_config = SeleneConfig.from_config_file()
        assert selene_config.config_file_path is not None
        _open_in_editor(selene_config.config_file_path)
...
class ProjectCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="project", help="Manage Selene projects. You can run `project <command> --help` for more info on each command."
        )
    ...
    def _create_project(project_path: str, name: str | None, language: tuple[str, ...]) -> RegisteredProject:
        ...
        selene_config = SeleneConfig.from_config_file()
        yml_path = selene_config.get_project_yml_location(str(project_root))
        ...
        registered_project = selene_config.get_registered_project(str(project_root))
        if registered_project is None:
            registered_project = RegisteredProject(str(project_root), generated_conf)
            selene_config.add_registered_project(registered_project)

        return registered_project

    @staticmethod
    @click.command("create", help="Create a new Selene project configuration.", context_settings={"max_content_width": _MAX_CONTENT_WIDTH})
    ...
    def create(project_path: str, name: str | None, language: tuple[str, ...], index: bool, log_level: str, timeout: float) -> None:
        try:
            ...
            raise click.ClickException(f"Project already exists: {e}\nUse 'selene project index' to index an existing project.")
    ...
    def index(project: str, name: str | None, language: tuple[str, ...], log_level: str, timeout: float) -> None:
        selene_config = SeleneConfig.from_config_file()
        registered_project = selene_config.get_registered_project(project, autoregister=True)
        if registered_project is None:
    ...
    def _index_project(registered_project: RegisteredProject, log_level: str, timeout: float) -> None:
        ...
        selene_config = SeleneConfig.from_config_file()
        proj = registered_project.get_project_instance(selene_config=selene_config)
        click.echo(f"Indexing symbols in {proj} …")
        ls_mgr = proj.create_language_server_manager()
        try:
            log_file = os.path.join(proj.project_root, ".selene", "logs", "indexing.txt")
    ...
    def is_ignored_path(path: str, project: str) -> None:
        ...
        selene_config = SeleneConfig.from_config_file()
        proj = Project.load(os.path.abspath(project), selene_config=selene_config)
    ...
    def health_check(project: str) -> None:
        ...
        from selene.agent import SeleneAgent
        from selene.project import Project
        from selene.tools import FindReferencingSymbolsTool, FindSymbolTool, GetSymbolsOverviewTool

        logging.configure(level=logging.INFO)
        project_path = os.path.abspath(project)
        selene_config = SeleneConfig.from_config_file().with_headless_mode_overrides()
        selene_config.language_backend = LanguageBackend.LSP
        proj = Project.load(project_path, selene_config=selene_config)

        # Create log file with timestamp
        timestamp = datetime_tag()
        log_dir = os.path.join(project_path, ".selene", "logs", "health-checks")
        ...
        with FileLoggerContext(log_file, append=False, enabled=True):
            ...
            try:
                # Create SeleneAgent with dashboard disabled
                log.info("Creating SeleneAgent with disabled dashboard...")

                agent = SeleneAgent(project=project_path, selene_config=selene_config)
                log.info("SeleneAgent created successfully")
...
class ToolCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="tools",
            help="Commands related to Selene's tools. You can run `selene tools <command> --help` for more info on each command.",
        )
    ...
    def list(quiet: bool = False, include_optional: bool = False, only_optional: bool = False) -> None:
        from selene.tools import ToolRegistry
    ...
    def description(tool_name: str, context: str | None = None) -> None:
        ...
        selene_context = None
        if context:
            selene_context = SeleneAgentContext.load(context)

        agent = SeleneAgent(
            project=None,
            selene_config=SeleneConfig(log_level=logging.INFO).with_headless_mode_overrides(),
            context=selene_context,
        )
        tool = agent.get_tool_by_name(tool_name)
        mcp_tool = SeleneMCPFactory.make_mcp_tool(tool)
...
class MemoryCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="memories",
            help="Inspect, read, write, and validate Selene memories. "
            "You can run `selene memories <command> --help` for more info on each command.",
        )
    ...
    def _load_memories_manager(project: str) -> "MemoryManager":
        ...
        Requires the project to already be a Selene project (``.selene/project.yml`` must
        exist at the configured location). Never auto-creates the ``.selene`` directory —
        if no project configuration is found, the user is directed at ``selene project create``.
        """
        selene_config = SeleneConfig.from_config_file()
        registered_project = selene_config.get_registered_project(project)
        if registered_project is None:
            raise click.UsageError(f"No Selene project found for '{project}'. Create one first.")
        return registered_project.get_project_instance(selene_config).memory_manager
    ...
    @click.command(
        ...
        help=(
            ...
            "Requires the project to already exist as a Selene project (run `selene project create` first); "
            "this command does not create a `.selene` directory on its own. "
    ...
    @click.command(
        ...
        help=(
            ...
            "\n\n"
            "Scope is narrower than what `selene memories check` reports. Only EXACT bare occurrences are rewritten "
...
class PromptCommands(AutoRegisteringGroup):
    def __init__(self) -> None:
        super().__init__(name="prompts", help="Commands related to Selene's prompts that are outside of contexts and modes.")

    @staticmethod
    def _get_user_prompt_yaml_path(prompt_yaml_name: str) -> str:
        templates_dir = SelenePaths().user_prompt_templates_dir
    ...
    def list() -> None:
        ...
        selene_prompt_yaml_names = [os.path.basename(f) for f in glob.glob(PROMPT_TEMPLATES_DIR_INTERNAL + "/*.yml")]
        for prompt_yaml_name in selene_prompt_yaml_names:
    ...
    @click.command(
        "create-override",
        help="Create an override of an internal prompts yaml for customizing Selene's prompts",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    ...
    def create_override(prompt_yaml_name: str) -> None:
        ...
        selene_prompt_yaml_path = os.path.join(PROMPT_TEMPLATES_DIR_INTERNAL, prompt_yaml_name)
        shutil.copyfile(selene_prompt_yaml_path, user_prompt_yaml_path)
    ...
    def list_overrides() -> None:
        ...
        for file_path in override_files:
            if os.path.basename(file_path) in selene_prompt_yaml_names:
    ...
    @click.command(
        ...
        "This respects custom prompt yaml overrides and thus will print the value that will be used in Selene",
        context_settings={"max_content_width": _MAX_CONTENT_WIDTH},
    )
    @click.argument("prompt_name", type=str)
    def print_prompt_template(prompt_name: str) -> None:
        click.echo(SelenePromptFactory().get_prompt_template_string(prompt_name))
    ...
    def print_cc_system_prompt_override() -> None:
        click.echo(SelenePromptFactory().create_cc_system_prompt_override())


_mode = ModeCommands()
_context = ContextCommands()
_project = ProjectCommands()
_config = SeleneConfigCommands()
_tools = ToolCommands()
