...
from selene.analytics import RegisteredTokenCountEstimator, ToolUsageStats
...
from selene.dashboard import SeleneDashboardAPI, SeleneDashboardTrayManager, SeleneDashboardViewer, open_url_in_browser
from selene.jetbrains import launch_coordinator as jetbrains_launch_coordinator
...
from selene.util.gui import system_has_usable_display
...
if TYPE_CHECKING:
    from selene.gui_log_viewer import GuiLogViewer
...
class AvailableTools:
    """
    Represents the set of available/exposed tools of a SeleneAgent.
    """
...
class ActiveModes:
    _mode_instances: dict[str, SeleneAgentMode] = {}

    def __init__(self, background_base_modes: Sequence[SeleneAgentMode] | None = None) -> None:
        """
    ...
    @classmethod
    def get_mode_instance(cls, mode_name: str) -> SeleneAgentMode:
        if mode_name not in cls._mode_instances:
            cls._mode_instances[mode_name] = SeleneAgentMode.load(mode_name)
        return cls._mode_instances[mode_name]

    def get_modes(self, include_background_base_modes: bool = False) -> Sequence[SeleneAgentMode]:
        result: list[SeleneAgentMode] = []
    ...
    def get_dynamically_activated_modes(self) -> Sequence[SeleneAgentMode]:
        return [self.get_mode_instance(mode_name) for mode_name in self._dynamically_activated_mode_names]

    def get_base_modes(self, include_background_base_modes: bool = False) -> Sequence[SeleneAgentMode]:
        result: list[SeleneAgentMode] = []
        if include_background_base_modes:
            result.extend(self._background_base_modes)
...
class ProjectPromptProvisionStatus:
    ...
    def is_mode_prompt_already_provided(self, mode_name: str, session_id: str) -> bool:
        ...
        :return: whether the mode name was already provided (in a project-specific activation message) and therefore
            should not be included again (in the Selene instructions manual)
        """
        if not self._get_session_status(session_id).mode_prompts_provided:
            return False
        return mode_name in self._newly_activated_mode_names

    def get_modes_with_prompts_to_be_provided_for_project_activation(self, session_id: str) -> list[SeleneAgentMode]:
        """
...
class DashboardManager:
    class Mode(Enum):
        ...
        def is_supported(self) -> bool:
            ...
            if self == DashboardManager.Mode.WEBVIEW:
                return SeleneDashboardViewer.is_current_platform_supported()
            elif self == DashboardManager.Mode.TRAY_MANAGER:
                return SeleneDashboardTrayManager.is_current_platform_supported()
            else:
                return True
            ...
            case self.Mode.BROWSER:
                if open_dashboard_on_launch:
                    if not system_has_usable_display():
                        log.info("Not opening the Selene dashboard because no usable display was detected.")
                    else:
    ...
    def _start_dashboard_viewer_process_function(url: str, minimized: bool, parent_process_id: int) -> None:
        ...
        """
        try:
            SeleneDashboardViewer(url, start_minimized=minimized, parent_process_id=parent_process_id).run()
        except webview.errors.WebViewException as e:
            log.warning(f"Could not open Selene Dashboard viewer. Cause:\n{e}")
    ...
    def _tray_manager_register(self, open_on_launch: bool, active_project: Project | None) -> None:
        ...
        with LogTime("Dashboard tray manager initialisation"):
            with self._tray_manager_lock:
                ...
                SeleneDashboardTrayManager.ensure_running()
                ...
                SeleneDashboardTrayManager.register_instance(
                    port=self._port,
    ...
    def shutdown(self) -> None:
        ...
        if self._mode == self.Mode.TRAY_MANAGER:
            with self._tray_manager_lock:
                SeleneDashboardTrayManager.unregister_instance(port=self._port)
    ...
    def update_active_project(self, active_project: Project | None) -> None:
        ...
        if self._mode == self.Mode.TRAY_MANAGER:
            with self._tray_manager_lock:
                project_name = active_project.project_name if active_project is not None else None
                SeleneDashboardTrayManager.update_project(port=self._port, project=project_name)
...
class SeleneAgent:
    def __init__(
        ...
        *,
        ...
        selene_config: SeleneConfig | None = None,
        context: SeleneAgentContext | None = None,
        ...
        :param project_activation_error: an initial error to report back to the client/LLM pertaining to project determination/activation
            in Selene's initial prompts. This is only applicable if `project` is None.
        :param selene_config: the Selene configuration or None to read the configuration from the default location.
        ...
        self.version = selene_version()
        self._config_changed_callbacks: list[Callable[[], None]] = []

        # obtain selene configuration using the decoupled factory function
        self.selene_config = selene_config or SeleneConfig.from_config_file()

        # propagate configuration to other components
        self.selene_config.propagate_settings()

        # determine registered project to be activated (if any)
        registered_project_to_activate: RegisteredProject | None = (
            self.selene_config.get_registered_project(project, autoregister=True) if project is not None else None
        )

        # adjust log level
        selene_log_level = self.selene_config.log_level
        if Logger.root.level != selene_log_level:
            log.info(f"Changing the root logger level to {selene_log_level}")
            Logger.root.setLevel(selene_log_level)

        def get_memory_log_handler() -> MemoryLogHandler:
            nonlocal memory_log_handler
            if memory_log_handler is None:
                memory_log_handler = MemoryLogHandler(level=selene_log_level)
                Logger.root.addHandler(memory_log_handler)
            return memory_log_handler

        # open GUI log window if enabled
        if self.selene_config.gui_log_window:
        ...
        if context is None:
            context = SeleneAgentContext.load_default()
        ...
        token_count_estimator = RegisteredTokenCountEstimator[self.selene_config.token_count_estimator]
        ...
        log.info(
            f"Starting Selene server (version={self.version}, process id={os.getpid()}, parent process id={os.getppid()}; "
            f"language backend={self.selene_config.language_backend.name}); Python version={platform.python_version()}, platform={platform.platform()}"
        )
        log.info("Configuration file: %s", self.selene_config.config_file_path)
        log.info("Available projects: {}".format(", ".join(self.selene_config.project_names)))
        ...
        # Since we don't want to change the toolset after startup, the language backend cannot be changed within a running Selene session
        self._language_backend = self.selene_config.determine_language_backend(
        ...
        self._task_executor = TaskExecutor("SeleneAgentTaskExecutor", self._task_completion_callback)

        # Initialize the prompt factory
        self.prompt_factory = SelenePromptFactory()
        ...
        self._base_toolset = self._create_base_toolset(self.selene_config, self._context, self._active_modes, self._active_project)
        ...
        dashboard_api: SeleneDashboardAPI | None = None
        if self.selene_config.web_dashboard:
            dashboard_api = SeleneDashboardAPI(
                get_memory_log_handler(),
                tool_names,
                agent=self,
                tool_usage_stats=self._tool_usage_stats,
                host=self.selene_config.web_dashboard_listen_address,
                trusted_hosts=self.selene_config.web_dashboard_trusted_hosts,
            )
        ...
        if dashboard_api:
            ...
            self._dashboard_manager = DashboardManager(
                port,
                self.selene_config.web_dashboard_listen_address,
                self.selene_config.web_dashboard_open_on_launch,
                self._active_project,
                mode_str=self.selene_config.web_dashboard_interface,
            )
            log.info("Selene web dashboard started at %s", self._dashboard_manager.url)
    ...
    def _create_base_toolset(
        ...
        """
        # determine whether to include the OpenDashboardTool based on the Selene configuration
        tool_inclusion_definitions: list[ToolInclusionDefinition] = []
        if selene_config.web_dashboard and not selene_config.web_dashboard_open_on_launch and not selene_config.gui_log_window:
        ...
        # consider Selene configuration and the active context
        tool_inclusion_definitions.append(selene_config)
    ...
    def get_log_inspection_instructions(self) -> str:
        if self.selene_config.web_dashboard:
            ...
            log_path = SelenePaths().last_returned_log_file_path
    ...
    def get_context(self) -> SeleneAgentContext:
    ...
    def open_dashboard(self) -> bool:
        """
        Opens the Selene dashboard (for on-demand usage as triggered by the user, e.g. via a tool)
        ...
        if not system_has_usable_display():
            log.warning("Not opening the Selene web dashboard because no usable display was detected.")
            return False
    ...
    def _create_global_memory_manager(self) -> MemoryManager:
        ...
        """
        return MemoryManager(selene_data_folder=None, read_only_memory_patterns=self.selene_config.read_only_memory_patterns)
    ...
    def create_system_prompt(self, session_id: str = "global") -> str:
        """
        Returns the 'Selene Instructions Manual', i.e. Selene's system prompt.
        ...
        return self._format_prompt_tag(system_prompt, tag="selene")
    ...
    def _update_active_modes(self, log_message: bool = True) -> None:
        """
        Updates the active modes based on the Selene configuration, the active project configuration (if any),
        and mode overrides (if any).
        """
        background_base_modes = []
        if self._language_backend.is_jetbrains():
            background_base_modes.append(SeleneAgentMode.from_name_internal("jetbrains"))
        self._active_modes = ActiveModes(background_base_modes=background_base_modes)
        self._active_modes.apply(self.selene_config)
    ...
    def _update_active_tools(self) -> None:
        ...
        The base tool set already takes the Selene configuration and the context into account
    ...
    def _init_active_project_language_backend(self) -> None:
        ...
        # for JetBrains mode, search for plugin server and spawn IDE (if not found and launch command provided)
            ...
            if client is not None:
                log.info("Found Selene JetBrains Plugin server: %s", client)
            else:
                log.info("Selene JetBrains Plugin server not found for project %s", project.project_name)
                if self.selene_config.jetbrains_launch_command:
                    jetbrains_launch_coordinator.launch_and_wait_for_plugin_server(project, self.selene_config.jetbrains_launch_command)
    ...
    def activate_project_from_path_or_name(
        ...
        If the argument is a path at which no Selene project previously existed, the project will be created beforehand.
        Raises ProjectNotFoundError if the project could neither be found nor created.

        :return: True if the project was newly activated, False if it was already active
        """
        project_instance: Project | None = self.selene_config.get_project(project_root_or_name)
        if project_instance is not None:
            ...
            project_instance = self.selene_config.add_project_from_path(project_root_or_name, asynchronous_autogen=True)
        ...
        if project_instance is None:
            raise ProjectNotFoundError(
                ...
                f"Existing project names: {self.selene_config.project_names}"
            )
    ...
    def get_current_config_overview(self) -> str:
        ...
        result_str += f"Selene version: {self.version}\n"
        result_str += f"Loglevel: {self.selene_config.log_level}, trace_lsp_communication={self.selene_config.trace_lsp_communication}\n"
        if self._active_project is not None:
        ...
        result_str += f" (global default: {self.selene_config.language_backend.value})\n"
        ...
        result_str += "Available projects:\n" + "\n".join(list(self.selene_config.project_names)) + "\n"
        ...
        all_available_modes = SeleneAgentMode.list_registered_mode_names()
    ...
    def on_shutdown(self, timeout: float = 2.0) -> None:
        ...
        """
        log.info("SeleneAgent is shutting down ...")
