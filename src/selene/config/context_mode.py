...
from selene.config.selene_config import SelenePaths, ToolInclusionDefinition
...
class SeleneAgentMode(ToolInclusionDefinition, ToStringMixin):
    ...
    It is formatted by the agent (see SeleneAgent._format_prompt()).
    """
    description: str = ""
    ...
    def from_yaml(cls, yaml_path: str | Path) -> Self:
        ...
        with Path(yaml_as_path).open(encoding=SELENE_FILE_ENCODING) as f:
    ...
    def get_path(cls, name: str, instance: Self | None = None) -> str:
        ...
        fname = f"{name}.yml"
        custom_mode_path = os.path.join(SelenePaths().user_modes_dir, fname)
        if os.path.exists(custom_mode_path):
            return custom_mode_path

        own_yaml_path = os.path.join(SELENES_OWN_MODE_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Mode {name} not found in {SelenePaths().user_modes_dir} or in {SELENES_OWN_MODE_YAMLS_DIR}."
    ...
    def list_registered_mode_names(cls, include_user_modes: bool = True) -> list[str]:
        """Names of all registered modes (from the corresponding YAML files in the selene repo)."""
        modes = [f.stem for f in Path(SELENES_OWN_MODE_YAMLS_DIR).glob("*.yml") if f.name != "mode.template.yml"]
    ...
    def list_custom_mode_names(cls) -> list[str]:
        ...
        return [f.stem for f in Path(SelenePaths().user_modes_dir).glob("*.yml")]
...
@dataclass(kw_only=True)
class SeleneAgentContext(ToolInclusionDefinition, ToStringMixin):
    ...
    """
    whether to assume that Selene shall only work on a single project in this context (provided that a project is given
    when Selene is started).
    ...
    def from_yaml(cls, yaml_path: str | Path) -> Self:
        ...
        yaml_as_path = Path(yaml_path).resolve()
        with yaml_as_path.open(encoding=SELENE_FILE_ENCODING) as f:
            data = yaml.safe_load(f)
        name = data.pop("name", yaml_as_path.stem)
    ...
    def get_path(cls, name: str, instance: Self | None = None) -> str:
        ...
        fname = f"{name}.yml"
        custom_context_path = os.path.join(SelenePaths().user_contexts_dir, fname)
        if os.path.exists(custom_context_path):
            return custom_context_path

        own_yaml_path = os.path.join(SELENES_OWN_CONTEXT_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Context {name} not found in {SelenePaths().user_contexts_dir} or in {SELENES_OWN_CONTEXT_YAMLS_DIR}."
    ...
    @classmethod
    def from_name(cls, name: str) -> Self:
        """Load a registered Selene context."""
        legacy_name_mapping = {
        ...
        if name in legacy_name_mapping:
            log.warning(
                ...
                "docs/02-usage/050_configuration.md#contexts"
            )
    ...
    def list_registered_context_names(cls, include_user_contexts: bool = True) -> list[str]:
        """Names of all registered contexts (from the corresponding YAML files in the selene repo)."""
        contexts = [f.stem for f in Path(SELENES_OWN_CONTEXT_YAMLS_DIR).glob("*.yml")]
    ...
    def list_custom_context_names(cls) -> list[str]:
        ...
        return [f.stem for f in Path(SelenePaths().user_contexts_dir).glob("*.yml")]
