"""
The Selene Model Context Protocol (MCP) Server
"""
...
from selene.constants import (
    ...
    REPO_ROOT,
    RESOURCES_DIR,
    SELENE_CONFIG_TEMPLATE_FILE,
    SELENE_FILE_ENCODING,
    SELENE_MANAGED_DIR_NAME,
)
from selene.util.inspection import compute_language_server_support_composition
from selene.util.text_utils import GlobMatcher
from selene.util.yaml import YamlCommentNormalisation, load_yaml, normalise_yaml_comments, save_yaml, transfer_yaml_comments
...
class SelenePaths:
    """
    Provides paths to various Selene-related directories and files.
    """

    def __init__(self) -> None:
        home_dir = os.getenv("SELENE_HOME")
        if home_dir is None or home_dir.strip() == "":
            home_dir = str(Path.home() / SELENE_MANAGED_DIR_NAME)
        else:
            home_dir = home_dir.strip()
        self.resources_dir: str = RESOURCES_DIR
        """
        the resources directory (within the `selene` package)
        """
        self.selene_user_home_dir: str = home_dir
        """
        the path to the Selene home directory, where the user's configuration/data is stored.
        This is ~/.selene by default, but it can be overridden via the SELENE_HOME environment variable.
        """
        self.user_prompt_templates_dir: str = os.path.join(self.selene_user_home_dir, "prompt_templates")
        """
        directory containing prompt templates defined by the user.
        Prompts defined by the user take precedence over Selene's built-in prompt templates.
        """
        self.user_contexts_dir: str = os.path.join(self.selene_user_home_dir, "contexts")
        """
        directory containing contexts defined by the user.
        If a name of a context matches a name of a context in SELENES_OWN_CONTEXT_YAMLS_DIR,
        the user context will override the default context definition.
        """
        self.user_modes_dir: str = os.path.join(self.selene_user_home_dir, "modes")
        """
        directory containing modes defined by the user.
        If a name of a mode matches a name of a mode in SELENES_OWN_MODES_YAML_DIR,
        the user mode will override the default mode definition.
        """
        global_memories_path = Path(os.path.join(self.selene_user_home_dir, "memories", "global"))
    ...
    def get_next_log_file_path(self, prefix: str) -> str:
        ...
        """
        log_dir = os.path.join(self.selene_user_home_dir, "logs", datetime.now().strftime("%Y-%m-%d"))
...
class ToolInclusionDefinition:
    """
    Defines which tools to include/exclude in Selene's operation.
...
class LanguageBackend(Enum):
    ...
    via the SolidLSP library that is part of Selene
    """
    JETBRAINS = "JetBrains"
    """
    Use the Selene plugin in your JetBrains IDE.
...
class SharedConfig(ToolInclusionDefinition, ToStringMixin):
    """Shared between SeleneConfig and ProjectConfig, the latter used to override values in the form
    (same as in ModeSelectionDefinition).
    The defaults here shall be none and should be set to the global default values in SeleneConfig.
    """
...
class SeleneConfigError(Exception):
    pass


DEFAULT_PROJECT_SELENE_FOLDER_LOCATION = "$projectDir/" + SELENE_MANAGED_DIR_NAME
"""
The default template for the project Selene folder location.
...
class ProjectConfig(SharedConfig, ModeSelectionDefinitionWithAddedModes):
    ...
    # class-level members
    SELENE_PROJECT_FILE = "project.yml"
    SELENE_LOCAL_PROJECT_FILE = "project.local.yml"
    ...
    def _determine_project_language_servers(
        cls, project_root: str, interactive: bool, selene_config: "SeleneConfig"
        ...
        for language in LanguageServerId:
            priority = selene_config.get_ls_priority(language)
            if priority > 0:
        ...
        if len(ls_composition) == 0:
            log.warning(
                ...
                "when using the LSP backend. You can add languages later via the Selene dashboard "
            ...
            if len(other_language_pairs) > 0 and interactive:
                ...
                print("      system-level installations/configuration (see Selene documentation).")
    ...
    def autogenerate(
        ...
        :param selene_config: the global Selene configuration
        ...
        with LogTime("Project configuration auto-generation", logger=log):
            ...
            if languages is None:
                if asynchronous:
                    ...
                    determined_languages = cls._determine_project_language_servers(
                        str(project_root), interactive=interactive, selene_config=selene_config
                    )
            ...
            project_yml_path = selene_config.get_project_yml_location(str(project_root))
            if save_to_disk:
                log.info("Saving project configuration to %s", project_yml_path)
                with cls._save_lock:
                    save_yaml(project_yml_path, config_with_comments)
                project_local_yml_path = os.path.join(os.path.dirname(project_yml_path), cls.SELENE_LOCAL_PROJECT_FILE)
                    ...
                    try:
                        with LogTime("Asynchronous language determination", logger=log):
                            project_config.language_servers = cls._determine_project_language_servers(
                                str(project_root), interactive=False, selene_config=selene_config
                            )
                            if save_to_disk:
    ...
    def default_project_yml_path(cls, project_root: str | Path) -> str:
        """
        :return: the default path to the project.yml file (inside ``$projectDir/.selene/``).
            This is suitable as a fallback when no ``SeleneConfig`` is available to resolve
            a potentially customised location.
        """
        return os.path.join(str(project_root), SELENE_MANAGED_DIR_NAME, cls.SELENE_PROJECT_FILE)
    ...
    def _project_local_yml_path(cls, project_yml_path: str) -> str:
        return os.path.join(os.path.dirname(project_yml_path), cls.SELENE_LOCAL_PROJECT_FILE)
    ...
    def load(
        ...
        project_folder_name = project_root.name
        yaml_path = selene_config.get_project_yml_location(project_root)
        log.debug("Loading project configuration from %s", yaml_path)

        # auto-generate if necessary
        if not os.path.exists(yaml_path):
            if autogen.is_autogen_enabled():
                return cls.autogenerate(project_root, selene_config, asynchronous=autogen == ProjectConfigAutoGenerationMode.ASYNCHRONOUS)
            else:
...
class RegisteredProject(ToStringMixin):
    def __init__(
        ...
        """
        Represents a registered project in the Selene configuration.
    ...
    def from_project_root(
        ...
        :param selene_config: the Selene configuration
        ...
        """
        project_config = ProjectConfig.load(project_root, selene_config=selene_config, autogen=autogen)
        return RegisteredProject(
    ...
    def get_project_instance(self, selene_config: "SeleneConfig") -> "Project":
        """
...
@dataclass(kw_only=True)
class SeleneConfig(SharedConfig, ModeSelectionDefinitionWithBaseModes):
    """
    Holds the Selene agent configuration, which is typically loaded from a YAML configuration file
    ...
    on the first run, which requires internet access. Token counting itself is local; the default CHAR_COUNT requires no downloads.
    """
    ...
    project_selene_folder_location: str = DEFAULT_PROJECT_SELENE_FOLDER_LOCATION
    """
    Template for the location of the per-project .selene data folder (memories, caches, etc.).
    Supports the following placeholders:
      - $projectDir: the absolute path to the project root directory
      - $projectFolderName: the name of the project folder
    Examples:
      - "$projectDir/.selene" (default, stores data inside the project)
      - "/projects-metadata/$projectFolderName/.selene" (stores data in a central location)
    """
    ...
    If the budget is exceeded, Selene stops issuing further requests and returns partial info results.
    ...
    def _generate_config_file(cls, config_file_path: str) -> None:
        """
        Generates a Selene configuration file at the specified path from the template file.

        :param config_file_path: the path where the configuration file should be generated
        """
        log.info(f"Auto-generating Selene configuration file in {config_file_path}")
        loaded_commented_yaml = load_yaml(SELENE_CONFIG_TEMPLATE_FILE)
        save_yaml(config_file_path, loaded_commented_yaml)

    @classmethod
    def _determine_config_file_path(cls) -> str:
        """
        :return: the location where the Selene configuration file is stored/should be stored
        """
        config_path = os.path.join(SelenePaths().selene_user_home_dir, cls.CONFIG_FILE)
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(old_config_path):
                log.info(f"Moving Selene configuration file from {old_config_path} to {config_path}")
    ...
    @classmethod
    def from_config_file(cls, generate_if_missing: bool = True) -> "SeleneConfig":
        """
        Static constructor to create SeleneConfig from the configuration file
        """
        config_file_path = cls._determine_config_file_path()
        ...
        if not os.path.exists(config_file_path):
            if not generate_if_missing:
                raise FileNotFoundError(f"Selene configuration file not found: {config_file_path}")
            log.info(f"Selene configuration file not found at {config_file_path}, autogenerating...")
            cls._generate_config_file(config_file_path)

        # load the configuration
        log.info(f"Loading Selene configuration from {config_file_path}")
        try:
            loaded_commented_yaml = load_yaml(config_file_path)
        except Exception as e:
            raise ValueError(f"Error loading Selene configuration from {config_file_path}: {e}") from e
        ...
        def get_value_or_default(field_name: str) -> Any:
            ...
            if field_name not in loaded_commented_yaml:
                num_migrations += 1
            return loaded_commented_yaml.get(field_name, get_dataclass_default(SeleneConfig, field_name))
        ...
        if "projects" not in loaded_commented_yaml:
            raise SeleneConfigError("`projects` key not found in Selene configuration. Please update your `selene_config.yml` file.")
        instance.projects = []
        for path in loaded_commented_yaml["projects"] or []:
            ...
            if path.is_file():
                ...
                num_migrations += 1
            try:
                project_config = ProjectConfig.load(path, selene_config=instance)  # instance is sufficiently populated
            except Exception as e:
                log.error(
                    "Failed to load project configuration for %s: %s. "
                    "This project will be skipped. Fix or delete its "
                    ".selene/project.yml (or remove it from "
                    "selene_config.yml) to re-enable it.",
                    path,
                    e,
                )
                continue
        ...
        language_backend = get_dataclass_default(SeleneConfig, "language_backend")
        ...
        if line_ending_value:
            ...
            num_migrations += 1
            instance.line_ending = get_dataclass_default(SeleneConfig, "line_ending")
    ...
    def _migrate_out_of_project_config_file(cls, path: Path) -> Path | None:
        ...
        try:
            with open(path, encoding=SELENE_FILE_ENCODING) as f:
                project_config_data = yaml.safe_load(f)
            if "project_name" not in project_config_data:
                project_name = path.stem
                with open(path, "a", encoding=SELENE_FILE_ENCODING) as f:
    ...
    @classmethod
    def init(cls, language_backend: LanguageBackend) -> "SeleneConfig":
        """
        ...
        :return: the created SeleneConfig instance
        """
    ...
    def with_headless_mode_overrides(self) -> "SeleneConfig":
        """
        Modifies this instance to apply overrides for headless mode, where any GUI/user interaction-based features are disabled.
        This is intended to be applied for cases where a `SeleneConfig` instance is needed to instantiate a `SeleneAgent` instance
    ...
    def get_registered_project(self, project_root_or_name: str, autoregister: bool = False) -> Optional[RegisteredProject]:
        ...
        :param autoregister: whether to auto-register projects that are not yet registered in Selene's global configuration
        ...
        if autoregister:
            ...
            if os.path.isfile(config_path):
                registered_project = RegisteredProject.from_project_root(project_root_or_name, selene_config=self)
    ...
    def get_project(self, project_root_or_name: str) -> Optional["Project"]:
        ...
        if registered_project is None:
            ...
            return registered_project.get_project_instance(selene_config=self)
    ...
    def add_project_from_path(self, project_root: Path | str, asynchronous_autogen: bool = False) -> "Project":
        """
        Adds a new project to the Selene configuration from a given path, auto-generating the project
        ...
        project_config = ProjectConfig.load(project_root, selene_config=self, autogen=autogen)
    ...
    def remove_project(self, project_name: str) -> None:
        ...
        for i, project in enumerate(list(self.projects)):
            ...
            raise ValueError(f"Project '{project_name}' not found in Selene configuration; valid project names: {self.project_names}")
        self._persist_projects()
    ...
    def _persist_projects(self) -> None:
        ...
        persisted = SeleneConfig.from_config_file()
    ...
    def _save(self) -> None:
        ...
        template_yaml = load_yaml(SELENE_CONFIG_TEMPLATE_FILE, comment_normalisation=YamlCommentNormalisation.LEADING)
    ...
    @staticmethod
    def _resolve_selene_folder_location(template: str, placeholders: dict[str, str]) -> str:
        """
        Resolves a folder location template by replacing known ``$placeholder`` tokens
        and raising on any unrecognised ones.

        :param template: the template string (e.g. ``"$projectDir/.selene"``)
        :param placeholders: mapping from placeholder name (without ``$``) to replacement value
        :return: the resolved absolute path
        :raises SeleneConfigError: if the template contains an unknown ``$placeholder``
        """

        def _replace(match: re.Match[str]) -> str:
            name = match.group(1)
            if name not in placeholders:
                raise SeleneConfigError(
                    f"Unknown placeholder '${name}' in project_selene_folder_location. "
    ...
    def get_configured_project_selene_folder(self, project_root: str | Path) -> str:
        """
        Returns the resolved absolute path to the .selene data folder for a project,
        applying placeholder substitution to ``project_selene_folder_location``
        without any fallback logic.

        :param project_root: the absolute path to the project root directory
        :return: the resolved absolute path to the project's .selene folder
        :raises SeleneConfigError: if the template contains an unknown placeholder
        """
        ...
        return self._resolve_selene_folder_location(self.project_selene_folder_location, placeholders)

    def get_project_selene_folder(self, project_root: str | Path) -> str:
        """
        Resolves the location of the project's .selene data folder using fallback logic:

        1. If the folder exists at the configured path (``project_selene_folder_location``), use it.
        ...
        :param project_root: the absolute path to the project root directory
        :return: the resolved absolute path to the .selene data folder
        :raises SeleneConfigError: if the configured template contains an unknown placeholder
        """
        configured_path = self.get_configured_project_selene_folder(project_root)
        if os.path.isdir(configured_path):
            return configured_path
        default_path = os.path.join(str(project_root), SELENE_MANAGED_DIR_NAME)
    ...
    def get_project_yml_location(self, project_root: str | Path) -> str:
        ...
        based on the resolved .selene data folder (with fallback logic).

        :param project_root: the absolute path to the project root directory
        :return: the resolved absolute path to the project's project.yml file
        """
        selene_folder = self.get_project_selene_folder(project_root)
        return os.path.join(selene_folder, ProjectConfig.SELENE_PROJECT_FILE)
    ...
    def propagate_settings(self) -> None:
        ...
        """
        from selene.tools import JetBrainsPluginClient
