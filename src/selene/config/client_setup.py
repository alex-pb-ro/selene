...
class ClientSetupHandler(ABC):
    ...
    def get_mcp_server_command(self) -> str:
        return f"selene start-mcp-server {' '.join(self.get_mcp_server_options())}"
...
class ClientSetupHandlerClaudeCode(ClientSetupHandler):
    ...
    def apply(self) -> bool:
        cmd = f"claude mcp add --scope user selene -- {self.get_mcp_server_command()}"
        ...
        if is_success:
            ...
            click.echo("   docs/02-usage/030_clients.md#claude-code")
        return is_success
...
class ClientSetupHandlerCodex(ClientSetupHandler):
    ...
    def apply(self) -> bool:
        return self._run_shell_command(f"codex mcp add selene -- {self.get_mcp_server_command()}")
...
class ClientSetupHandlerCodeBuddy(ClientSetupHandler):
    ...
    def apply(self) -> bool:
        cmd = f"codebuddy mcp add --scope user selene -- {self.get_mcp_server_command()}"
        is_success = self._run_shell_command(cmd)
        if is_success:
            click.echo("\nIMPORTANT: We additionally recommend to set up hooks for CodeBuddy to ensure the best experience.")
            click.echo("   Please read the instructions here:")
            click.echo("   docs/02-usage/030_clients.md#codebuddy")
        return is_success
...
class ClientSetupHandlerGrok(ClientSetupHandler):
    ...
    def apply(self) -> bool:
        cmd = f"grok mcp add --scope user selene -- {self.get_mcp_server_command()}"
        is_success = self._run_shell_command(cmd)
        if is_success:
            click.echo("\nIMPORTANT: We additionally recommend to set up hooks for Grok to ensure the best experience.")
            click.echo("   Please read the instructions here:")
            click.echo("   docs/02-usage/030_clients.md#grok")
        return is_success
