...
def save_yaml(path: str, data: dict | CommentedMap, preserve_comments: bool = True) -> None:
    ...
    # Selene process updating the auto-managed registered-projects list) or an interrupted write can
    ...
    try:
        with os.fdopen(fd, "w", encoding=SELENE_FILE_ENCODING) as f:
            yaml.dump(data, f)
...
def _replace_with_retry(src: str, dst: str, *, attempts: int = 10, delay_s: float = 0.05) -> None:
    ...
    momentarily holds ``dst`` open — e.g. a second Selene process reading or replacing the same
