...
from solidlsp.util.persistence import DataOnlyUnpickler

# copied from selene_config.py, we don't want to import anything here to keep the hook commands fast
selene_home_dir = os.getenv("SELENE_HOME", "").strip() or str(Path.home() / ".selene")
...
class Hook(ABC):
    def __init__(self, client: HookClient):
        ...
        if self._session_id in {".", ".."} or any(c in self._session_id for c in "/\\:\x00"):
            raise ValueError("Session ID must be a single safe path component")
        self.session_persistence_dir = os.path.join(selene_home_dir, "hook_data", self._session_id)
...
#: substrings that mark a "selene"-containing tool name as one of Selene's own non-symbolic
...
_NON_SYMBOLIC_SELENE_TOOL_NAME_SUBSTRINGS = frozenset(
    (
        "pattern",
        "read",
        "diagnostics",
...
def _is_selene_symbolic_tool_name(tool_name: str) -> bool:
    return "selene" in tool_name and not any(substring in tool_name for substring in _NON_SYMBOLIC_SELENE_TOOL_NAME_SUBSTRINGS)
...
class PreToolUseHook(Hook, ABC):
    ...
    def is_selene_symbolic_tool(self) -> bool:
        return _is_selene_symbolic_tool_name(self._tool_name)


class PreToolUseRemindAboutSymbolicToolsHook(PreToolUseHook):
    """Pre-tool-use hook that nudges the agent toward Selene's symbolic tools.
    ...
    * a Selene tool is invoked (both counters are reset),
    ...
    * or the configured reset period elapses *between two consecutive calls of that
    ...
    class ToolUseCounter:
        ...
        # :meth:`reset` so the rate limit survives counter resets (e.g. Selene tool use)
        ...
        def load(cls, hook: Hook) -> Self:
            ...
            try:
                with open(path, "rb") as f:
                    return DataOnlyUnpickler(f, (cls, datetime)).load()
            except Exception:
                return cls()
        ...
        def update(self, hook: "PreToolUseRemindAboutSymbolicToolsHook") -> None:
            if hook.is_selene_symbolic_tool():
                self.reset()
                return
    ...
    def _build_grep_deny(self) -> "PreToolUseHook.OutputData":
        return self.OutputData(
            ...
            additional_context=(
                "You were using many grep calls recently. Consider using Selene's symbolic "
    ...
    def _build_code_read_deny(self) -> "PreToolUseHook.OutputData":
        return self.OutputData(
            ...
            additional_context=(
                "You were using many read calls on files recently. Consider using Selene's symbolic "
    ...
    def _build_non_symbolic_deny(self) -> "PreToolUseHook.OutputData":
        return self.OutputData(
            ...
            additional_context=(
                ...
                "Selene's symbolic mcp tools. Consider using symbolic search and targeted symbol "
...
class PostToolUseResetSymbolicToolCounterHook(Hook):
    ...
    persisted counters after a successful Selene symbolic tool call.

    ``PreToolUseRemindAboutSymbolicToolsHook`` already resets on a Selene tool call, but
    only when it is itself invoked for that call, which requires the client's PreToolUse
    matcher to observe ``mcp__selene__*`` tool names. Codex's documented wiring (see
    docs/02-usage/030_clients.md) attaches ``remind`` to the ``Bash`` matcher only, so it
    is never invoked for Selene's own tools there and the reset branch is unreachable.
    This hook closes that gap from the other side of the call: wired to PostToolUse with a
    matcher on Selene's tools, it fires once the call has completed.

    Gated on the call having succeeded (``tool_response`` carrying no ``isError: true``,
    the MCP ``tools/call`` result shape) so a failed Selene call does not mask a real
    ...
    def execute(self) -> None:
        if not _is_selene_symbolic_tool_name(self._tool_name) or not self._call_succeeded():
            return
...
class SessionStartActivateProjectHook(Hook):
    def execute(self) -> None:
        message = (
            ...
            " 1. activate it using Selene's activate_project tool unless already done. "
            " 2. if you haven't read Selene Instructions Manual yet, do so. "
...
class PreToolUseAutoApproveSeleneHook(PreToolUseHook):
    """Pre-tool-use hook that auto-approves Selene tool calls while the client is in a permissive permission mode.
    ...
    its auto-mode classifier; Selene's destructive tools (e.g. ``replace_symbol_body`` or
    ``rename_symbol``) would still prompt the user on every call. This hook emits an ``allow``
    decision for any Selene MCP tool call whenever the client reports one of these modes as
    the active permission mode, so blanket approvals also cover Selene's tools. In all other
    ...
    def execute(self) -> None:
        ...
        if not self.is_selene_symbolic_tool() or not self.is_auto_approve_mode():
            return
        ...
        output_data = self.OutputData(
            ...
            permission_decision_reason=f"Auto-approved: Selene tool call while client is in {self._permission_mode} mode.",
        )
        click.echo(output_data.to_json_string(self._client))
...
class HookCommands(AutoRegisteringGroup):
    def __init__(self) -> None:
        super().__init__(name="selene-hook", help="Commands that send reminders to agents when appropriate, to be used in hooks.")

    @staticmethod
    @click.command(
        "activate",
        help="Set this as hook at session startup to prompt the agent to activate the project at the start of the session and read Selene's instructions",
    )
    @_client_option
    ...
    @click.command(
        "remind",
        help="Set this as hook at PreToolUse to remind the agent to use Selene's tools instead of overrelying on read_file and grep",
    )
    @_client_option
    ...
    @click.command(
        "auto-approve",
        help="Set this as hook at PreToolUse to auto-approve Selene tool calls while the client is in a "
        "permissive permission mode (acceptEdits or auto, Claude Code).",
    )
    @_client_option
    def auto_approve(client: str) -> None:
        PreToolUseAutoApproveSeleneHook(HookClient(client)).execute()

    @staticmethod
    @click.command(
        "reset",
        help="Set this as hook at PostToolUse, matched to Selene's own tools, to reset the grep/read-drift "
        "counters after a successful Selene tool call. For clients whose PreToolUse wiring does not observe "
        "Selene tool calls (e.g. Codex, matched to Bash only); complements `remind`'s own reset branch.",
    )
    @_client_option
