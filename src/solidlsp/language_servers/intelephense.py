...
class Intelephense(SolidLanguageServer):
    ...
    You can pass the following entries in ls_specific_settings["php"]:
        ...
        - file_filter: list of additional file extensions (with leading dot) to treat as PHP
          ...
          ".phtml") of Selene's PHP source-file matcher and are pushed to Intelephense via
    ...
    def __init__(self, config: LanguageServerConfig, repository_root_path: str, solidlsp_settings: SolidLSPSettings):
        ...
        # such that Selene's symbol tools treat the added extensions as PHP sources as well (#1710)
    ...
    def _start_server(self) -> None:
        ...
        # Intelephense reads these settings before emitting its initialized telemetry event.
        associations = [f"*{ext}" for ext in self.ls_id.get_source_fn_matcher().file_extensions]
        settings = {"telemetry": {"enabled": False}, "files": {"associations": associations}}

        def workspace_configuration(params: dict) -> list[dict]:
            return [settings if item.get("section") == "intelephense" else {} for item in params.get("items", [])]
        ...
        # files that Selene's symbol tools treat as PHP sources (#1710). This cannot be passed in
        ...
        intelephense_config: DidChangeConfigurationParams = {"settings": {"intelephense": settings}}
