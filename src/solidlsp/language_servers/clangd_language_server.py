...
class ClangdLanguageServer(SolidLanguageServer):
    ...
    You can pass the following entries in ``ls_specific_settings["cpp"]``:
        - compile_commands_dir: Directory where Selene writes its transformed
          ``compile_commands.json`` if needed.
        - clangd_version: Override the pinned Clangd version downloaded by Selene
          (default: the bundled Selene version).
    """
    ...
    def _raise_if_unreal_without_compile_db(self) -> None:
        ...
        raise SolidLSPException(
            ...
            "database, reconnect or restart Selene's MCP so the language server re-checks for it.\n"
            "See docs/03-special-guides/unreal_engine_setup_guide_for_selene.md for details."
        )
    ...
    def _prepare_compile_commands(self) -> str | None:
        ...
        compilation database to the selene managed directory.

        The transformed file is persisted in .selene/selene_compile_commands.json
        ...
        Returns the path to the selene directory containing the transformed database,
        ...
        try:
            ...
            # Get the target directory from ls_specific_settings, default to .selene
            cpp_settings = self._custom_settings or {}
            compile_commands_rel_dir = cpp_settings.get("compile_commands_dir", ".selene")
            ...
            log.info(f"Created selene compilation database with absolute paths at {compile_commands_path}")
    ...
    def _create_process_launch_info(self) -> ProcessLaunchInfo:
        """
        Override to add --compile-commands-dir argument if we created a selene compilation database.
        """
        # First, ensure the selene compile commands database is prepared
        ...
        # If we created a selene compilation database, add --compile-commands-dir to the command
