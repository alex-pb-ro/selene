...
class JuliaLanguageServer(SolidLanguageServer):
    ...
    def __init__(self, config: LanguageServerConfig, repository_root_path: str, solidlsp_settings: SolidLSPSettings):
        ...
        if platform.system() == "Windows":
            # On Windows, pass as list (Selene handles shell=True differently)
    ...
    def _setup_runtime_dependency() -> str:
        ...
        # stdin=DEVNULL: when Selene runs over the stdio MCP transport, the JSON-RPC
        # channel *is* the process stdin/stdout. Without this, the Julia child inherits
        # Selene's stdin (the MCP pipe) and clobbers it, killing the server right after
        # initialize ("tools fetch failed"). See historical issue #1577
