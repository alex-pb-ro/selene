...
# The `window/showMessageRequest` actions Selene answers affirmatively: the ones standing between
...
def choose_show_message_request_action(params: dict, auto_import_build: bool = DEFAULT_AUTO_IMPORT_BUILD) -> dict | None:
    """
    Choose Selene's answer to a `window/showMessageRequest`, which Metals uses to ask whether to
    import the build.
...
class ScalaLanguageServer(SolidLanguageServer):
    ...
    Configurable options in ls_specific_settings (in selene_config.yml):
    ...
    Multi-instance support:
        ...
        concurrent instances sharing the same database. Running Selene's Metals
