...
# AdaCore uses "darwin"/"win32" in asset names; remap from Selene's platform ids.
...
class AdaLanguageServer(SolidLanguageServer):
    ...
    You can pass the following entries in ``ls_specific_settings["ada"]``:
        - ls_path: Absolute path to a pre-installed ``ada_language_server``
          executable. If set, Selene does not download ALS and uses this
        ...
        - als_version: Override the pinned ALS version downloaded by Selene
          (default: the bundled Selene version). Setting this requires
