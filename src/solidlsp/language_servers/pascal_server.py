...
Version management:
- pasls_version: Override the pinned pasls version downloaded by Selene
...
Example configuration in ~/.selene/selene_config.yml:
...
class PascalLanguageServer(SolidLanguageServer):
    ...
    def _get_latest_version(cls) -> str | None:
        ...
        try:
            headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "Selene-LSP"}
    ...
    def _get_checksums(cls) -> dict[str, str] | None:
        ...
        try:
            req = urllib.request.Request(checksums_url, headers={"User-Agent": "Selene-LSP"})
            with urllib.request.urlopen(req, timeout=cls.NETWORK_TIMEOUT) as response:
    ...
    def _download_archive(cls, url: str, target_path: str) -> bool:
        ...
        try:
            ...
            req = urllib.request.Request(url, headers={"User-Agent": "Selene-LSP"})
    ...
    def _setup_runtime_dependencies(cls, solidlsp_settings: SolidLSPSettings) -> str:
        ...
        Downloads the pinned Selene-supported pasls release with checksum verification.
