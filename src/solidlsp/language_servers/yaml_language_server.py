...
class YamlLanguageServer(SolidLanguageServer):
    ...
    def _start_server(self) -> None:
        ...
        def workspace_configuration(params: dict) -> list[dict]:
            settings = self._create_base_initialize_params()["initializationOptions"]["yaml"]
            return [settings if item.get("section") == "yaml" else {} for item in params.get("items", [])]

        def reject_remote_schema(uri: str) -> None:
            raise ValueError("Remote YAML schema fetching is disabled in Selene")

        self.server.on_request("workspace/configuration", workspace_configuration)
        self.server.on_request("vscode/content", reject_remote_schema)
        ...
        # Route HTTP schemas back to this client, which refuses them without making a request.
        self.server.notify.send_notification("yaml/registerContentRequest", {})
