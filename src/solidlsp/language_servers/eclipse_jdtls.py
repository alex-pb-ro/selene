...
# Mapping from Selene's platform identifiers to upstream JDTLS config_<platform> directory names
...
class RuntimeDependencyPaths:
    ...
    Gradle distribution path is ``None`` because Selene does not download it.
    """
...
class EclipseJDTLS(SolidLanguageServer):
    ...
    You can configure the following options in ls_specific_settings (in selene_config.yml):
        ...
        - lombok_show_generated: Show Lombok-generated methods (getX/setX/builder()/...) in document
              ...
              key silently. See eclipse-jdtls/eclipse.jdt.ls#3706 and selene #1432.
        - gradle_version: Override the pinned Gradle distribution version downloaded by Selene
        - vscode_java_version: Override the pinned vscode-java runtime bundle version downloaded by Selene.
    ...
    class DependencyProvider(LanguageServerDependencyProvider):
        ...
        def _setup_runtime_dependencies(
            ...
            * **Upstream JDTLS mode** (activated when both ``jdtls_path`` and ``lombok_path`` are set
              ...
              Nothing is downloaded by Selene. Suitable for restricted-network/corporate environments.
            * **Default vscode-java VSIX mode** (activated otherwise): downloads the platform-specific
            ...
            if vscode_java_version == cls.INITIAL_VSCODE_JAVA_VERSION:
                ...
                raise SolidLSPException(
                    f"Resource paths inside the vscode-java {vscode_java_version} VSIX are not pinned in selene "
        ...
        if gradle_java_home is not None:
            if not os.path.exists(gradle_java_home):
                error_msg = (
                    ...
                    f"Fix: update path in ~/.selene/selene_config.yml (ls_specific_settings -> java -> gradle_java_home), "
        ...
        for entry in configured_runtimes:
            ...
            if not os.path.exists(path):
                error_msg = (
                    ...
                    f"does not exist: {path}. Fix: update the path in ~/.selene/selene_config.yml "
        ...
        if custom_maven_settings_path is not None:
            ...
            if not os.path.exists(custom_maven_settings_path):
                error_msg = (
                    ...
                    f"Fix: create the file, update path in ~/.selene/selene_config.yml (ls_specific_settings -> java -> maven_user_settings), "
        ...
        if custom_gradle_home is not None:
            ...
            if not os.path.exists(custom_gradle_home):
                error_msg = (
                    ...
                    f"Fix: create the directory, update path in ~/.selene/selene_config.yml (ls_specific_settings -> java -> gradle_user_home), "
