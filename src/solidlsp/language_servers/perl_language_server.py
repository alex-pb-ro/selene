...
# also synced into Language.PERL.get_source_fn_matcher() (a cached singleton) so Selene's symbol
...
class PerlLanguageServer(SolidLanguageServer):
    ...
    You can pass the following entries in ``ls_specific_settings["perl"]``:
        - file_filter: List of file extensions (with leading dot) that Perl::LanguageServer should
          ...
          The same extensions are added to Selene's Perl source-file matcher, so ``find_symbol``
    ...
    def _sync_source_fn_matcher(file_filter: list[str]) -> None:
        """Keep Selene's Perl source-file matcher in sync with the LS ``fileFilter``.
    ...
    def __init__(self, config: LanguageServerConfig, repository_root_path: str, solidlsp_settings: SolidLSPSettings):
        ...
        # Sync Selene's source-file matcher with the configured extensions so find_symbol and the
