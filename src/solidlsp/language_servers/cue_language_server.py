...
#   keyed by the Selene PlatformId (osx-arm64, osx-x64, linux-arm64, linux-x64, win-x64, win-arm64).
...
class CueLanguageServer(SolidLanguageServer):
    ...
    Recognised entries in ``ls_specific_settings["cue"]``:
        - ``ls_path``: Absolute path to a pre-installed ``cue`` binary. Bypasses Selene's
          auto-download mechanism; useful when the user already has ``cue`` on ``$PATH``
          (e.g. via ``brew install cue`` or ``go install``).
        - ``cue_version``: Override the pinned cue version downloaded by Selene
          (default: the bundled Selene version). Setting this to a version other than
