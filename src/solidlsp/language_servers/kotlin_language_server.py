...
class KotlinLanguageServer(SolidLanguageServer):
    ...
    class DependencyProvider(LanguageServerDependencyProviderSinglePath):
        ...
        def _claim_storage_dir(self, project_cache_dir: str) -> str:
            ...
            single Selene instance keeps reusing its index across restarts (the primary use case, which must
            not regress). If another live Selene instance already holds that directory (concurrent sessions
            on the same project, see historical issue #1966), falls back to a directory unique to this process
            ...
            try:
                ...
                log.info(
                    "Kotlin LSP storage directory %s is in use by another Selene instance; using %s for this instance",
                    project_cache_dir,
                    instance_dir,
                )
                return instance_dir
        ...
        def _create_artifact(cls, version: str, platform_id: PlatformId) -> KotlinLSPArtifact:
            ...
            262.8190.0 onward. Selene verifies the frozen initial and current default
        ...
        def _create_launch_command(self, core_path: str) -> list[str]:
            ...
            # A custom ls_path is independent of Selene's managed version. Select
