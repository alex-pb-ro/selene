...
from solidlsp.util.cache import load_cache, load_cached_values, save_cache
...
class SolidLanguageServer(ABC):
    ...
    # VCS internals, virtual environments, caches, and selene's own data.
    ...
    def _normalize_symbol_name(self, symbol: RawDocumentSymbol, relative_file_path: str) -> str:
        ...
        NOTE: The returned name must not contain '/', which separates the components of a name path
              (see :class:`selene.symbol.NamePathMatcher`). A symbol whose name contains it cannot be
    ...
    def request_signature_help(self, relative_file_path: str, line: int, column: int) -> ls_types.SignatureHelp | None:
        ...
        This means for Selene's purposes, this method is not particularly useful. The result is also fairly verbose (but well structured).
        ...
        if not cache_file.exists():
            ...
            if legacy_cache_file.exists():
                try:
                    ...
                    ] = load_cached_values(str(legacy_cache_file))
