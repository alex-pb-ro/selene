...
REF_FILE = "src/selene/tools/tools_base.py"

# file with many symbols, good for testing overview shortening
OVERVIEW_FILE = "src/selene/tools/tools_base.py"


def run_with_shrinking(agent: SeleneAgent, label: str, fn, char_limits: list[int]) -> None:
...
def run_lsp_tools(agent: SeleneAgent) -> None:
    print("\n\n### LSP BACKEND ###\n")
...
def run_backend_independent_tools(agent: SeleneAgent) -> None:
    ...
    run_with_shrinking(
        ...
        lambda lim: search_tool.apply(
            ...
            relative_path="src/selene/tools",
...
def run_jb_tools(agent: SeleneAgent) -> None:
    print("\n\n### JETBRAINS BACKEND ###\n")
...
def make_agent(backend: LanguageBackend) -> SeleneAgent:
    config = SeleneConfig.from_config_file()
    config.web_dashboard = False
    config.language_backend = backend
    return SeleneAgent(project=REPO_ROOT, selene_config=config)
