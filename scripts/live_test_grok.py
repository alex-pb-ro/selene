"""
Live smoke test for Selene's Grok Build integration.

Verifies that the Grok support (``selene setup grok``, the ``grok`` context and the native
``selene-hooks --client=grok`` protocol) works against a *real* ``grok`` CLI installation —
exercising the exact code paths that the unit tests cover with mocks
(``test/selene/config/test_client_setup.py``, ``test/selene/test_hooks.py``, ...).
...
* **Zero inference cost.** No Grok session is ever started and no prompt is sent; only
  ...
  read-only Selene tool call) are used.
* **State-preserving.** The Grok user configuration is backed up before the first mutation,
  ...
  temporary ``SELENE_HOME``. The script refuses to run if a ``selene`` MCP server is already
...
prefix ``selene-grok-live-``). The exit code is 0 iff no check FAILed.
...
"""tools excluded by the ``grok`` context (mirrors ``src/selene/resources/config/contexts/grok.yml``)"""
...
UNIT_TEST_FILES = (
    "test/selene/test_hooks.py",
    "test/selene/test_cli_setup.py",
    "test/selene/config/test_client_setup.py",
    "test/selene/config/test_context_mode.py",
    "test/selene/config/test_grok_docs.py",
)
"""unit-test files whose mocked assumptions this live test validates"""

LIVE_MCP_SERVER_NAME = "selene-live"
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [{"type": "command", "command": "selene-hooks remind --client=grok", "timeout": 5}],
            }
        ],
        "Stop": [{"hooks": [{"type": "command", "command": "selene-hooks cleanup --client=grok", "timeout": 5}]}],
    }
}
...
class McpStdioProbe:
    ...
    def __init__(self, argv: list[str], cwd: Path, stderr_log: IO[str]) -> None:
        ...
        :param stderr_log: an open file receiving the server's stderr (Selene logs to stderr)
        """
...
class LiveTestConfig:
    ...
    def hook_home(self) -> Path:
        """An isolated ``SELENE_HOME`` so hook state never touches the user's real one"""
        return self.work_dir / "selene-home"
    ...
    def from_args(cls, args: argparse.Namespace) -> "LiveTestConfig":
        ...
        selene_bin = Path(args.selene_bin) if args.selene_bin else cls._venv_executable(repo_root, "selene")
        selene_hooks_bin = Path(args.selene_hooks_bin) if args.selene_hooks_bin else cls._venv_executable(repo_root, "selene-hooks")
        python_bin = cls._venv_executable(repo_root, "python")
        for executable in (selene_bin, selene_hooks_bin, python_bin):
            if not executable.exists():
                raise AbortError(f"executable not found: {executable} — run 'uv sync' first (or pass --selene-bin/--selene-hooks-bin)")
        ...
        work_dir = Path(args.work_dir) if args.work_dir else Path(tempfile.mkdtemp(prefix="selene-grok-live-"))
        return cls(
            repo_root=repo_root,
            selene_bin=selene_bin,
            selene_hooks_bin=selene_hooks_bin,
...
class GrokLiveTest:
    ...
    def _run_hook(self, subcommand: str, payload: dict) -> CommandResult:
        argv = [str(self.config.selene_hooks_bin), subcommand, "--client", "grok"]
        return run_command(argv, timeout=30, env_overlay={"SELENE_HOME": str(self.config.hook_home)}, stdin_text=json.dumps(payload))
    ...
    def check_p1_environment(self) -> None:
        ...
        module_path = run_command([str(self.config.python_bin), "-c", "import selene; print(selene.__file__)"]).stdout.strip()
        setup_help = run_command([str(self.config.selene_bin), "setup", "--help"]).output
        remind_help = run_command([str(self.config.selene_hooks_bin), "remind", "--help"]).output
        self._environment_summary = f"branch {branch} ({commit}); {grok_version}"

        # validate that the grok CLI works and the selene under test actually has Grok support
        problems = []
        if not grok_version.lower().startswith("grok "):
            problems.append(f"grok CLI not functional: {grok_version!r}")
        expected_module = self.config.repo_root / "src" / "selene" / "__init__.py"
        if Path(module_path) != expected_module:
            problems.append(f"selene resolves to {module_path}, not the repository source ({expected_module}) — run 'uv sync'")
        if "grok" not in setup_help:
            problems.append("'selene setup' does not offer the grok client — is Grok support present on this branch?")
        if "grok" not in remind_help:
            problems.append("'selene-hooks remind --client' does not offer grok")
    ...
    def check_p3_baseline(self) -> None:
        ...
        # snapshot the MCP baseline and refuse to run against a pre-existing selene registration
        ...
        if "selene" in self._baseline_mcp_list:
            self._record("P3", Status.FAIL, "a selene MCP entry is already registered in Grok — refusing to touch it")
            raise AbortError(
                "remove it first if it is a leftover:  grok mcp remove --scope user selene  "
    ...
    def check_p2_unit_smoke(self) -> None:
        ...
        agent_result = run_command(
            [str(self.config.python_bin), "-m", "pytest", "test/selene/test_selene_agent.py", "-k", "grok", "-q"],
            timeout=900,
    ...
    def check_p4_context_loads(self) -> None:
        ...
        code = (
            ...
            "ctx = SeleneAgentContext.from_name('grok')\n"
    ...
    def check_h_hook_protocol(self) -> None:
        ...
        result = self._run_hook(
            "auto-approve", self._pre_tool_use_payload("live-H7", "selene__find_symbol", {}, permission_mode="acceptEdits")
        )
    ...
    def check_s_client_setup(self) -> None:
        self._section("S1 — selene setup grok")
        expected_add_command = "grok mcp add --scope user selene -- selene start-mcp-server --context=grok --project-from-cwd"
        result = run_command([str(self.config.selene_bin), "setup", "grok"], timeout=120, cwd=self.config.repo_root)
        ...
        section = self._read_grok_mcp_server_section("selene")
        list_result = self._grok("mcp", "list")
        self._write_evidence("S2.txt", f"mcp list:\n{list_result.output}\n\n[mcp_servers.selene] = {section!r}")
        registered_tokens = " ".join([str(section.get("command", "")), *section.get("args", [])])
        required_tokens = ("selene", "start-mcp-server", "--context=grok", "--project-from-cwd")
        if "selene" in list_result.output and all(token in registered_tokens for token in required_tokens):
            self._record("S2", Status.PASS, "config registers command 'selene' with the context and project flags")
        else:
        ...
        if "selene" in server_names:
            self._record("S3", Status.PASS, "inspect --json lists selene under mcpServers")
        else:
            self._record("S3", Status.WARN, f"selene not among inspect's mcpServers {server_names} — review evidence/S3-inspect.json")

        self._section("S4 — idempotent re-run")
        rerun = run_command([str(self.config.selene_bin), "setup", "grok"], timeout=120, cwd=self.config.repo_root)
        self._write_evidence("S4.txt", rerun.output)
        if rerun.return_code == 0 and self._read_grok_mcp_server_section("selene"):
        ...
        result = run_command([str(self.config.selene_bin), "setup", "grok"], env_overlay={"PATH": minimal_path})
    ...
    def _server_argv(self, with_project: bool, context: str = "grok") -> list[str]:
        argv = [str(self.config.selene_bin), "start-mcp-server", "--context", context, "--enable-web-dashboard", "false"]
        if with_project:
        ...
        with self._open_evidence(f"{check_id}-server.log") as stderr_log:
            ...
            try:
                init_params = {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "selene-live-test", "version": "0.0.0"},
                }
    ...
    def check_m_mcp_server(self) -> None:
        ...
        self._section("M3 — grok mcp doctor spawns the real server (absolute selene path)")
        ...
        self._section("M4 — readiness of the 'selene' command on PATH (informational)")
        self._check_m4_path_selene()
    ...
    def _check_m3_doctor(self) -> None:
        ...
        if not self._doctor_looks_healthy(doctor) and self._insert_startup_timeout_sec(60):
            ...
            if self._doctor_looks_healthy(doctor):
                self._finding(
                    "grok's default MCP startup timeout (30s) was too low for Selene's cold start in this environment; "
    ...
    def _check_m4_path_selene(self) -> None:
        """Checks whether the ``selene`` resolved via PATH (what the production registration will spawn) supports the grok context."""
        path_selene = shutil.which("selene")
        if path_selene is None:
            self._record("M4", Status.INFO, "no 'selene' on PATH — the production registration requires one (e.g. via uv tool install)")
            return
        if Path(path_selene).resolve() == self.config.selene_bin.resolve():
            self._record(
                "M4", Status.INFO, "PATH resolves 'selene' to the binary under test — production registration will behave like M1-M3"
            )
            return

        # probe the foreign installation briefly: surviving the timeout means the context loaded
        probe = run_command([path_selene, "start-mcp-server", "--context", "grok", "--enable-web-dashboard", "false"], timeout=15)
        self._write_evidence(
            "M4.txt", f"PATH selene: {path_selene}\nrc={probe.return_code} timed_out={probe.timed_out}\n{probe.output[-3000:]}"
        )
        if probe.timed_out:
            self._record(
                "M4", Status.INFO, f"'selene' on PATH ({path_selene}) supports the grok context — production registration is functional"
            )
        else:
            self._record(
                "M4", Status.INFO, f"'selene' on PATH ({path_selene}) cannot start with the grok context — likely a pre-Grok release"
            )
            self._finding(
                "the production registration spawns the bare 'selene' command; it only works once the installation on PATH "
    ...
    def _probe_tool_call(self, check_id: str, context: str) -> Optional[tuple[Optional[dict], dict]]:
        ...
        :param context: the Selene context to start the server with
    ...
    def _check_m5_structured_output(self) -> None:
        ...
        This pins what Selene *serves* under each context; whether Grok Build unpacks
    ...
    def check_d_hook_discovery(self) -> None:
        ...
        (hooks_dir / "selene-hooks.json").write_text(hooks_json_text, encoding="utf-8")
        ...
        if "selene-hooks" in inspect_result.output:
        ...
        global_hooks_file = global_hooks_dir / "selene-hooks.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Live (zero-inference) smoke test of Selene's Grok Build integration against a real grok CLI.",
        epilog="Backs up the Grok config before any change and restores the baseline afterwards; "
        "refuses to run if a 'selene' MCP entry is already registered in Grok.",
    )
    ...
    parser.add_argument("--repo-root", help="Selene repository root (default: derived from this script's location)")
    parser.add_argument("--selene-bin", help="selene executable under test (default: <repo>/.venv)")
    parser.add_argument("--selene-hooks-bin", help="selene-hooks executable under test (default: <repo>/.venv)")
