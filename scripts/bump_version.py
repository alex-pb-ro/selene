...
from selene.util.git import get_git_status
...
def bump_repo_version(
    ...
    *,
    ...
    init_path = repo_root / "src" / "selene" / "__init__.py"
    ...
    init_version = extract_version(init_text, _INIT_VERSION_PATTERN, "src/selene/__init__.py")
    log.info("pyproject.toml version: %s, __init__.py version: %s", current_version, init_version)
    if current_version != init_version:
        raise click.ClickException(
            f"Version mismatch between pyproject.toml and src/selene/__init__.py: {current_version} != {init_version}"
        )
    ...
    new_init_text = replace_version(init_text, _INIT_VERSION_PATTERN, new_version, "src/selene/__init__.py")
