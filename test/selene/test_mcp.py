...
from selene.agent import Tool, ToolRegistry
...
class MockAgent:
    def __init__(self):
        ...
        self.selene_config = None

    @staticmethod
    def get_context() -> SeleneAgentContext:
        return SeleneAgentContext.load_default()
...
def test_create_mcp_server_reports_selene_version(monkeypatch: pytest.MonkeyPatch) -> None:
    """MCP initialize must report Selene's version, not the installed mcp SDK version."""

    class MinimalAgent:
        def create_connection_prompt(self) -> str:
            return ""

    monkeypatch.setattr(SeleneConfig, "from_config_file", classmethod(lambda cls: SeleneConfig()))
    factory = SeleneMCPFactory(transport="stdio")
    monkeypatch.setattr(factory, "_create_selene_agent", lambda *args, **kwargs: MinimalAgent())

    mcp_server = factory.create_mcp_server()
    initialization_options = mcp_server._mcp_server.create_initialization_options()

    assert initialization_options.server_name == "Selene"
