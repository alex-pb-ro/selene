...
from selene.util.exception import is_headless_environment, show_fatal_exception_safe
...
class TestShowFatalExceptionSafe:
    ...
    @patch("selene.util.exception.is_headless_environment", return_value=True)
    @patch("selene.util.exception.log")
    def test_show_fatal_exception_safe_headless(self, mock_log, mock_is_headless):
        ...
        with patch("selene.gui_log_viewer.show_fatal_exception") as mock_show_gui:
    ...
    @patch("selene.util.exception.is_headless_environment", return_value=False)
    @patch("selene.util.exception.log")
    ...
    def test_show_fatal_exception_safe_gui_failure(self, mock_log, mock_is_headless):
        ...
        with patch("selene.gui_log_viewer.show_fatal_exception", side_effect=gui_error):
            show_fatal_exception_safe(test_exception)
    ...
    def test_show_fatal_exception_safe_prints_to_stderr(self):
        ...
        with patch("sys.stderr", new_callable=MagicMock) as mock_stderr:
            with patch("selene.util.exception.is_headless_environment", return_value=True):
                with patch("selene.util.exception.log"):
                    show_fatal_exception_safe(test_exception)
