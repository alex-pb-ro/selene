"""Tests for selene.util.yaml — focused on the atomic-write guarantee of ``save_yaml``.

Regression: ``~/.selene/selene_config.yml`` was corrupted in the field when a non-atomic
truncate-and-write saved a SHORTER value over a longer existing file (or two Selene processes
...
from selene.util.yaml import load_yaml, save_yaml
...
def test_shorter_write_over_longer_leaves_no_stale_tail(tmp_path):
    ...
    """
    path = str(tmp_path / "config.yml")
    long_projects = [f"C:/Users/misch/Projects/some/really/long/path/number/{i}/selene" for i in range(50)]
    save_yaml(path, {"projects": long_projects})

    save_yaml(path, {"projects": ["C:/Users/developer/Projects/selene"]})

    loaded = load_yaml(path)
    assert list(loaded["projects"]) == ["C:/Users/developer/Projects/selene"]
