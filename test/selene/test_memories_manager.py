"""
Unit tests for :class:`selene.project.MemoriesManager` helpers that do not
...
from selene.memories.memory_reference_analysis import (
...
from selene.project import MemoryManager
from selene.util import file_system


@pytest.fixture
def manager() -> MemoryManager:
    # selene_data_folder=None: the tested helpers do not access the filesystem
    return MemoryManager(selene_data_folder=None)
...
def fs_manager(tmp_path) -> MemoryManager:
    ...
    return MemoryManager(selene_data_folder=tmp_path)
...
class TestListMemoriesFollowsSymlinks:
    ...
    ``.selene/memories`` symlinks each submodule's memory folder, making them addressable as
    ...
    def test_lists_and_reads_memories_behind_directory_symlink(self, tmp_path) -> None:
        project_dir = tmp_path / "project" / ".selene"
        manager = MemoryManager(selene_data_folder=project_dir)
        _write(manager, "own_memory", "# local")

        # a sibling submodule with its own (possibly nested) memories
        submodule_memories = tmp_path / "submodule" / ".selene" / "memories"
...
class TestGetMemoryFilePathContainment:
    ...
    ``.selene/memories``. Since save/load/delete/move all route through this method, that let an
...
class TestValidateReferentialIntegrity:
    ...
    def test_fuzzy_near_miss_rejects_substring_only_matches(self, fs_manager: MemoryManager) -> None:
        ...
        ``Repository`` would be flagged as near-misses for ``selene_repository_structure``
        purely because they appear as substrings — a noise flood. The cases below are taken
        from a real run on the Selene codebase that produced six false positives.
        """
        _write(fs_manager, "selene_repository_structure", "# repo structure notes")
    ...
    def test_long_flat_name_near_miss_is_high_confidence(self, fs_manager: MemoryManager) -> None:
        ...
        Real-world case: ``selene_repository_structure.md`` mentions
    ...
    def test_read_only_status_propagated(self, tmp_path) -> None:
        manager = MemoryManager(selene_data_folder=tmp_path, read_only_memory_patterns=[r"frozen/.*"])
