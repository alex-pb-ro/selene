...
from selene.cli import TopLevelCommands
from selene.util.shell import ShellCommandResult

GROK_ADD_COMMAND = "grok mcp add --scope user selene -- selene start-mcp-server --context=grok --project-from-cwd"
...
def test_setup_grok_success(monkeypatch):
    ...
    monkeypatch.setattr("selene.config.client_setup.execute_shell_command", fake_execute_shell_command)
    ...
    assert "030_clients.md#grok" in result.output
...
def test_setup_grok_apply_failure_exits_1(monkeypatch):
    ...
    assert result.exit_code == 1
    assert "Failed to set up Selene for grok" in result.output
    assert "recommend" not in result.output.lower()
    assert "030_clients.md#grok" not in result.output
...
def test_init_suggests_grok_when_detected(monkeypatch):
    ...
    def fake_init(**kwargs) -> SimpleNamespace:
        return SimpleNamespace(config_file_path="/tmp/selene_config.yml")

    monkeypatch.setattr("selene.config.client_setup.execute_shell_command", fake_execute_shell_command)
    monkeypatch.setattr("selene.cli.SeleneConfig.init", fake_init)

    result = CliRunner().invoke(TopLevelCommands.init, [])

    assert result.exit_code == 0, result.output
    assert "selene setup grok" in result.output
    assert "selene setup claude-code" not in result.output
