...
from selene.config.selene_config import ProjectConfig, RegisteredProject, SeleneConfig
...
from test.selene.config.test_context_mode import GROK_EXCLUDED_TOOLS
...
@pytest.fixture
def selene_config():
    config = SeleneConfig(log_level=logging.ERROR, tool_timeout=600).with_headless_mode_overrides()
...
@contextmanager
def project_file_modification_context(selene_agent: SeleneAgent, relative_path: str) -> Iterator[None]:
    """Context manager to modify a project file and revert the changes after use."""
    project = selene_agent.get_active_project()
    file_path = os.path.join(project.project_root, relative_path)
...
@pytest.fixture
def selene_agent(request: pytest.FixtureRequest, selene_config) -> Iterator[SeleneAgent]:
    ...
    agent = SeleneAgent(project=project_name, selene_config=selene_config)
...
class TestSeleneAgent:
    ...
    def test_agent_instantiation(self, project: str | None):
        ...
        SeleneAgent(project=project, selene_config=selene_config)
    ...
    def test_grok_context_restricts_toolset_and_prompt(self, selene_config):
        agent = SeleneAgent(
            project="test_repo_python",
            selene_config=selene_config,
            context=SeleneAgentContext.from_name("grok"),
        )
        agent.execute_task(lambda: None)
        ...
        try:
            ...
            assert "Selene's code intelligence tools" in agent.create_system_prompt()
        finally:
            agent.on_shutdown(timeout=5)
    ...
    def _assert_symbol_info_present(
        self,
        selene_agent: SeleneAgent,
        symbol: dict,
        expected_name: str | None = None,
    ) -> None:
        if selene_agent.get_active_language_server_ids() == [LanguageServerId.KOTLIN]:
        ...
        if expected_name is not None:
            assert expected_name in symbol_info, (
                f"[{selene_agent.get_active_language_server_ids()[0]}] Expected symbol info to contain symbol name "
        ...
        if symbol["kind"] == SymbolKind.Class.name and selene_agent.get_active_language_server_ids() == [LanguageServerId.JAVA]:
            assert "A simple model class" in symbol_info, f"Java class docstring not found in symbol info: {symbol}"

    @pytest.mark.parametrize("selene_agent,case", FIND_SYMBOL_REFERENCES_CASES, indirect=["selene_agent"])
    def test_find_symbol(self, selene_agent: SeleneAgent, case: FindSymbolCase) -> None:
        agent = selene_agent
        ...
        for symbol in symbols:
            self._assert_symbol_info_present(selene_agent, symbol, case.symbol_name)

    @pytest.mark.parametrize("selene_agent,case", FIND_REFERENCE_CASES, indirect=["selene_agent"])
    def test_find_symbol_references(self, selene_agent: SeleneAgent, case: FindReferenceCase) -> None:
        # Find the symbol location first
        find_symbol_tool = selene_agent.get_tool(FindSymbolTool)
        ...
        # Now find references
        find_refs_tool = selene_agent.get_tool(FindReferencingSymbolsTool)
    ...
    @pytest.mark.parametrize("selene_agent,case", FIND_DEFINING_SYMBOL_REGEX_CASES, indirect=["selene_agent"])
    def test_find_declaration(self, selene_agent: SeleneAgent, case: RegexDefiningSymbolCase) -> None:
        tool = selene_agent.get_tool(FindDeclarationTool)
        result = tool.apply(
            regex=case.regex,
        ...
        self._assert_symbol_info_present(selene_agent, defining_symbol)

    @pytest.mark.parametrize("selene_agent,case", FIND_DEFINING_SYMBOL_REGEX_ERROR_CASES, indirect=["selene_agent"])
    ...
    @pytest.mark.parametrize("selene_agent,diagnostic_case", DIAGNOSTIC_CASES, indirect=["selene_agent"])
    def test_get_diagnostics_for_file(self, selene_agent: SeleneAgent, diagnostic_case: DiagnosticCase) -> None:
        diagnostics_tool = selene_agent.get_tool(GetDiagnosticsForFileTool)
    ...
    @pytest.mark.parametrize("selene_agent,case", FIND_IMPLEMENTATION_CASES, indirect=["selene_agent"])
    def test_find_symbol_implementations(self, selene_agent: SeleneAgent, case: FindImplementationCase) -> None:
        agent = selene_agent
        find_symbol_tool = agent.get_tool(FindSymbolTool)
        result = find_symbol_tool.apply(name_path_pattern=case.symbol_name, relative_path=case.definition_file)
        symbols = json.loads(result)
        ...
        for implementation in implementations:
            self._assert_symbol_info_present(selene_agent, implementation)

    @pytest.mark.parametrize("selene_agent,case", FIND_SYMBOL_NAME_PATH_CASES, indirect=["selene_agent"])
    def test_find_symbol_name_path(self, selene_agent: SeleneAgent, case: FindSymbolNamePathCase) -> None:
        agent = selene_agent
    ...
    @pytest.mark.parametrize("selene_agent,case", FIND_SYMBOL_NAME_PATH_NO_MATCH_CASES, indirect=["selene_agent"])
    def test_find_symbol_name_path_no_match(self, selene_agent: SeleneAgent, case: FindSymbolNoMatchCase) -> None:
        agent = selene_agent
    ...
    @pytest.mark.parametrize("selene_agent,case", FIND_SYMBOL_OVERLOADED_FUNCTION_CASES, indirect=["selene_agent"])
    def test_find_symbol_overloaded_function(self, selene_agent: SeleneAgent, case: FindSymbolOverloadedCase) -> None:
        """
    ...
    @pytest.mark.parametrize("selene_agent,case", NON_UNIQUE_SYMBOL_REFERENCE_ERROR_CASES, indirect=["selene_agent"])
    def test_non_unique_symbol_reference_error(
        ...
        replace_symbol_body_tool = selene_agent.get_tool(ReplaceSymbolBodyTool)
        with pytest.raises(ValueError, match=case.expected_error_fragment):
    ...
    @pytest.mark.parametrize(
        ...
        indirect=["selene_agent"],
    )
    def test_replace_content_regex_with_wildcard_ok(self, selene_agent: SeleneAgent):
        """
        Tests a regex-based content replacement that has a unique match
        """
        relative_path = "ws_manager.js"
        with project_file_modification_context(selene_agent, relative_path):
            replace_content_tool = selene_agent.get_tool(ReplaceContentTool)
    ...
    def test_replace_content_with_backslashes(self, selene_agent: SeleneAgent, mode: Literal["literal", "regex"]):
        """
        ...
        with project_file_modification_context(selene_agent, relative_path):
            ...
            assert result == SUCCESS_RESULT
            new_content = read_project_file(selene_agent.get_active_project(), relative_path)
    ...
    def test_replace_in_files_dry_run_then_selective_apply(self, selene_agent: SeleneAgent):
        ...
        tool = selene_agent.get_tool(ReplaceInFilesTool)
        with project_file_modification_context(selene_agent, relative_path):
            original_content = read_project_file(selene_agent.get_active_project(), relative_path)
            ...
            assert read_project_file(selene_agent.get_active_project(), relative_path) == original_content
    ...
    def test_replace_in_files_expected_count_guard_returns_listing(self, selene_agent: SeleneAgent):
    ...
    def test_replace_content_reports_new_diagnostics(self, selene_agent: SeleneAgent):
    ...
    def test_replace_symbol_body_reports_new_diagnostics(self, selene_agent: SeleneAgent):
    ...
    def test_replace_content_regex_with_wildcard_ambiguous(self, selene_agent: SeleneAgent):
        """
    ...
    @pytest.mark.parametrize("selene_agent,case", SAFE_DELETE_BLOCKED_CASES, indirect=["selene_agent"])
    def test_safe_delete_symbol_blocked_by_references(self, selene_agent: SeleneAgent, case: SafeDeleteCase):
        """
        ...
        with project_file_modification_context(selene_agent, case.relative_path):
            safe_delete_tool = selene_agent.get_tool(SafeDeleteSymbol)
    ...
    @pytest.mark.parametrize("selene_agent,case", SAFE_DELETE_SUCCEEDS_CASES, indirect=["selene_agent"])
    def test_safe_delete_symbol_succeeds_when_no_references(self, selene_agent: SeleneAgent, case: SafeDeleteCase):
        """
        ...
        with project_file_modification_context(selene_agent, case.relative_path):
            ...
            file_content = read_project_file(selene_agent.get_active_project(), case.relative_path)
...
class TestPromptProvision:
    ...
    @classmethod
    def _call_tool(cls, agent: SeleneAgent, tool_class: type[Tool], session_id: str = "global", **kwargs) -> str:
    ...
    @pytest.mark.parametrize("selene_agent", [LanguageServerId.PYTHON], indirect=True)
    def test_initial_instructions_provide_project_activation_message_once_per_session(self, selene_agent: SeleneAgent) -> None:
        """
        ...
        result1 = self._call_tool(selene_agent, InitialInstructionsTool, session_id=session1)
        self._assert_activation_message(result1, project_name, present=True)

        result2 = self._call_tool(selene_agent, InitialInstructionsTool, session_id=session2)
        self._assert_activation_message(result2, project_name, present=True)

        result3 = self._call_tool(selene_agent, InitialInstructionsTool, session_id=session1)
        self._assert_activation_message(result3, project_name, present=False)

    @pytest.mark.parametrize("selene_agent", [LanguageServerId.PYTHON], indirect=True)
    def test_dynamically_activated_mode_is_provided_once_per_session(self, selene_agent: SeleneAgent) -> None:
        """
        ...
        reg_project = selene_agent.selene_config.get_registered_project(project_name2)
        reg_project.project_config.default_modes = ["no-onboarding"]
        expected_new_mode_message = "The onboarding process is not applied."
        result2 = self._call_tool(selene_agent, ActivateProjectTool, project=project_name2, session_id=session1)
        ...
        result4 = self._call_tool(selene_agent, InitialInstructionsTool, session_id=session2)
    ...
    @pytest.mark.parametrize("selene_agent", [LanguageServerId.PYTHON], indirect=True)
    def test_activate_project_tool_always_returns_activation_message(self, selene_agent: SeleneAgent) -> None:
        project_name = "test_repo_python"
        session = "session1"

        result1 = self._call_tool(selene_agent, ActivateProjectTool, project=project_name, session_id=session)
        self._assert_activation_message(result1, project_name, present=True)

        result2 = self._call_tool(selene_agent, ActivateProjectTool, project=project_name, session_id=session)
        self._assert_activation_message(result2, project_name, present=True)

    @pytest.mark.python
    @pytest.mark.skipif(not language_server_tests_enabled(LanguageServerId.PYTHON), reason="python tests are disabled in this environment")
    def test_initial_instructions_report_project_activation_error_until_project_is_activated(self, selene_config) -> None:
        """
        ...
        agent = SeleneAgent(project=None, selene_config=selene_config, project_activation_error=activation_error)
        agent.execute_task(lambda: None)
        try:
