...
from selene.cli import ProjectCommands, TopLevelCommands, find_project_root
from selene.config.selene_config import ProjectConfig
...
class TestProjectCreate:
    ...
    def test_create_basic_with_language(self, cli_runner, temp_project_dir):
        ...
        yml_path = os.path.join(temp_project_dir, ".selene", "project.yml")
    ...
    def test_create_auto_detect_language(self, cli_runner, temp_project_dir_with_python_file):
        ...
        # Verify project.yml was created
        yml_path = os.path.join(temp_project_dir_with_python_file, ".selene", "project.yml")
    ...
    def test_create_with_index_flag(self, cli_runner, temp_project_dir_with_python_file):
        ...
        cache_dir = os.path.join(temp_project_dir_with_python_file, ".selene", "cache")
    ...
    def test_create_without_index_flag(self, cli_runner, temp_project_dir):
        ...
        cache_dir = os.path.join(temp_project_dir, ".selene", "cache")
...
class TestProjectIndex:
    ...
    def test_index_is_equivalent_to_create_with_index(self, cli_runner, temp_project_dir_with_python_file):
        ...
        try:
            ...
            assert os.path.exists(os.path.join(dir1, ".selene", "project.yml"))
            assert os.path.exists(os.path.join(dir2, ".selene", "project.yml"))

            # Both should create cache (proof of indexing)
            assert os.path.exists(os.path.join(dir1, ".selene", "cache"))
            assert os.path.exists(os.path.join(dir2, ".selene", "cache"))
        finally:
...
class TestFindProjectRoot:
    ...
    def test_finds_selene_from_subdirectory(self, temp_project_dir):
        """Test that .selene/project.yml is found when searching from a subdirectory."""
        selene_dir = os.path.join(temp_project_dir, ".selene")
        os.makedirs(selene_dir)
        Path(os.path.join(selene_dir, "project.yml")).touch()
    ...
    def test_selene_preferred_over_git(self, temp_project_dir):
        """Test that .selene/project.yml takes priority over .git at the same level."""
        selene_dir = os.path.join(temp_project_dir, ".selene")
        os.makedirs(selene_dir)
        Path(os.path.join(selene_dir, "project.yml")).touch()
        ...
        try:
            ...
            result = find_project_root(root=temp_project_dir)
            assert result is not None
            assert os.path.isdir(os.path.join(result, ".selene"))
            assert os.path.samefile(result, temp_project_dir)
        finally:
            os.chdir(original_cwd)

    def test_git_used_as_fallback(self, temp_project_dir):
        """Test that .git is found when no .selene exists."""
        os.makedirs(os.path.join(temp_project_dir, ".git"))
    ...
    def test_git_worktree_not_hijacked_by_ancestor_selene(self, temp_project_dir):
        """A git worktree nested under a Selene project must resolve to the worktree.

        Regression test: when a git worktree (whose .git is a pointer *file*) lives
        below a directory that is an explicit Selene project (.selene/project.yml),
        the worktree's own .git boundary must win over the ancestor's project marker.
        The old two-pass search returned the ancestor Selene project, causing reads
        and edits to land in the wrong working tree.
        """
        # Ancestor directory is an explicit Selene project.
        selene_dir = os.path.join(temp_project_dir, ".selene")
        os.makedirs(selene_dir)
        Path(os.path.join(selene_dir, "project.yml")).touch()
...
if __name__ == "__main__":
    ...
    # uv run pytest test/selene/test_cli_project_commands.py -v
