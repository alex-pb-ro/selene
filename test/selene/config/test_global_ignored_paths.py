...
def _create_test_project(
    ...
    selene_config = SeleneConfig(ignored_paths=global_ignored_paths).with_headless_mode_overrides()
    return Project(
...
class TestGlobalIgnoredPaths:
    ...
    def test_global_ignored_paths_are_applied(self) -> None:
        """Global ignored_paths from SeleneConfig are respected by Project.is_ignored_path()."""
...
class TestRegisteredProjectGlobalIgnoredPaths:
    ...
    def test_get_project_instance_passes_global_ignored_paths(self) -> None:
        ...
        selene_config = SeleneConfig(ignored_paths=["node_modules"]).with_headless_mode_overrides()
        registered = RegisteredProject(
            project_root=str(self.project_path),
            project_config=config,
        )
        project = registered.get_project_instance(selene_config=selene_config)
    ...
    def test_get_project_instance_without_global_ignored_paths(self) -> None:
        ...
        selene_config = SeleneConfig(ignored_paths=[]).with_headless_mode_overrides()
        project = registered.get_project_instance(selene_config=selene_config)
    ...
    def test_from_project_root_passes_global_ignored_paths(self) -> None:
        ...
        selene_dir = self.project_path / ".selene"
        selene_dir.mkdir(exist_ok=True)
        (selene_dir / "project.yml").write_text(
            'project_name: "test_project"\nlanguages: ["python"]\nignored_paths: []\nignore_all_files_in_gitignore: false\n'
        )
        selene_config = SeleneConfig(ignored_paths=["node_modules"]).with_headless_mode_overrides()
    ...
    def test_from_project_instance_passes_global_ignored_paths(self) -> None:
        ...
        retrieved = registered.get_project_instance(selene_config=selene_config)
...
class TestSeleneConfigIgnoredPaths:
    """Config loading with ignored_paths in selene_config.yml works correctly."""

    def test_selene_config_default_ignored_paths(self) -> None:
        """SeleneConfig defaults to empty ignored_paths."""
        config = SeleneConfig().with_headless_mode_overrides()
        assert config.ignored_paths == []

    def test_selene_config_with_ignored_paths(self) -> None:
        """SeleneConfig can be created with explicit ignored_paths."""
        config = SeleneConfig(
