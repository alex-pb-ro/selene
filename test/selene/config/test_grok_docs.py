...
from selene.config.client_setup import ClientSetupHandlerGrok
from selene.hooks import HookClient, PreToolUseRemindAboutSymbolicToolsHook
...
def test_grok_docs_are_consistent():
    ...
    assert "030_clients.md#grok" in handler_source
    assert "selene setup grok" in grok_section
    assert "selene-hooks remind --client=grok" in grok_section
    assert "selene-hooks cleanup --client=grok" in grok_section
...
def test_grok_hook_matcher_docs_match_code(tmp_path: Path):
    ...
    for tool_name in matcher_tools:
        with patch("sys.stdin", StringIO(json.dumps(payloads[tool_name]))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
