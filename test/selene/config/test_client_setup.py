...
from selene.config.client_setup import ClientSetupHandlerGrok, client_setup_handlers
...
def test_grok_setup_handler_apply_uses_grok_mcp_add(monkeypatch):
    ...
    assert commands == ["grok mcp add --scope user selene -- selene start-mcp-server --context=grok --project-from-cwd"]
...
def test_client_setup_handlers_use_resolvable_contexts():
    ...
    for handler in client_setup_handlers:
        ...
        assert SeleneAgentContext.from_name(context_name).name == context_name
