...
BUILTIN_RUNTIME_CONTEXT_NAMES = [
    name for name in SeleneAgentContext.list_registered_context_names(include_user_contexts=False) if name != "context.template"
]


def _render_context_prompt(context: SeleneAgentContext) -> str:
    """Render with the same template variables SeleneAgent._format_prompt provides.
...
def test_grok_context_prompt_renders():
    ...
    assert "Selene's code intelligence tools" in rendered_prompt
...
def test_builtin_contexts_load_and_prompt_templates_render(context_name: str):
    context = SeleneAgentContext.from_name(context_name)
