...
from selene.config.selene_config import (
    DEFAULT_PROJECT_SELENE_FOLDER_LOCATION,
    LanguageBackend,
    ProjectConfig,
    RegisteredProject,
    SeleneConfig,
...
from selene.constants import PROJECT_TEMPLATE_FILE, SELENE_MANAGED_DIR_NAME
from selene.project import MemoryManager, Project
from solidlsp.ls_config import LanguageServerId
from test.conftest import create_default_selene_config
...
class TestProjectConfigAutogenerate:
    ...
    def setup_method(self):
        ...
        self.selene_config = create_default_selene_config()
    ...
    def test_autogenerate_empty_directory(self):
        ...
        config = ProjectConfig.autogenerate(self.project_path, self.selene_config, save_to_disk=False)
    ...
    def test_autogenerate_empty_directory_logs_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            ProjectConfig.autogenerate(self.project_path, self.selene_config, save_to_disk=False)
    ...
    def test_autogenerate_with_python_files_and_custom_ls_priorities(self):
        ...
        selene_config = deepcopy(self.selene_config)
        selene_config.ls_priorities = {LanguageServerId.PYTHON_TY.value: 3}

        # Run autogenerate
        config = ProjectConfig.autogenerate(self.project_path, selene_config, save_to_disk=False)
    ...
    def test_autogenerate_saves_to_disk(self):
        ...
        config = ProjectConfig.autogenerate(self.project_path, self.selene_config, save_to_disk=True)

        # Verify the configuration file was created
        config_path = self.project_path / ".selene" / "project.yml"
    ...
    def test_autogenerate_nonexistent_path(self):
        ...
        with pytest.raises(FileNotFoundError) as exc_info:
            ProjectConfig.autogenerate(non_existent, self.selene_config, save_to_disk=False)
    ...
    def test_autogenerate_custom_project_name(self):
        ...
        config = ProjectConfig.autogenerate(self.project_path, self.selene_config, project_name=custom_name, save_to_disk=False)
...
) -> tuple[SeleneConfig, str]:
    """Create a SeleneConfig with a single registered project and return (config, project_name)."""
    config = SeleneConfig(
...
class TestEffectiveLanguageBackend:
    """Tests for per-project language_backend override logic in SeleneAgent."""
    ...
    def test_default_backend_is_global(self):
        ...
        agent = SeleneAgent(project=name, selene_config=config)
        try:
    ...
    def test_no_project_uses_global_backend(self):
        ...
        agent = SeleneAgent(project=None, selene_config=config)
        try:
...
class TestGetConfiguredProjectSeleneFolder:
    """Tests for SeleneConfig.get_configured_project_selene_folder (pure template resolution)."""

    def test_default_location(self):
        config = SeleneConfig().with_headless_mode_overrides()
        result = config.get_configured_project_selene_folder("/home/user/myproject")
        assert result == os.path.abspath("/home/user/myproject/.selene")
    ...
    def test_custom_location_with_project_folder_name(self):
        ...
        result = config.get_configured_project_selene_folder("/home/user/myproject")
        assert result == os.path.abspath("/projects-metadata/myproject/.selene")

    def test_custom_location_with_project_dir(self):
        config = SeleneConfig(
            project_selene_folder_location="$projectDir/.custom-selene",
        ).with_headless_mode_overrides()
        result = config.get_configured_project_selene_folder("/home/user/myproject")
        assert result == os.path.abspath("/home/user/myproject/.custom-selene")

    def test_custom_location_with_both_placeholders(self):
        config = SeleneConfig(
            project_selene_folder_location="/data/$projectFolderName/$projectDir/.selene",
        ).with_headless_mode_overrides()
        result = config.get_configured_project_selene_folder("/home/user/proj")
        assert result == os.path.abspath("/data/proj/home/user/proj/.selene")

    def test_default_field_value(self):
        config = SeleneConfig().with_headless_mode_overrides()
        assert config.project_selene_folder_location == DEFAULT_PROJECT_SELENE_FOLDER_LOCATION

    def test_rejects_unknown_placeholder(self):
        config = SeleneConfig(
            project_selene_folder_location="$projectDir/$unknownVar/.selene",
        ).with_headless_mode_overrides()
        with pytest.raises(SeleneConfigError, match=r"Unknown placeholder '\$unknownVar'"):
            config.get_configured_project_selene_folder("/home/user/myproject")

    def test_rejects_typo_projectDirs(self):
        """$projectDirs should not be silently treated as $projectDir + 's'."""
        config = SeleneConfig(
            project_selene_folder_location="$projectDirs/.selene",
        ).with_headless_mode_overrides()
        with pytest.raises(SeleneConfigError, match=r"Unknown placeholder '\$projectDirs'"):
            config.get_configured_project_selene_folder("/home/user/myproject")

    def test_rejects_typo_projectfoldername_lowercase(self):
        config = SeleneConfig(
            project_selene_folder_location="/data/$projectfoldername/.selene",
        ).with_headless_mode_overrides()
        with pytest.raises(SeleneConfigError, match=r"Unknown placeholder '\$projectfoldername'"):
            config.get_configured_project_selene_folder("/home/user/myproject")

    def test_no_placeholders_is_valid(self):
        config = SeleneConfig(
            project_selene_folder_location="/fixed/path/.selene",
        ).with_headless_mode_overrides()
        result = config.get_configured_project_selene_folder("/home/user/myproject")
        assert result == os.path.abspath("/fixed/path/.selene")

    def test_error_message_lists_supported_placeholders(self):
        config = SeleneConfig(
            project_selene_folder_location="$bogus/.selene",
        ).with_headless_mode_overrides()
        with pytest.raises(SeleneConfigError, match=r"\$projectDir.*\$projectFolderName|\$projectFolderName.*\$projectDir"):
            config.get_configured_project_selene_folder("/home/user/myproject")


class TestProjectSeleneDataFolder:
    """Tests for SeleneConfig.get_project_selene_folder fallback logic (via Project)."""
    ...
    def _make_project(self, selene_config: "SeleneConfig | None" = None) -> Project:
        project_config = ProjectConfig(
    ...
    def test_default_config_creates_in_project_dir(self):
        ...
        expected = os.path.abspath(str(self.project_path / SELENE_MANAGED_DIR_NAME))
        assert project.path_to_selene_data_folder() == expected

    def test_custom_location_creates_outside_project(self):
        custom_base = Path(self.test_dir) / "metadata"
        custom_base.mkdir()
        config = SeleneConfig(
            project_selene_folder_location=str(custom_base) + "/$projectFolderName/.selene",
        ).with_headless_mode_overrides()
        project = self._make_project(config)
        expected = os.path.abspath(str(custom_base / "myproject" / ".selene"))
        assert project.path_to_selene_data_folder() == expected

    def test_fallback_to_existing_project_dir(self):
        """If config points to a non-existent path but .selene exists in the project root, use the existing one."""
        existing_selene = self.project_path / SELENE_MANAGED_DIR_NAME
        existing_selene.mkdir()
        config = SeleneConfig(
            project_selene_folder_location="/nonexistent/path/$projectFolderName/.selene",
        ).with_headless_mode_overrides()
        project = self._make_project(config)
        assert project.path_to_selene_data_folder() == str(existing_selene)
    ...
    def test_configured_path_takes_precedence_when_exists(self):
        ...
        custom_base = Path(self.test_dir) / "metadata"
        custom_selene = custom_base / "myproject" / ".selene"
        custom_selene.mkdir(parents=True)

        config = SeleneConfig(
            project_selene_folder_location=str(custom_base) + "/$projectFolderName/.selene",
        ).with_headless_mode_overrides()
        project = self._make_project(config)
        assert project.path_to_selene_data_folder() == str(custom_selene)
...
class TestProjectConfigYamlValidation:
    def test_ignored_paths_globs_starting_with_star_require_quotes(self):
        ...
        try:
            selene_dir = project_dir / SELENE_MANAGED_DIR_NAME
            selene_dir.mkdir(parents=True)
            (selene_dir / "project.yml").write_text('project_name: "demo"\nlanguages: ["csharp"]\nignored_paths:\n- **/bin/**\n')

            with pytest.raises(ValueError) as exc_info:
                ProjectConfig.load(project_dir, create_default_selene_config())
...
class TestSeleneConfigLoadSave:
    """
    Supports tests of loading/saving SeleneConfig,
    ...
    def setup(self, monkeypatch):
        """
        Sets up a temporary directory for testing, and monkeypatch SeleneConfig to use a test config file path.
        """
        self.test_dir = Path(tempfile.mkdtemp())
        self.master_config_path = self.test_dir / "selene_config.yml"
        monkeypatch.setattr(
            SeleneConfig,
    ...
    def _make_project_dir(self, name: str, project_yml_body: str) -> Path:
        ...
        (project_dir / SELENE_MANAGED_DIR_NAME).mkdir(parents=True)
        (project_dir / SELENE_MANAGED_DIR_NAME / "project.yml").write_text(project_yml_body)
        return project_dir
    ...
    def test_empty_projects_key_is_treated_as_empty_list(self):
        ...
        config = SeleneConfig.from_config_file(generate_if_missing=False)
    ...
    def test_parallel_agents_changing_project_lists(self):
        """
        Tests that agents changing SeleneConfig by adding projects in parallel do not lose each other's changes.
        """
        ...
        config1 = SeleneConfig.from_config_file(generate_if_missing=False)
        config2 = SeleneConfig.from_config_file(generate_if_missing=False)

        config1.add_project_from_path(p3)
        config2.add_project_from_path(p4)

        resulting_config = SeleneConfig.from_config_file(generate_if_missing=False)
    ...
    def test_remove_project_persists_across_reload(self):
        ...
        reloaded = SeleneConfig.from_config_file(generate_if_missing=False)
    ...
    def test_remove_project_preserves_concurrent_addition(self):
        ...
        removing_config = SeleneConfig.from_config_file(generate_if_missing=False)
        adding_config = SeleneConfig.from_config_file(generate_if_missing=False)
...
class TestGetRegisteredProjectWithDanglingProject:
    ...
    def _make_project_dir(self, name: str) -> Path:
        project_dir = self.test_dir / name
        (project_dir / SELENE_MANAGED_DIR_NAME).mkdir(parents=True)
        (project_dir / SELENE_MANAGED_DIR_NAME / "project.yml").write_text(f'project_name: "{name}"\nlanguages: ["python"]\n')
        return project_dir

    def test_dangling_project_does_not_break_lookup_of_valid_project(self):
        config = create_default_selene_config()
        ...
        config.projects = [
            RegisteredProject.from_project_root(dangling_dir, selene_config=config),
            RegisteredProject.from_project_root(valid_dir, selene_config=config),
        ]

        # Delete the dangling project's directory out from under Selene.
...
class TestMemoriesManagerCustomPath:
    """Tests for MemoriesManager with a custom selene data folder."""

    def setup_method(self):
        self.test_dir = tempfile.mkdtemp()
        self.data_folder = Path(self.test_dir) / "custom_selene"
