...
from selene.hooks import (
    HookClient,
    PostToolUseResetSymbolicToolCounterHook,
    PreToolUseAutoApproveSeleneHook,
    PreToolUseHook,
...
def _execute_remind_hook(client: HookClient, payload: dict, tmp_path: Path) -> None:
    with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
...
class TestHookClientDetection:
    ...
    def test_claude_code_client(self, tmp_path: Path):
        ...
        with patch("sys.stdin", _make_stdin(stdin_data)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
...
class TestPreToolUseRemindAboutSeleneHook:
    ...
    def test_grep_tool_detection_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [("grep", True), ("grep_search", False), ("mcp_grep", False), ("read", False)]:
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE)
    ...
    def test_grep_tool_detection_non_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [("grep_search", True), ("mcp_grep", True), ("read_file", False), ("selene_find", False)]:
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.VSCODE)
    ...
    def test_grep_tool_detection_codex_shell_commands(self, tmp_path: Path):
        ...
        for tool_name, tool_input, expected in cases:
            with (
                ...
                patch("selene.hooks.selene_home_dir", str(tmp_path)),
            ):
    ...
    def test_grok_camel_case_payload_detection(self, tmp_path: Path):
        ...
        with patch("sys.stdin", _make_stdin(grep_payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
        ...
        with patch("sys.stdin", _make_stdin(read_payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_read_file_tool_detection_claude_code(self, tmp_path: Path):
        ...
        for name, expected in [("read", True), ("mcp__selene__read_file", True), ("grep", False), ("selene_search_for_pattern", False)]:
            with (
    ...
    def test_selene_tool_detection(self, tmp_path: Path):
        for name, expected in [("mcp_selene_find_symbol", True), ("selene_overview", True), ("grep_search", False)]:
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE)
            assert hook.is_selene_symbolic_tool() == expected, f"is_selene_tool() wrong for {name}"

    def test_selene_tool_detection_grok_namespace(self, tmp_path: Path):
        payload = _grok_input("selene__find_symbol")
        with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK)
        assert hook.is_selene_symbolic_tool() is True
    ...
    def test_no_output_below_threshold(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        ...
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(_base_input("grep"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_deny_output_after_threshold_greps_vscode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        ...
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(_base_input("grep_search"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_grok_selene_tool_resets_counters(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Grok MCP tool names use ``selene__`` and must reset non-symbolic counters."""
        session_id = "grok-reset"
        grep_payload = _grok_input("grep", {"pattern": "foo", "path": "."}, session_id=session_id)
        selene_payload = _grok_input("selene__find_symbol", {"name_path": "Foo"}, session_id=session_id)

        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(grep_payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK).execute()

        with patch("sys.stdin", _make_stdin(selene_payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK).execute()
    ...
    def test_deny_output_after_threshold_reads(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        ...
        """
        for _ in range(ToolUseCounter._READ_FILE_USES_THRESHOLD):
            with patch("sys.stdin", _make_stdin(_read_input("read"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_selene_tool_resets_counters(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Using a Selene tool should reset counters, so the threshold is not reached."""
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(_base_input("grep"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()

        with patch("sys.stdin", _make_stdin(_base_input("mcp_selene_find_symbol"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_non_symbolic_deny_emitted_when_combined_threshold_tripped(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        ...
        # invoke execute with a neutral (non-grep, non-read, non-selene) tool so that
        # update() leaves the counters untouched and the fall-through branch is taken
        with patch("sys.stdin", _make_stdin(_base_input("Edit"))), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()
...
class TestToolUseCounter:
    ...
    def test_selene_tool_resets_all_counters(self):
        counter = ToolUseCounter(
        ...
        hook = self._make_hook_stub("mcp_selene_overview", datetime.now())
        counter.update(hook)
...
class TestPreToolUseAutoApproveSeleneHook:
    """Tests for the auto-approve hook that allows Selene tools while the client is in a permissive permission mode (``acceptEdits`` or ``auto``)."""

    @staticmethod
    def _approve_input(
        tool_name: str = "mcp__selene__find_symbol",
    ...
    def test_approves_selene_tool_in_accept_edits_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """When the tool is a Selene tool and the mode is ``acceptEdits``, an allow decision is emitted."""
        stdin_data = self._approve_input()
        with patch("sys.stdin", _make_stdin(stdin_data)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PreToolUseAutoApproveSeleneHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_approves_selene_tool_in_auto_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """When the tool is a Selene tool and the mode is ``auto``, an allow decision is emitted.

        Claude Code's ``auto`` mode is the hands-off-execution mode users adopt for autonomous
        runs; Selene tool calls should be auto-approved there for the same reason as in
        ``acceptEdits`` (Selene's destructive tools would otherwise still prompt per call).
        """
    ...
    def test_stays_silent_for_non_selene_tool(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Non-Selene tools get no decision even in ``acceptEdits`` mode (the hook stays silent)."""
    ...
    def test_stays_silent_in_default_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Selene tools in ``default`` mode get no decision (the hook stays silent)."""
...
class TestPostToolUseResetSymbolicToolCounterHook:
    ...
    successful Selene symbolic tool call, for clients (Codex) whose PreToolUse wiring
    never observes Selene's own tools.
    """
    ...
    def test_missing_session_id_raises(self, tmp_path: Path):
        stdin_data = {"tool_name": "mcp__selene__find_symbol", "tool_response": {"isError": False}}
        with patch("sys.stdin", _make_stdin(stdin_data)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            with pytest.raises(ValueError, match="Session ID is required"):
                PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX)
    ...
    def test_resets_persisted_counter_on_successful_selene_call(self, tmp_path: Path):
        ...
        payload = _post_tool_use_input("mcp__selene__find_symbol", session_id=session_id, tool_response={"isError": False})
        with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_does_not_reset_on_failed_selene_call(self, tmp_path: Path):
        """A Selene call that itself errored must not mask a real grep/read streak."""
        session_id = "reset-failure"
        seeded = ToolUseCounter(n_recent_grep_uses=2)
        self._seed_counter(tmp_path, session_id, seeded)

        payload = _post_tool_use_input("mcp__selene__find_symbol", session_id=session_id, tool_response={"isError": True})
        with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
    ...
    def test_does_not_reset_without_a_tool_response(self, tmp_path: Path):
        ...
        payload = {"session_id": session_id, "tool_name": "mcp__selene__find_symbol"}
        with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()

        assert self._persisted_counter(tmp_path, session_id).n_recent_grep_uses == 2

    def test_does_not_reset_for_non_selene_tool(self, tmp_path: Path):
        session_id = "reset-non-selene"
        seeded = ToolUseCounter(n_recent_grep_uses=2)
        self._seed_counter(tmp_path, session_id, seeded)
    ...
    def test_does_not_reset_for_non_symbolic_selene_tool(self, tmp_path: Path):
        """``read_file``-like Selene tools are excluded, same as the PreToolUse classification."""
        session_id = "reset-non-symbolic"
        seeded = ToolUseCounter(n_recent_grep_uses=2)
        self._seed_counter(tmp_path, session_id, seeded)

        payload = _post_tool_use_input("mcp__selene__read_file", session_id=session_id, tool_response={"isError": False})
        with patch("sys.stdin", _make_stdin(payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
    ...
    def test_creates_fresh_counter_when_none_persisted_yet(self, tmp_path: Path):
        """A successful Selene call as the very first hook invocation of a session must not raise."""
    ...
    def test_codex_acceptance_scenario_successful_selene_call_prevents_deny(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """The issue's own acceptance test: Bash(rg) -> Bash(rg) -> successful Selene call -> Bash(rg)
        must not deny the final call, using exactly the documented Codex hook wiring
        (``remind`` on PreToolUse/Bash, ``reset`` on PostToolUse/``mcp__selene__.*``).
        """
        ...
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(grep_shell_payload)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.CODEX).execute()
        assert capsys.readouterr().out == ""

        selene_call = _post_tool_use_input("mcp__selene__find_symbol", session_id=session_id, tool_response={"isError": False})
        with patch("sys.stdin", _make_stdin(selene_call)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
    ...
    def test_codex_acceptance_scenario_failed_selene_call_still_denies(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """The issue's second acceptance test: a failed Selene call must not reset the streak,
        ...
        failed_selene_call = _post_tool_use_input("mcp__selene__find_symbol", session_id=session_id, tool_response={"isError": True})
        with patch("sys.stdin", _make_stdin(failed_selene_call)), patch("selene.hooks.selene_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
...
class TestHookCli:
    """Tests for the Click CLI entry point (selene-hooks)."""
    ...
    def test_cleanup_command(self, tmp_path: Path):
        ...
        runner = CliRunner()
        with patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_reset_command(self, tmp_path: Path):
        """The ``reset`` CLI command clears a persisted counter after a successful Selene call."""
        ...
        stdin_json = json.dumps({"session_id": session_id, "tool_name": "mcp__selene__find_symbol", "tool_response": {"isError": False}})
        runner = CliRunner()
        with patch("selene.hooks.selene_home_dir", str(tmp_path)):
    ...
    def test_reset_command_stays_silent_on_failed_call(self, tmp_path: Path):
        """The ``reset`` CLI command must not clear the counter for a failed Selene call."""
        ...
        stdin_json = json.dumps({"session_id": session_id, "tool_name": "mcp__selene__find_symbol", "tool_response": {"isError": True}})
        runner = CliRunner()
        with patch("selene.hooks.selene_home_dir", str(tmp_path)):
            result = runner.invoke(hook_commands, ["reset", "--client", "codex"], input=stdin_json)
        assert result.exit_code == 0
    ...
    def test_auto_approve_command(self, tmp_path: Path):
        """The ``auto-approve`` CLI command emits an allow for a Selene tool in acceptEdits mode."""
        stdin_json = json.dumps(
            {
                "session_id": "cli-auto-approve",
                "tool_name": "mcp__selene__find_symbol",
                "tool_input": {},
    ...
    def test_auto_approve_command_in_auto_mode(self, tmp_path: Path):
        """The ``auto-approve`` CLI command emits an allow for a Selene tool in ``auto`` mode."""
        stdin_json = json.dumps(
            {
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("selene__find_symbol", "auto", {"decision": "allow"}),
            ("selene__find_symbol", "bypassPermissions", None),
            ("selene__find_symbol", "dontAsk", None),
            ("selene__find_symbol", "", None),
            ("grep", "auto", None),
        ],
        ids=["auto-allows-selene", "bypass-silent", "dont-ask-silent", "empty-mode-silent", "non-selene-silent"],
    )
    def test_auto_approve_command_grok_permission_modes(
        ...
        """Grok auto-approve is intentionally active only for Selene tools in ``auto`` mode."""
        stdin_json = json.dumps(
            {
