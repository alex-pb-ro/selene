...
from selene.tools import FindReferencingSymbolsTool, FindSymbolTool
...
class FileSystemSyncTestCase:
    ...
    It supports two modes:
      * using Selene's tools (where an explicit sync is not needed because it builds on find_symbol)
    ...
    def _referencing_symbol_names(self, agent: SeleneAgent) -> list[str]:
        """Names of the symbols that reference ``_TARGET_SYMBOL``, per the warm language server."""
        if self._use_selene_tool:
    ...
    def _sync_fs(self, project: Project, num_expected_events: int, event_type: str):
        # apply sync only if Selene's tool is not being used
        if not self._use_selene_tool:
...
def test_find_referencing_symbol_tool_handles_sync_with_relative_path(tmp_path):
    ...
    """
    FileSystemSyncTestCase(use_selene_tool=True, tool_use_relative_path=True).run(tmp_path)
...
def test_find_referencing_symbol_tool_handles_sync_without_relative_path(tmp_path):
    ...
    """
    FileSystemSyncTestCase(use_selene_tool=True, tool_use_relative_path=False).run(tmp_path)
