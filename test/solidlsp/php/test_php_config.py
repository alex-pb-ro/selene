...
Selene treats only ``.php`` / ``.phtml`` files as PHP sources by default, while Drupal projects
...
class TestFileFilterIntegration:
    ...
    def test_module_file_symbols_and_references_visible(self) -> None:
        with start_ls_context(
            ...
            # Layer 1 (Selene's source matcher): the .module file takes part in symbol traversal.
