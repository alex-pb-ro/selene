...
Reproduces historical issue #1797: Erlang LS names functions, types and
parameterised macros ``name/arity`` (e.g. ``create_user/4``), but ``/`` separates the components of
a Selene name path. Such a name was therefore parsed as "symbol ``4`` nested inside ``create_user``"
and could never be matched -- not even by the very name path Selene itself reported for the symbol.
...
from selene.symbol import LanguageServerSymbolRetriever
...
# `models:create_user/4`, spelled the way Selene addresses it
