...
def test_darwin_uses_isolated_state_dir_and_preserves_node_options(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert launch_env["SELENE_SOLIDITY_STATE_DIR"] == str(state_dir)
...
def test_darwin_defaults_state_dir_under_language_server_resources(
    ...
    assert launch_env["SELENE_SOLIDITY_STATE_DIR"] == str(Path(provider._ls_resources_dir) / "solidity-state")
    assert Path(launch_env["SELENE_SOLIDITY_STATE_DIR"]).is_dir()
...
def test_non_darwin_does_not_add_state_isolation(
    ...
    assert "SELENE_SOLIDITY_STATE_DIR" not in launch_env
