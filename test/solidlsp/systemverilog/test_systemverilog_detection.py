...
- Detection failing means Selene is unusable for SystemVerilog, even when verible is correctly installed
...
class TestVeribleVerilogLsDetection:
    ...
    def test_downloads_when_not_in_path(self):
        ...
        Selene should auto-download it. This enables zero-setup experience.
        """
        with tempfile.TemporaryDirectory() as temp_dir:
            custom_settings = SolidLSPSettings.CustomLSSettings({})
            provider = SystemVerilogLanguageServer.DependencyProvider(custom_settings, temp_dir)
