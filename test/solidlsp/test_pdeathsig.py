"""Regression test for issue #1490: a SIGKILLed Selene leaves its language server process running.
...
(``subprocess_util.terminate_process_tree_with_kill_fallback``). That code never runs if Selene
...
This drives a real ``StdioLanguageServer`` in a child ("fake Selene") process, SIGKILLs that
...
def test_language_server_process_survives_a_short_lived_calling_thread() -> None:
    ...
    delivers the death signal right then, even though "Selene" (the process) never died. Starting
    ...
    try:
        ...
        assert survivors, (
            "language server process died on its own even though the driver ('Selene') process "
...
def test_language_server_process_dies_with_a_sigkilled_selene() -> None:
    ...
    try:
        ...
        driver.kill()  # SIGKILL: simulates Selene being killed without a chance to clean up
        ...
        assert not survivors, (
            f"language server process(es) {[p.pid for p in survivors]} survived a SIGKILLed Selene "
