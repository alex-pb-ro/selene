# Docker Setup for Selene (Experimental)

⚠️ **EXPERIMENTAL FEATURE**: The Docker setup for Selene is still experimental and has some limitations. Please read this entire document before using Docker with Selene.

## Overview

Docker support allows you to run Selene in an isolated container environment, which provides better security isolation for the shell tool and consistent dependencies across different systems.
...
Selene's configuration and log files are stored in the container in `/workspaces/selene/config/`.
Any local configuration you may have for Selene will not apply; the container uses its own separate configuration.
...
Simply mount your local directory to `/workspaces/selene/config` in the container.
Initially, be sure to add a `selene_config.yml` file to the mounted directory which applies the following
...
```bash
# Use default ports
docker-compose up selene

# Use custom ports
SELENE_DASHBOARD_PORT=8080 docker-compose up selene
```
...
1. **Production mode** (for using Selene as MCP server):
   ```bash
   docker-compose up selene
   ```
...
services:
  selene:
    ...
    command:
      - "uv run --directory . selene-mcp-server --transport sse --port 9121 --host 0.0.0.0 --context claude-code"
```
...
- Custom port: http://localhost:${SELENE_DASHBOARD_PORT}/dashboard
...
- `SELENE_DOCKER=1`: Set automatically to indicate Docker environment
- `SELENE_PORT`: MCP server port (default: 9121)
- `SELENE_DASHBOARD_PORT`: Web dashboard port (default: 24282)
...
rm selene_config.docker.yml

# Selene will auto-generate a new one on next run
```
