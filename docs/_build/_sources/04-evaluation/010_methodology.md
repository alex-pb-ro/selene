...
In this section we describe the methodology we applied in evaluating the performance of Selene's tools.

The evaluation measures the **concrete delta** that Selene's tools provide on top of an agent's
...
1. **Generality.** The evaluation mechanism should be broadly applicable and repeatable by any user, on any project, with any agent.
   ...
   of choice, in your client of choice. This means the results directly reflect the value Selene
...
3. **Comprehensive and unbiased by design.**
   ...
   span Selene's capabilities: codebase understanding, single-file edits of varying sizes, multi-file
   ...
   explicitly requires reporting negative deltas and cases where Selene offers no improvement,
...
For every task, the agent executes the full end-to-end workflow using **both** toolsets (Selene's semantic
...
* **(a)** tasks where Selene adds capability,
* **(b)** tasks where Selene applies but offers no improvement, and
* **(c)** tasks outside Selene's scope.
...
one-sentence, user-facing recommendation — the quotes shown on the historical main page.
...
changes and simple one-file renames see no benefit from Selene. Neither report reads as promotional;
...
Java IDE plugin), they converge on the same core findings — cross-file refactoring is Selene's
...
**Prompt**:
> The attached prompt is designed to evaluate the additive value of Selene as an augmentation layer on top of built-in tools
> (not to compare two competing alternatives).
> Is the methodology suitable for that goal, or does it introduce bias that would distort the measurement of what Selene adds?
...
**Claude Opus' Answer**:
> "The methodology is well-suited to its stated goal: by explicitly scoping to correct use, treating Selene as an
...
> rule" could subtly favor Selene by preemptively filtering out any scenario where its tools underperform — but the prompt
...
but they are a poor fit for evaluating a **tool augmentation layer** like Selene for several reasons:
...
- **Benchmarks don't reflect real usage patterns.** Benchmark tasks are typically small, self-contained
  ...
  workflows where Selene's tools shine — cross-file refactoring, navigating large codebases by symbol
  structure, chaining multiple edits with stable addressing, or querying type hierarchies and
  external dependencies. A benchmark score would mostly measure performance on tasks where Selene
  is not expected to help.
- **Results would not generalise to the user's project.** Selene's value depends on the codebase
  ...
  agent tells you little about what Selene would add to *your* setup.
- **Predefined tasks bias the measurement.** Choosing specific tasks to evaluate inevitably
  introduces selection bias — we would end up picking tasks that either favour or disfavour Selene.
  We wanted an evaluation that systematically covers the full surface area of Selene's capabilities
