...
:class: note
**Generated by:** GPT-5.4 (high) in Codex<br>
**Codebase:** Selene JetBrains Plugin (Java)<br>
**Date**: 2026-04-13
:::
...
> **One-line summary:** As a coding agent, I would ask my owner to add Selene because it turns fragile text-and-line-number work into precise symbol-aware navigation and
refactoring, which makes real code changes feel faster, safer, and far less blind.

This report compares Selene's JetBrains-backed semantic coding tools with built-in file, shell, search, and patch tools in this repository. The comparison assumes competent use of both toolsets: built-ins are used for text, file, shell, config, and small patch work; Selene is used where symbol identity, language semantics, or IDE refactoring semantics apply.
...
Selene adds a semantic layer over the codebase. Its concrete delta is the ability to address and transform code by symbols: name paths, overload indexes, reference graphs, type hierarchies, external declarations, and JetBrains refactoring operations.

Tasks where Selene adds capability:
...
Tasks where Selene applies but offered little or no improvement:

- Tiny intra-method edits: a 1-line error-message change was smaller with a built-in patch; Selene body replacement required sending the whole method.
...
- Method insertion at a known spot: both workflows produced the same 4-line diff; Selene's benefit was target stability, not smaller payload.
- Whole-method rewrite: Selene targets the method boundary cleanly, but the full replacement body still has to be sent.

Tasks outside Selene's scope:
...
| 7 | Small local edits | High | Neutral/negative for Selene | Built-in patch/edit usually sends less. |

**Verdict:** Selene materially changes symbol-centric exploration and refactoring; it does not replace built-ins for ordinary text, file, shell, config, or tiny local edits.
...
- Semantic search separates code usage from text mention. Frequency: high. Value per hit: medium-high. Selene returned only code references for `Symbol/findReferences`; `rg` returned code, route strings, comments, and the declaration. Built-ins remain better for "mentioned anywhere."
...
- Type/dependency queries add capabilities not practically present in built-ins. Frequency: low-medium. Value per hit: high. Selene returned a transitive type graph and an external IntelliJ API method body; text tools needed iterative searches or extra source/decompiler setup.

- Small localized edits are not improved by symbol-body replacement. Frequency: high. Value per hit: neutral or negative for Selene. A one-line patch was smaller than replacing an 11-line method body.
...
**Verdict:** Selene's highest-value differences are symbol identity, reference graphs, and IDE refactor execution; built-ins remain superior for simple text-local work.
...
Selene call chain: not applicable for repository inventory and non-code config.
...
Payloads: built-ins used 5 small shell/read calls and returned several KB of code/config context; Selene had no relevant semantic operation.

**Verdict:** Repository layout and non-code entry-point discovery are built-in work; Selene starts adding value once the target is code symbols.

### 3.2 Large file structural overview

Attempted: compare structural overview on `src/main/java/local/selene/symbol/Symbol.java`.
...
Selene output: one top-level `Symbol` class; fields; nested `ChildrenCollector` and `DocumentationResolver`; methods including overload-indexed names such as `getLocationString[0]`, `getLocationString[1]`, `move[0]`, `move[1]`, `getDocumentation[0]`, and `getDocumentation[1]`.
...
Payloads: Selene used 1 compact overview call plus a targeted follow-up. Built-ins used 2 regex searches plus a later line-range read.

**Verdict:** Selene's overview is more actionable because its output feeds directly into stable symbol-addressed calls; regex outlines are useful but line-based.

### 3.3 Targeted method body retrieval

Attempted: retrieve `Symbol/findReferences` without reading surrounding file content.

Selene call chain: `find_symbol(relative_path=Symbol.java, name_path_pattern=Symbol/findReferences, include_body=true)`.

Selene output: only the `public ArrayList<SymbolReference> findReferences()` body.
...
Payloads: Selene input was one path/name-path request and returned roughly 25 method lines. Built-ins required search output plus a separate slice read.

**Verdict:** If the symbol is known, Selene retrieves the body in one precise call; built-ins need search plus line-targeted reading.

### 3.4 References: code usages vs mentions anywhere

Attempted: find all references to `Symbol/findReferences`.

Selene call chain: `find_referencing_symbols(relative_path=Symbol.java, name_path=Symbol/findReferences)`.
...
Payloads: Selene returned a compact grouped usage list of about 500-700 characters. `rg` returned broader line context of about 1-2 KB and mixed code usages with text mentions.

**Verdict:** Selene has higher precision for "who uses this symbol in code"; built-ins have higher recall for "where is this text mentioned anywhere."

### 3.5 Type hierarchy

Attempted: list supertypes and subtypes transitively for `PostRequestHandler`.

Selene call chain: `type_hierarchy(relative_path=PostRequestHandler.java, name_path=PostRequestHandler, hierarchy_type=both, depth=0)`.

Selene output: external `Object` supertype; 18 direct endpoint subclasses; transitive nested branch `TypeHierarchyHandler` containing `GetSupertypesHandler` and `GetSubtypesHandler`.
...
Payloads: Selene used 1 hierarchy query and returned structured JSON. Built-ins used pattern search and manual transitive grouping.

**Verdict:** Selene turns hierarchy discovery into a semantic graph query; text search requires iterative pattern expansion and manual reasoning.
...
Selene output: declaration `ReferencesSearch/search[0]` in an external class path and the overload body:
...
Infrastructure difference: Selene depends on the JetBrains IDE index; built-ins need source jars, decompiler tooling, or classpath-specific jar inspection.

**Verdict:** External dependency lookup is a genuine Selene capability when IDE indexes are available; ordinary built-ins do not provide it.
...
Observed diff: both produced the same 1-line change. Payload difference: built-in edit input was a tiny hunk; Selene input was the full 11-line method body.
...
Selene call chain: `replace_symbol_body(SymbolFinder/findFilesByName, new method body)`, then diff/status/revert/status.
...
Selene call chain: `replace_symbol_body(InspectionRunner/collectResults, full symbol text including Javadoc and replacement body)`, then diff/status/revert/status.

Observed diff: both produced `38 +++++++++++-----------`, 19 insertions and 19 deletions. Payload difference: built-in required a large contextual hunk; Selene required the full symbol text, about 70 lines including Javadoc/signature/body.

**Verdict:** Selene is not automatically more token-efficient for edits; its edit advantage is stable symbol targeting, while built-ins are smaller for tiny local hunks.
...
Payloads: built-ins sent inserted body plus surrounding text context; Selene sent inserted body plus symbol target.

**Verdict:** Structural insertion is a modest Selene improvement: same resulting diff, less dependence on line/context stability.
...
Observed result: Selene returned `"Success"`. Both produced the same 2-line diff.
...
**Verdict:** Cross-file class renames are high-value Selene cases because file/class coupling, imports, usages, and optional comments are handled as one semantic refactor.
...
1. `move(relative_path=InspectionRunner.java, name_path=InspectionRunner/PendingFix, target_relative_path=src/main/java/local/selene/service/endpoint)`.
...
```json
{
  "source_relative_path": "src/main/java/local/selene/service/endpoint/InspectionRunner.java",
  "target_relative_path": "src/main/java/local/selene/service/endpoint",
...
**Verdict:** Symbol move is a substantial Selene addition because the difficult part is reference/import repair, not copying text.
...
1. `move(relative_path=src/main/java/local/selene/util/PluginUtil.java, target_relative_path=src/main/java/local/selene/service)`.
2. Diff/status/cached-diff inspection.
...
```json
{
  "source_relative_path": "src/main/java/local/selene/util/PluginUtil.java",
  "target_relative_path": "src/main/java/local/selene/service"
}
```
...
- Changed package declaration to `de.local.selene.service`.
- Updated import in `PluginStartupActivity`.
- Removed now-unneeded same-package import in `SeleneBackendService`.
...
```json
{
  ...
  "relative_path": "src/main/java/local/selene/util/DebugUtil.java",
  "affected_references": [],
...
**Verdict:** Inline is a strong Selene refactor for legally substitutable helpers; built-ins can reproduce it but must manually adapt each substitution.
...
**Verdict:** Selene's overload-indexed name paths provide precise symbol addressing that text search does not.

### 3.16 Atomicity and success signals

Observed Selene success signals:
...
Atomicity comparison: Selene refactors executed as single IDE operations. Built-in equivalents are chains of searches, file operations, and patches; a competent user can make them correct, but intermediate states can be partial.
...
**Verdict:** Selene gives clearer semantic success signals and more atomic cross-file edits; built-ins require manual consistency management.
...
**Verdict:** Selene's name paths remain useful after line shifts, so its advantage compounds in multi-edit sessions.
...
3. `rg "http://|https://|localhost|127\\.0\\.0\\.1|8080|24538|SELENE|selene" src/main/java src/main/resources -n`.

Selene call chain: not applicable.
...
**Verdict:** Built-ins are natural and sufficient for config reading and free-text search; this is outside Selene's semantic-code scope.

## 4. Token-efficiency analysis

Payload differences across edit sizes:

| Task | Built-in payload | Selene payload | More efficient |
|---|---|---|---|
...
| Large body rewrite | Large contextual patch, 38-line diff | Full symbol text, about 70 lines | Roughly equal; Selene has better target |
| Insert method | Inserted body plus surrounding context | Inserted body plus name path | Selene slightly |
| Private one-file rename | `rg` plus 2-line patch | optional refs query plus rename command | Roughly equal |
| Cross-file rename/move/inline | Multiple searches, file operations, patches, verifications | One semantic refactor plus diff/status | Selene |

Forced reads:

- Built-ins commonly need a search before a precise read, then a line slice or full context before editing.
- Selene can skip full-file reads when the symbol name path is known.
- When the symbol is not known, Selene's `get_symbols_overview` and shallow `find_symbol` calls provide compact discovery rather than full-file reads.

Output payload:

- Selene exploration output is structured and scoped to symbols.
...
- Selene addresses symbols by `relative_path + name_path`, with overload indexes where needed. The chained-edit experiment showed those targets survived line shifts.
...
**Verdict:** Selene saves tokens by avoiding broad reads/search triage and by collapsing multi-file edit chains; built-ins remain more token-efficient for tiny known-location hunks.

## 5. Reliability & correctness under correct use

Precision of matching:

- Selene reference search returned code usages and excluded route strings/comments.
- Built-in text search returned all mentions and therefore mixed true code usage with broader text hits.
- Selene overload indexes targeted `Symbol/getLocationString[0]` and `[1]`; text search required manual signature inspection.

Scope disambiguation:

- Selene name paths encode class nesting and overload identity.
...
- Selene refactors apply through JetBrains refactoring operations and produce one operation-level success signal.
- Built-in equivalents are decomposed into separate searches, file moves, patches, and verification steps.
- Competent users can verify either path, but Selene reduces the number of intermediate inconsistent states.
...
- Selene's dependency lookup depends on IDE/language indexes and available dependency metadata.
- Built-ins can match or exceed it only if paired with source jars, decompilers, or separate language-server tooling.

Operational tradeoff:

- Several Selene refactors staged created/deleted files. This is not a correctness problem, but it adds index-state verification to the cleanup workflow.

**Verdict:** Selene improves correctness where symbol identity and cross-file semantics matter; built-ins remain reliable and simpler for text/file tasks.

## 6. Workflow effects across a session

Where Selene advantages compound:
...
Where Selene advantages diminish:
...
- For non-code and free-text work, Selene has no role.

Intermediate result durability:

- Selene intermediate results such as `SymbolFinder/findFilesByName` or `Symbol/getLocationString[1]` remain meaningful as long as the file/symbol still exists.
...
- Selene success signals reduce the need to inspect every call site manually, but not the need to inspect the intended diff.

**Verdict:** Selene's advantages compound inside symbol-centric sessions and diminish once the work becomes plain text, config, shell, or already-loaded single-file editing.
...
- External dependency declaration/body lookup from a call site. Frequency: low-medium. Impact: high for API behavior questions. Selene resolved `ReferencesSearch.search` into an external class method body; ordinary built-ins did not.
...
**Verdict:** Selene's unique practical capabilities are hierarchy/dependency queries and IDE refactors; its non-unique areas are ordinary text and file operations.

## 8. Tasks outside Selene's scope
...
- Selene applies to much of the remaining 50-70% when the work is code navigation, symbol understanding, references, hierarchy, or refactoring.
- Selene coverage rises in refactor-heavy sessions and falls in config/debug/test-log sessions.

**Verdict:** Selene augments the semantic-code portion of development; built-ins still carry a large and necessary share of routine work.

## 9. Practical usage rule

Use Selene when the task can be stated as a code-symbol operation:
...
2. Use Selene to identify and transform code symbols.
...
5. Prefer Selene for any refactor that crosses file boundaries or depends on type/reference semantics.

**Verdict:** Choose Selene for symbol identity and semantic refactoring; choose built-ins for text, shell, config, verification, and minimal local edits.
