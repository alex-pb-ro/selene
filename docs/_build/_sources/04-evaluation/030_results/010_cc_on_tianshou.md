...
:class: note
**Generated by**: Claude Opus 4.6 (coding AI agent in Claude Code CLI)<br>
...
> **One-line summary:** Selene's IDE-backed semantic tools are the single most impactful addition to my toolkit — cross-file renames, moves, and reference lookups that would cost me 8–12 careful, error-prone steps collapse into one atomic call, and I would absolutely ask any developer I work with to set them up.
...
Selene adds an IDE-backed semantic layer (powered by JetBrains) on top of the built-in text-level tools. The practical delta breaks down into three categories:

**(a) Tasks where Selene adds capability:**
- **Cross-file refactoring** (rename, move symbol, move file): 1 atomic call vs. N×(Grep+Read+Edit) chains. This is Selene's strongest contribution — it collapses multi-file, import-aware operations into single calls that are atomic and semantically correct.
...
- **External dependency introspection**: Selene can look up symbols in installed packages via the IDE index without requiring manual environment discovery.

**(b) Tasks where Selene applies but offers no improvement:**
...
- **Inserting new code at a known location**: Both approaches require ~1–2 calls. Edit can target by surrounding text; Selene targets by name path. The effort is comparable.

**(c) Tasks outside Selene's scope (built-in only):**
...
**Verdict:** Selene's primary contribution is collapsing multi-file, semantically-aware operations (rename, move, reference-finding, type hierarchy) from multi-step manual processes into single atomic calls; for single-file text edits, it is comparable or slightly less efficient.

---

## 2. Added Value and Differences by Area

**1. Cross-file rename/move (Selene: strong positive)**
- Selene: 1 call updates definition + all imports + all usages across N files atomically.
...
**2. Structural overview / targeted symbol reading (Selene: moderate positive)**
...
**3. Reference finding with structural context (Selene: moderate positive)**
...
**4. Type hierarchy (Selene: positive, niche)**
...
**6. Single-file small/medium edits (Selene: neutral to slight negative)**
- For a 1-line change inside a 13-line method, Selene's `replace_symbol_body` requires sending the entire 13-line body. Edit sends just the changed line. The prerequisite cost differs: Selene needs `find_symbol(include_body=True)` to get the body; Edit needs `Read` of the relevant lines. Both are 1 prerequisite call + 1 edit call.
- Frequency: Very high. Value per hit: Slightly negative for Selene — more payload for small changes.

**Verdict:** Selene's strongest contributions are in cross-file refactoring (high value, moderate frequency) and structured navigation (moderate value, high frequency); for small text edits, built-ins are slightly more efficient.
...
| Metric | Selene `get_symbols_overview(depth=1)` | Built-in `Grep` for class/def |
|---|---|---|
| Calls | 1 | 1 |
...
For `Collector/_collect` (330 lines): Selene returned exactly 330 lines. Built-in Read returned 332 lines (including the next method's signature).

**Verdict:** Selene saves 1 call and returns precisely the requested body; built-in is adequate but requires line-number discovery.
...
Grep finds 83 files including `README.md`, `CHANGELOG.md`, and `.ipynb` notebooks. Selene finds 63 code files. For the question "who uses this in code?", Selene's result is directly usable. For "where is this mentioned anywhere?", Grep is the right tool.

**Verdict:** Selene provides higher-precision code-usage results; Grep provides broader text-level coverage. Different tools for different questions.

### 3.4 Type Hierarchy (Task 5)

| Metric | Selene `type_hierarchy` | Built-in |
|---|---|---|
...
**Verdict:** Selene produces the full transitive hierarchy in 1 call; built-in requires iterative searches.
...
In this session, the Python environment wasn't directly accessible from bash, so built-in lookup would have required additional setup. Selene retrieved `torch.distributions.Distribution`'s location and docstring via the IDE index.

**Verdict:** Selene provides dependency introspection without environment setup; built-in requires a working interpreter.

### 3.6 Small Edit — 1 Line Change (Task 7a)

| Metric | Selene `replace_symbol_body` | Built-in `Edit` |
|---|---|---|
...
At this scale, Selene's payload is actually *smaller* because Edit must send both old and new text, while Selene sends only the new body. The crossover point is approximately where the changed region exceeds half the method body.

**Verdict:** At medium scale, payloads converge; Selene's is slightly smaller due to only sending the replacement.
...
**Verdict:** For full-method rewrites, Selene sends ~50% less payload because it only sends the replacement, not the original.

### 3.9 Cross-File Rename (Task 10)

| Metric | Selene `rename` | Built-in chain |
|---|---|---|
...
**Verdict:** Selene reduces a 9-call chain to 1 call for cross-file rename, with atomicity.

### 3.10 Move Symbol Across Modules (Task 11)

| Metric | Selene `move` | Built-in equivalent |
|---|---|---|
...
Moving `get_stddev_from_dist` from `collector.py` to `stats.py`: Selene updated 3 files (source, target, test) in 1 call, including adding necessary imports to the target module.

**Verdict:** Move is Selene's highest-value single operation — it handles import graph updates that would be error-prone manually.
...
**Verdict:** Same pattern as symbol move — Selene collapses an N-step process.

### 3.12 Safe Delete (Task 12b)

Selene's `safe_delete` in safe mode (default):
...
Selene's inline tool did not work for any Python function tested. The JetBrains backend does not support Python function inlining (this is a language-level limitation of the IDE's refactoring engine, not a Selene bug).
...
- Selene: `find_symbol("Collector/reset_env")` returns exactly that override's body.
- Grep: Returns 3 line numbers. Determining which belongs to which class requires reading surrounding context.

**Verdict:** Selene's name-path addressing eliminates class-level disambiguation that Grep requires.

### 3.15 Chained Edits (Task 17)

Three consecutive edits to methods in `CollectStats`:
- Selene: 3 `replace_symbol_body` calls, 0 intermediate reads. Name paths (`CollectStats/refresh_return_stats`, `CollectStats/refresh_len_stats`, `CollectStats/refresh_std_array_stats`) remained valid across all edits because they are structural identifiers, not positions.
...
The core asymmetry: Selene's addressing is *structural* (survives edits by definition), while built-in addressing is *positional* (line numbers from Grep/Read go stale after any insertion or deletion). Edit's text matching partially mitigates this, but only for the final step — the discovery steps (Grep, Read with offset) remain position-dependent.

**Verdict:** Selene's name-path stability eliminates the re-read/re-grep cycle between edits; built-in tools require re-acquiring positions after each edit that shifts line numbers.
...
| Edit scale | Selene payload (edit call) | Edit payload (edit call) | Winner |
|---|---|---|---|
| 1-line change in 13-line method | ~550 chars (full body) | ~120 chars | Edit (~4.5×) |
| 19-line medium rewrite | ~550 chars | ~1000 chars | Selene (~1.8×) |
| 55-line full rewrite | ~2200 chars | ~4400 chars | Selene (~2×) |
| Cross-file rename (4 files) | ~100 chars | ~800 chars (4 Edit calls) | Selene (~8×) |

### Prerequisite reads

- Selene: `find_symbol(include_body=True)` returns the symbol body. If you've already navigated to it during exploration, no additional call needed.
- Built-in: `Read` with offset/limit. Always required before Edit (enforced by the tool).

### Stable vs ephemeral addressing

- Selene's name paths (`CollectStats/refresh_return_stats`) are stable identifiers — they survive any edit to surrounding code. No re-read or re-discovery needed between edits.
...
- In a session with N edits to the same file, this costs up to N-1 additional Grep/Read round-trips with built-ins. With Selene, the cost is zero — the same name path works on the first and tenth edit.

**Verdict:** Selene is more token-efficient for medium-to-large edits and cross-file operations; Edit is more efficient for small, localized changes. Across multi-edit sessions, Selene's stable addressing avoids the re-read tax that compounds with each successive built-in edit.
...
- Selene: Exact symbol resolution via name paths. `Collector/reset_env` unambiguously selects one method.
- Edit: Text matching. Unique strings match correctly; non-unique strings fail (and Edit reports the error).

### Scope disambiguation
- Selene distinguishes overrides, overloads (via indices), and nested classes by name path.
- Grep/Edit cannot distinguish methods with the same name in different classes without reading surrounding context.

### Atomicity
- Selene's cross-file operations (rename, move) are atomic — all files updated or none.
...
- Selene: Available via IDE index, no environment setup needed. Can retrieve symbol docs and location.
...
**Verdict:** Selene provides stronger correctness guarantees for symbol-level operations (scope, atomicity, semantic precision); built-in tools are reliable for text-level operations with the caveat that scope disambiguation requires manual effort.
...
### Compound advantages
- **Exploration → Edit without position re-acquisition**: Selene's overview tools produce name paths that are directly usable as edit targets. The built-in path produces line numbers (from Grep) that are consumed by Read — but after an edit, those line numbers are stale. In a typical explore-edit-explore-edit cycle, Selene's name paths remain valid throughout while built-in line numbers must be re-acquired after each edit. Over 3 explore+edit cycles, this saves ~3–6 intermediate Grep/Read calls.
...
### Diminishing returns
- For purely exploratory sessions (reading code, no edits), Selene's advantage is modest — `Grep` and `Read` are nearly as fast for navigation, and Selene's structured output doesn't save many calls.
- For sessions dominated by small text edits (config changes, log message tweaks), Selene adds no value.
...
**Verdict:** Selene's advantages compound across multi-step refactoring sessions where each operation feeds into the next; for read-heavy or small-edit sessions, the advantage is marginal.
...
**Verdict:** Selene provides 3–5 capabilities with no practical built-in equivalent, concentrated in cross-file refactoring and semantic navigation.
...
Estimated share of daily work: These tasks constitute roughly 40–60% of a typical coding session (reading docs, running tests, searching for patterns, editing config). Selene's augmentation covers the remaining 40–60% where code-level semantic operations apply.

**Verdict:** Built-in tools handle roughly half of daily work that falls entirely outside Selene's scope; Selene augments the code-centric other half.
...
| Task type | Use |
|---|---|
| Cross-file rename, move, delete | Selene (unique capability) |
| Understanding class hierarchy or symbol relationships | Selene (1 call vs. iterative search) |
| Getting a structural overview of a large file | Selene (richer structure) or Grep (simpler, faster) |
| Reading a specific method body by name | Selene (direct) or Grep+Read (2 calls) |
| Small edit (1–5 lines inside a method) | Edit (less payload) |
| Full method rewrite | Selene `replace_symbol_body` (less payload, no re-read) |
...
| External dependency inspection | Selene (no env setup needed) |

**Verdict:** Use Selene for cross-file refactoring and semantic navigation; use built-ins for text-level edits, non-code files, and shell operations; for single-file code edits, choose based on edit size (Edit for small, Selene for full-body rewrites).
