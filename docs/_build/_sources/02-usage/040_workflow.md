...
Selene uses a project-based workflow.
A **project** is simply a directory on your filesystem that contains code and other files
that you want Selene to work with.

Assuming that you have project you want to work with (which may initially be empty),
setting up a project with Selene typically involves the following steps:

1. **Project creation**: Configuring project settings for Selene (and indexing the project, if desired)
2. **Project activation**: Making Selene aware of the project you want to work with
3. **Onboarding**: Getting Selene familiar with the project (creating memories)
4. **Working on coding tasks**: Using Selene to help you with actual coding tasks in the project

(project-creation-indexing)=
## Project Creation

Project creation is the process of defining fundamental project settings that are relevant to Selene's operation.

You can create a project either<br>
...
To explicitly create a project, use the following command while in the project directory:
    ...
    selene project create [options] [project directory]
 ...
 * For an existing project, the programming languages will be detected based on
   ...
   If multiple languages are detected, you will be prompted whether you want to enable them.<br>
...
After creation, you can adjust the project settings in the generated `.selene/project.yml` file
...
The file allows you to configure ...
  ...
  * the set of built-in language servers and explicitly registered external adapters for which servers are spawned
    ...
    adapters while Selene is running. It does not install adapter packages or offer unregistered adapter IDs.
...
[template file](../../src/selene/resources/project.template.yml).

:::{note}
Many settings in project.yml *extend* or *override* settings in [Selene's global configuration](global-config).
...
Indexing has to be called only once. During regular usage, Selene will automatically update the index whenever files change.

(project-activation)=
## Project Activation

Project activation makes Selene aware of the project you want to work with.
...
i.e. the folder that is activated in Selene should correspond to the root folder of the project in your IDE.

## Onboarding & Memories

By default, Selene will perform an **onboarding process** when
it is started for the first time for a project.
The goal of the onboarding is for Selene to get familiar with the project
and to store memories, which it can then draw upon in future interactions.

In general, **memories** provide a way for Selene to store and retrieve
...
are kept in sync across renames, and a `selene memories check` command is available
to report stale references.

During the first onboarding, Selene seeds a `memory_maintenance` memory describing the
...
convention, the `selene memories` CLI subcommands, and how to manage or disable these
features, see [Memories & Onboarding](045_memories).


## Preparing Your Project

When using Selene to work on your project, it can be helpful to follow a few best practices.

### Structure Your Codebase

Selene uses the code structure for finding, reading and editing code. This means that it will
...
**Important**: since Selene will write to files using the system-native line endings
...
Selene can successfully complete tasks in an _agent loop_, where it iteratively
acquires information, performs actions, and reflects on the results.
However, Selene cannot use a debugger; it must rely on the results of program executions,
...
and that has a good test coverage is much easier to work with for Selene.
...
i.e. a folder that contains all the projects as sub-folders, and open that monorepo folder as a project in Selene.
...
If, while working on a project, you want Selene to be able to read code or other information from another project (e.g. a library or otherwise related project),
this can be enabled via the `query_project` tool.
Provided that the project you want to query is known to Selene (i.e. you have created it as described above),
...
* When using the LSP backend, executing symbolic tools via the query tool requires that Selene's **Project Server** be started,
  which will automatically spawn the necessary language servers for the projects that are queried.

  To start the server, run

      selene start-project-server


### Multiple Agents Accessing a Single Selene Instance

If you want multiple agents to access the same project via a single Selene instance,
i.e. you do not want several instances of Selene (including its language servers) to be running,
you can achieve this by [starting the Selene MCP server in HTTP mode](streamable-http)
and connecting all client agents to the same HTTP endpoint.
The agents will then share the resources of the single Selene instance.

### Multiple Agents Working on Different Projects

For this use case, simply run a separate instance of Selene for each project, which naturally
occurs when Selene is started by the MCP client in stdio mode.
