...
Selene comes with built-in tools for monitoring and managing the current session:

* the **Selene Dashboard** (enabled by default)

  The dashboard provides detailed information on your Selene session, the current configuration and provides access to logs.
...
* the **GUI Log Viewer** (disabled by default)
  ...
  The GUI tool is a legacy native application which displays Selene's live logs.

  This is mainly supported on Windows, but it may also work on Linux; macOS is unsupported.

Both can be configured in Selene's [configuration](050_configuration) file (`selene_config.yml`).

## The Selene Dashboard
...
Configure your preferred interface via the setting `web_dashboard_interface` in [Selene's global configuration file](global-config) (see below).

By default, the dashboard can be accessed at `http://localhost:24538/dashboard/index.html`,
 ...
 * a detailed overview of
   * the current Selene status and configuration (e.g. active tools, active programming languages, enabled modes and contexts, etc.)
   * ongoing and past tool calls (and statistics on tool usage)
 * access to Selene's live logs
 * the ability to modify settings (e.g. the set of active programming languages) on the fly
 * the ability to edit
   * configuration files (global `selene_config.yml` and project-specific `project.yml`)
   * memories of the current project
 * the ability to shut down the Selene MCP server.
 ...
 * whether the dashboard is enabled at all.<br>
   We highly recommend keeping it enabled, as it provides valuable insights into Selene's operation and allows you to adjust configuration conveniently.
 * whether the dashboard window is opened automatically when Selene is started (see below).
 * network settings (for cases where you might want to access the Dashboard non-locally).

#### Dashboard Opening Behaviour

When Selene is started, the Dashboard window is opened by default in order to make users aware of its existence.
...
* asking the LLM to "open the Selene dashboard", which will open the dashboard in your default browser
...
The Selene GUI Log Viewer is a legacy application which provides access to Selene's live logs.
...
To enable the tool, set `gui_log_window` to `true` in Selene's [global configuration file](global-config).
