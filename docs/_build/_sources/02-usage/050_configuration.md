...
Selene is very flexible in terms of configuration. While for most users, the default configurations will work,
you can fully adjust it to your needs.

You can disable tools, change Selene's fundamental instructions
...
Selene is configured using a multi-layered approach:

 * **global configuration** (`selene_config.yml`, see below)
 ...
 * **command-line parameters** passed to the `start-mcp-server` server command (overriding/extending configured settings)<br>
   See [MCP Server Command-Line Arguments](mcp-args) for further information.<br>
...
Some of the configurable settings include:
  ...
  * UI settings affecting the [Selene Dashboard and GUI tool](060_dashboard.md)
...
[template file](../../src/selene/resources/selene_config.template.yml).

### Accessing the Configuration File

The configuration file is auto-created when you first run Selene. It is stored in your user directory:
  * Linux/macOS/Git-Bash: `~/.selene/selene_config.yml`
  * Windows (CMD/PowerShell): `%USERPROFILE%\.selene\selene_config.yml`

You can access it
  * through [Selene's dashboard](060_dashboard) while Selene is running (use the respective button)
...
Selene's behaviour and toolset can be adjusted using contexts and modes.
These allow for a high degree of customization to best suit your workflow and the environment Selene is operating in.

(contexts)=
### Contexts

A **context** defines the general environment in which Selene is operating.
It influences the initial system prompt and the set of available tools.
A context is set at startup when launching Selene (e.g., via CLI options for an MCP server or in the agent script) and cannot be changed during an active session.

Selene comes with pre-defined contexts:

* `desktop-app`: Tailored for use with desktop applications like Claude Desktop. This is the default.
  The full set of Selene's tools is provided, as the application is assumed to have no prior coding-specific capabilities.
...
* `agent`: Designed for scenarios where Selene acts as a more autonomous agent, for example, when used with Agno.

Choose the context that best matches the type of integration you are using.

Find the concrete definitions of the above contexts in `src/selene/resources/config/contexts/` in your checkout.
...
When launching Selene, specify the context using `--context <context-name>`.
...
You can manage contexts using the `context` command,
    ...
    selene context --help
    selene context list
    selene context create <context-name>
    selene context edit <context-name>
    selene context delete <context-name>


(modes)=
### Modes

Modes further refine Selene's behavior for specific types of tasks or interaction styles. Multiple modes can be active simultaneously, allowing you to combine their effects. Modes influence the system prompt and can also alter the set of available tools by excluding certain ones.

Examples of built-in modes include:

* `planning`: Focuses Selene on planning and analysis tasks.
* `editing`: Optimizes Selene for direct code modification tasks.
* `interactive`: Suitable for a conversational, back-and-forth interaction style.
* `one-shot`: Configures Selene for tasks that should be completed in a single response, often used with `planning` for generating reports or initial plans.
...
* `query-projects`: Enables tools for querying other Selene projects (without activating them); see section [Reading from External Projects](query-projects)

Find the concrete definitions of these modes in `src/selene/resources/config/modes/` in your checkout.

The modes to be activated are configured in:
  * the global configuration file (`selene_config.yml`)
...
Ultimately, the active modes are given by the union of
  * `base_modes` defined in the global configuration (always active)<br>
...
Selene currently does not prevent incompatible combinations; it is up to the user to choose sensible mode configurations.
:::
...
You can manage modes using the `mode` command,
    ...
    selene mode --help
    selene mode list
    selene mode create <mode-name>
    selene mode edit <mode-name>
    selene mode delete <mode-name>

(prompt-templates)=
## Prompt Templates

All prompts that Selene provides to the LLM are [Jinja2](https://jinja.palletsprojects.com/) templates.
Templating applies to

 * **Selene's system prompt** (the "Selene Instructions Manual"), which is defined in the prompt template `system_prompt`
...
Context, mode and project prompts (but not Selene's system prompt) additionally support the following function:
...
For advanced users, Selene's configuration can be further customized.

### Selene Data Directory

The Selene user data directory (where configuration, language server files, logs, etc. are stored) defaults to `~/.selene`.
You can change this location by setting the `SELENE_HOME` environment variable to your desired path.

### Per-Project Selene Folder Location

By default, each project stores its Selene data (memories, caches, etc.) in a `.selene` folder inside the project root.
You can customize this location globally via the `project_selene_folder_location` setting in `selene_config.yml`.
...
project_selene_folder_location: "$projectDir/.selene"

# Central location: all project data under a shared directory
project_selene_folder_location: "/projects-metadata/$projectFolderName/.selene"
```

When a project is loaded, Selene uses the following fallback logic:
1. Check if a `.selene` folder exists at the configured path.
...
This ensures backward compatibility: existing projects that already have a `.selene` folder in the project root will continue to work, even after changing the `project_selene_folder_location` setting.
...
Under the key `ls_specific_settings` in `selene_config.yml`, you can you pass global per-language,
...
The settings are merged on top-level, meaning that project-level settings for a language will replace global settings for the same language.<br>
...
Most of Selene's language servers construct the command that launches the language server process from
...
| Setting                                    | Description                                                                                                                                                                                                                                                                                                                                           |
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ls_path` (string) or `ls_base_cmd` (list) | overrides the path of the language server's core dependency (`ls_path`), e.g. its executable or a JAR file, or a base command for its execution (`ls_base_cmd`), e.g. `["npx", "-y", "/my/local/package"]`. Use this if you have installed the language server yourself and want Selene to use your installation instead of its managed installation. |
...
If `ls_path` is set, Selene's managed download or install is bypassed for that language server.
...
When Selene starts a language server, it sends a set of `initializationOptions` as part of the
...
* If Selene constructs `initializationOptions` for the language server, each top-level key you
...
* If Selene does not construct any `initializationOptions` for the language server, your custom
...
Selene uses the AL language server bundled in the Microsoft Dynamics 365 Business Central VS Code extension.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `al_extension_version` | `18.0.2242655` | Override the AL VS Code extension version Selene downloads from the VS Code Marketplace. |

#### Angular

Selene uses `@angular/language-server` (`ngserver`) for the `angular` language key, orchestrated together with a
...
| Setting | Default | Description |
|---|---|---|
| `angular_language_server_version` | `21.2.10` | Override the bundled `@angular/language-server` npm package version Selene installs. |
...
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. Falls back to `ls_specific_settings.typescript.npm_registry` if unset. |
...
Selene uses `@ansible/ansible-language-server` for the `ansible` language key.
...
| `ansible_language_server_version` | `1.2.3` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
| `ansible_path` | `"ansible"` | Path to the `ansible` executable forwarded to the language server. |
| `ansible_settings` | `null` | Full Ansible LS settings dict, deep-merged on top of Selene's defaults. |
...
Selene uses `bash-language-server` for Bash support.
...
| `bash_language_server_version` | `5.6.0` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |

#### BSL (1C:Enterprise / OneScript)

Selene uses [bsl-language-server](https://github.com/1c-syntax/bsl-language-server) by 1c-syntax
...
| Setting | Default | Description |
|---|---|---|
| `ls_path` | managed download | Override the path to an existing `bsl-language-server-*-exec.jar`. When set, Selene does not download anything; the JAR is launched directly via `java -jar`. |
| `bsl_ls_version` | `0.29.0` | Override the bsl-language-server release version Selene downloads when `ls_path` is not set. SHA-256 verification is performed only for the default version; user-overridden versions install without SHA verification. |
...
Selene uses `clojure-lsp` for Clojure support.
...
| `clojure_lsp_version` | `2026.02.20-16.08.58` | Override the `clojure-lsp` release version Selene downloads when `ls_path` is not set. |
...
Example — a monorepo without a `.lsp/config.edn`, where you want to override what Selene scanned:
...
Selene uses `clangd` for the `cpp` language key.
...
| `compile_commands_dir` | `.selene` | Directory where Selene writes a transformed `compile_commands.json` if the project's original database uses relative `directory` entries. |
| `clangd_version` | `19.1.2` | Override the `clangd` version Selene downloads when `ls_path` is not set. |

#### C/C++ via `ccls`

Selene uses the `cpp_ccls` language key for `ccls`.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `ls_path` | `ccls` from PATH | Override the `ccls` executable path. Selene does not manage `ccls` downloads or installs. |


#### C# (Roslyn Language Server)

Selene uses [Microsoft's Roslyn Language Server](https://github.com/dotnet/roslyn) for C# support.

**Runtime Requirements:**

- .NET 10 or higher is required. If not found in PATH, Selene automatically installs it using Microsoft's official install scripts.
...
- If you have .NET 10+ already installed, Selene will use your system installation.

#### C# (`OmniSharp`)

Selene uses the `csharp_omnisharp` language key for OmniSharp.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `omnisharp_version` | `1.39.10` | Override the OmniSharp version Selene downloads. |
| `razor_omnisharp_version` | `7.0.0-preview.23363.1` | Override the Razor OmniSharp plugin version Selene downloads. |

#### Dart

Selene uses the Dart SDK's built-in language server for Dart support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `dart_sdk_version` | `3.7.1` | Override the Dart SDK version Selene downloads. |

#### Elixir

Selene uses [Expert](https://github.com/elixir-lang/expert) for Elixir support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `expert_version` | `v0.1.0-rc.6` | Override the Expert version Selene downloads when it does not use an `expert` executable already found in PATH. |

#### Elm

Selene uses `@elm-tooling/elm-language-server` for Elm support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `elm_language_server_version` | `2.8.0` | Override the npm package version Selene installs when no system `elm-language-server` is found. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |

#### F#

Selene uses FsAutoComplete (Ionide LSP) for F# support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `fsautocomplete_version` | `0.83.0` | Override the FsAutoComplete version Selene installs as a .NET tool. |


#### GDScript (Godot Engine)

Selene connects to the Godot editor's built-in LSP server over TCP. No separate process is launched.
...
Selene forwards `ls_specific_settings.go.gopls_settings` to `gopls` as LSP `initializationOptions` when the Go language server is started.
...
- `GOFLAGS` (from the environment you start Selene in) may also affect the Go build context. Prefer `buildFlags` for tags.
- Build context changes are only picked up when `gopls` starts. After changing `gopls_settings` (or relevant env vars like `GOFLAGS`), restart the Selene process (or server) that hosts the Go language server, or use your client's "Restart language server" action if it causes `gopls` to restart.

#### Groovy

Selene uses a user-provided Groovy Language Server JAR for Groovy support. If `ls_java_home_path` is not set, Selene downloads
...
| `ls_java_home_path` | `null` | Path to a Java installation to use instead of Selene's managed runtime |
| `ls_jar_options` | `""` | Additional options passed when launching the Groovy LS JAR |
| `vscode_java_version` | `1.42.0-561` | Override the bundled Java runtime bundle version Selene downloads by default |

Note:
- When overriding `vscode_java_version`, Selene still assumes that the downloaded runtime bundle keeps the same internal
...
Selene uses `shader-language-server` for the `hlsl` language key.
...
| `version` | `1.3.1` | Override the bundled version Selene downloads, or builds from source on macOS, when `ls_path` is not set. |


#### Haxe

Selene uses the [vshaxe/haxe-language-server](https://github.com/vshaxe/haxe-language-server) for Haxe support.
...
| `buildFile` | auto-discovered `.hxml` | Relative path to the `.hxml` build file used for compilation (e.g., `build/debug.hxml`). If not set, Selene searches the project for `.hxml` files (max depth 5, skipping dependency directories). |
...
Selene uses `vscode-html-language-server` from Microsoft's `vscode-langservers-extracted` npm package for the
...
| `vscode_langservers_version` | `4.10.0` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
...
1. **Default vscode-java VSIX mode** (no extra config required): Selene downloads the platform-specific
...
- **Default vscode-java VSIX mode** — recommended for most users. No setup required;
  Selene downloads everything on first use.
...
**JDK 21+ is required** in upstream mode. Selene resolves the JDK in this order:
...
| `gradle_java_home` | `null` | Path to the JDK used by Gradle. When unset, Gradle uses `JAVA_HOME` if `use_system_java_home` is enabled and `JAVA_HOME` is set; otherwise it falls back to Selene's bundled JRE. |
...
| `gradle_version` | `8.14.2` | (vscode-java mode only) Override the Gradle distribution version Selene downloads by default. |
| `vscode_java_version` | `1.54.0-923` | (vscode-java mode only) Override the bundled `vscode-java` runtime bundle version Selene downloads by default. |
...
- When overriding `vscode_java_version`, Selene still assumes that the downloaded runtime bundle keeps the same internal
  directory layout and file names as the bundled default version.
- Selene does not download or load IntelliCode because its completion ranking is not used by Selene's tools. The retired
...
- In upstream-jdtls mode Selene does not ship a Gradle distribution. Maven projects work via JDTLS's bundled m2e.
...
- Without `runtimes`, JDT-LS only knows about the bundled `JavaSE-21` JRE. Projects that request a newer
  ...
  `java.lang.Object`. Register the matching installed JDK via `runtimes` instead of symlinking over Selene's
...
Selene uses [JetBrains' Kotlin Language Server](https://github.com/Kotlin/kotlin-lsp) for Kotlin support.
...
| `kotlin_lsp_version` | `262.9593.0` | Override the Kotlin Language Server version Selene downloads when `ls_path` is not set. |
...
`bin/intellij-server` (`bin/intellij-server.exe` on Windows). Selene also retains the legacy download
...
Selene uses `lean --server` for Lean 4 support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `ls_path` | `lean` from PATH | Override the `lean` executable path. Selene does not manage Lean downloads. |

#### Lua

Selene uses `lua-language-server` for Lua support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `lua_language_server_version` | `3.15.0` | Override the bundled `lua-language-server` version Selene downloads when it cannot use an existing installation from PATH or common install locations. |


#### Luau

Selene uses [`luau-lsp`](https://github.com/JohnnyMorganz/luau-lsp) for Luau support.

**Runtime Requirements:**

- `luau-lsp` is used from PATH if available.
- Otherwise, Selene downloads the pinned `luau-lsp` release for the current platform.
...
Notes:
- In `roblox` mode, Selene downloads Roblox definitions and Roblox API docs and passes them to `luau-lsp`.
- In `standard` mode, Selene skips Roblox definitions and only downloads the standard Luau docs bundle.

#### Markdown

Selene uses [Marksman](https://github.com/artempyanykh/marksman) for the `markdown` language key.
...
| `marksman_version` | `2024-12-18` | Override the Marksman release tag Selene downloads when `ls_path` is not set. |

#### MATLAB

Selene uses the official MathWorks MATLAB language server from the VS Code extension.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `matlab_path` | auto-detected | Path to the MATLAB installation. This overrides `MATLAB_PATH` and auto-detection, but not Selene's managed extension download. |
| `matlab_extension_version` | `1.3.9` | Override the MathWorks VS Code extension version Selene downloads. |

#### Nix

Selene uses [nixd](https://github.com/nix-community/nixd) for Nix support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `ls_path` | PATH/common-path discovery followed by managed installation | Absolute path to a nixd executable or launcher. When set, Selene bypasses its nixd discovery, installation, and version check. |
...
Selene loads this file once when creating the language server, uses it as nixd's `initializationOptions`, and serves the same effective
...
remain top-level overrides and are reflected in both paths. Restart Selene after changing the JSON file.


#### Pascal (`pasls`)

Selene uses [pasls](https://github.com/genericptr/pascal-language-server) (Pascal Language Server) for Pascal/Free Pascal support.

**Language Server Installation:**

1. If `pasls` is found in your system PATH, Selene uses it directly
2. Otherwise, Selene automatically downloads a prebuilt binary from GitHub releases

Supported platforms for automatic download: Linux (x64, arm64), macOS (x64, arm64), Windows (x64).

**Auto-Update:**

Selene automatically checks for pasls updates every 24 hours. Updates include:
...
Configure pasls via `ls_specific_settings.pascal` in `selene_config.yml`:

| Setting          | Description                                                                 |
| ---------------- | --------------------------------------------------------------------------- |
| `pasls_version`  | Override the pinned pasls version Selene downloads by default               |
...
Selene uses [Perl::LanguageServer](https://metacpan.org/pod/Perl::LanguageServer) for Perl support. Install Perl and the server with `cpanm Perl::LanguageServer`; Linux and macOS only (the server does not run on Windows).
...
Configure the language server via `ls_specific_settings.perl` in `selene_config.yml`:
...
Notes:
- Extensions added via `file_filter` are also synced into Selene's Perl source-file matcher, so `find_symbol` and symbol indexing treat the same files as the language server. Defaults are unchanged when these keys are omitted.
- The matcher is reset on every language server activation, so one project's `file_filter` does not leak into another.

#### PHP (`Intelephense`)

Selene uses Intelephense for the `php` language key.
...
| `intelephense_version` | `1.14.4` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
...
Notes:
- Extensions added via `file_filter` are synced into Selene's PHP source-file matcher and pushed to Intelephense as `intelephense.files.associations` globs at startup, so `find_symbol` and the language server treat the same files as PHP sources.
- The matcher is reset on every language server activation, so one project's `file_filter` does not leak into another.

#### PHP (`Phpactor`)

Selene uses the `php_phpactor` language key for Phpactor.
...
| `phpactor_version` | `2025.12.21.1` | Override the Phpactor PHAR version Selene downloads when `ls_path` is not set. |
| `ignore_vendor` | `true` | Ignore directories named `vendor` while indexing the project. |

#### PowerShell

Selene uses PowerShell Editor Services for PowerShell support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `pses_version` | `4.4.0` | Override the PowerShell Editor Services version Selene downloads. Selene still requires `pwsh` to be available locally. |

#### Python

Selene supports several Python language servers through separate language keys.
...
| Setting | Default | Description |
|---|---|---|
| `pyright_version` | `1.1.403` | Override the exact Pyright package version Selene launches through `uvx` / `uv tool run`. |
...
| Setting | Default | Description |
|---|---|---|
| `basedpyright_version` | `1.39.9` | Override the exact BasedPyright package version Selene launches through `uvx` / `uv tool run`. |
...
Selene uses Shopify's `ruby-lsp` for Ruby support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `ruby_lsp_version` | `0.26.8` | Override the `ruby-lsp` gem version Selene installs when no project-local or global `ruby-lsp` is already available. |

#### Rust

Selene uses `rust-analyzer` for Rust support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `ls_path` | auto-detected | Override the `rust-analyzer` executable path. Without `ls_path`, Selene prefers `rustup which rust-analyzer`, then `rustup component add rust-analyzer`, then PATH/common install locations. |

#### Scala

Selene uses Metals for Scala support.
...
given. Selene detects them automatically; `project_roots` overrides that detection where it guesses
wrong, and `project_root_scan_depth` bounds how far it looks.

Metals reports its build import, indexing and compilation as LSP work-done progress, and Selene waits
...
| Setting | Default | Description |
|---|---|---|
| `metals_version` | `1.6.4` | Override the Metals version Selene bootstraps. |
| `client_name` | `Selene` | Client identifier sent to Metals. |
| `on_stale_lock` | `auto-clean` | How Selene handles stale Metals H2 database locks. Supported values: `auto-clean`, `warn`, `fail`. |
...
Selene uses [`some-sass-language-server`](https://github.com/wkillerud/some-sass) for the `scss` language key.
...
share the same engine; CSS feature toggles default to off upstream and Selene flips them on at startup so that
...
| `some_sass_version` | `2.3.8` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |

#### Solidity

Selene uses `@nomicfoundation/solidity-language-server` for Solidity support.
...
| `solidity_language_server_version` | `0.8.4` | Override the npm package version Selene installs when `ls_path` is not set. |
| `solidity_state_dir` | `<ls_resources_dir>/solidity-state` on macOS | Writable state root for the managed Solidity language server on macOS. Selene uses a child-process-only home-directory override so Hardhat does not write to `~/Library`; `HOME` in the Selene process is unchanged. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
...
Selene uses `verible-verilog-ls` for SystemVerilog support.
...
| `verible_version` | `v0.0-4051-g9fdb4057` | Override the Verible release Selene downloads when `ls_path` is not set and no system installation is found. |

#### Terraform

Selene uses `terraform-ls` for Terraform support.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `terraform_ls_version` | `0.36.5` | Override the `terraform-ls` version Selene downloads. Terraform itself must still be installed and available in PATH. |

#### TOML

Selene uses [Taplo](https://github.com/tamasfe/taplo) for the `toml` language key.
...
| `taplo_version` | `0.10.0` | Override the Taplo version Selene downloads when `ls_path` is not set. |

#### TypeScript

Selene uses `typescript-language-server` for the `typescript` language key.
...
| `typescript_version` | `5.9.3` | Override the bundled `typescript` npm package version Selene installs when `ls_path` is not set. |
| `typescript_language_server_version` | `5.1.3` | Override the bundled `typescript-language-server` npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
| `indexing_timeout` | `30.0` | Timeout in seconds for waiting on tsserver's `$/progress` project-indexing signal to *drain* once it has started (both at startup and before the first cross-file reference query). If indexing does not complete within this window, Selene logs a warning and proceeds anyway. Increase it for very large projects. |
| `server_ready_timeout` | `10.0` | Timeout in seconds for waiting on the server-ready signal after initialization. If the signal does not arrive within this window, Selene logs a message and proceeds anyway. |
| `indexing_start_grace` | `5.0` | Timeout in seconds to wait for tsserver to *start* reporting `$/progress` before the first cross-file reference query. tsserver must resolve the project graph before it can emit the first progress token, and that can take longer than the default on a very large project; if it takes longer than this window, Selene assumes no indexing was needed and may return incomplete cross-file references. Raising `indexing_timeout` alone does not help here, since this grace elapses first. Increase this for very large projects if `find_referencing_symbols`/`request_references` returns incomplete results shortly after project load. |

#### Svelte

Selene uses `svelte-language-server` for the `svelte` language key. Use `svelte` for Svelte projects instead of also listing `typescript`, unless you intentionally want multiple language servers active for the same files.

A companion TypeScript language server (`typescript-language-server` + `typescript-svelte-plugin`) is spawned automatically alongside the Svelte LSP. The plugin makes the TypeScript program `.svelte`-aware so that cross-file operations — rename, go-to-definition, and find-references from `.ts`/`.js` files — correctly include `.svelte` consumers. Selene merges and deduplicates reference results from both servers automatically.
...
| `svelte_language_server_version` | `0.18.0` | Override the `svelte-language-server` npm package version Selene installs. |
...
| `npm_registry` | `null` | Override the npm registry Selene uses for all managed installs. |
...
| Setting | Default | Description |
|---|---|---|
| `vtsls_version` | `0.2.9` | Override the `@vtsls/language-server` npm package version Selene installs. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |
...
vtsls reads `typescript.tsdk` through the `workspace/configuration` pull, not through `initializationOptions`, so Selene answers those pulls from the same dict (and also pushes it on `workspace/didChangeConfiguration` for compatibility with servers that expect the notification). Without `autoUseWorkspaceTsdk: true`, vtsls falls back to its bundled TypeScript and ignores `tsdk` (there is no UI prompt to confirm the switch in a headless LSP).

The dict is forwarded to vtsls verbatim — Selene does not validate its structure. For the list of supported keys and their expected types, refer to the vtsls [configuration schema](https://github.com/yioneko/vtsls/blob/main/packages/service/configuration.schema.json) and the underlying [VS Code TypeScript settings](https://code.visualstudio.com/docs/languages/typescript). `null` (the default) and `{}` are both treated as "unset": no `initializationOptions` are sent and no `workspace/didChangeConfiguration` notification is pushed. A non-dict value (e.g. a string or list) raises an error at server start.
...
- *Need to inspect what Selene is actually forwarding* — the dict is logged at INFO level via the `Forwarding user-provided initializationOptions to vtsls: …` line at language server startup.

#### Vue

Selene uses `@vue/language-server` (Volar) for the `vue` language key, together with a companion TypeScript language server.

Supported settings:

| Setting | Default | Description |
|---|---|---|
| `vue_language_server_version` | `3.1.5` | Override the bundled `@vue/language-server` npm package version Selene installs. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. If unset on `vue`, Selene falls back to `ls_specific_settings.typescript.npm_registry`. |
...
Selene uses `yaml-language-server` for the `yaml` language key.
...
| `yaml_language_server_version` | `1.19.2` | Override the npm package version Selene installs when `ls_path` is not set. |
| `npm_registry` | `null` | Override the npm registry Selene uses for the managed install. |

(custom-prompts)=
### Custom Prompts

All of Selene's prompts can be fully customized.
We define prompt as jinja templates in yaml files, and you can inspect our default prompts in `src/selene/resources/config/prompt_templates/` in your checkout.

To override a prompt, simply add a .yml file to the `prompt_templates` folder in your Selene data directory
which defines the prompt with the same name as the default prompt you want to override.
For example, to override the `system_prompt`, you could create a file `~/.selene/prompt_templates/system_prompt.yml` (assuming default Selene data folder location)
with content like:
...
On startup, Selene reports anonymous usage data to help us understand Selene usage.
Specifically, we collect the Selene version, the operating system & language backend being used as well as the dashboard enabled status.
No personally identifiable information or project-specific information is collected.

If you want to opt out of usage reporting, set the environment variable `SELENE_USAGE_REPORTING` to `false`.
