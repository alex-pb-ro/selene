...
## Selene's Assumptions

The current security model for Selene assumes:

- the local machine is trusted,<br>
- the MCP client (i.e. the LLM) is trusted,
- the code repository being worked on is trusted,
- user configuration is trusted,
- package manager configuration (e.g. npm) for downloading additional dependencies (i.e. language servers when using Selene with the LSP backend) is trusted.

Selene contains tools for executing shell commands and modifying files.
...
Therefore, the only way to *fully* protect against unintended consequences is to use a [sandboxed environment](sandboxing) for running Selene.
...
However, reports which amount to noting that Selene's tools can execute commands or modify files
describe intended functionality rather than vulnerabilities, and we will reject advisories that fail to recognise this
or otherwise ignore the above assumptions.<br>
...
- do not expose [Selene's network services](network-security) to untrusted networks.
...
[Running Selene inside a docker container](docker) which only exposes the necessary files and tools to the agent is a good way to achieve this.
...
Sandboxing limits what Selene can affect while doing what it was asked to do.
The notion of *trusted projects* (introduced in Selene v1.6.0) addresses a different question:
to what extent may the repository being worked on influence Selene's behaviour in the first place?

A project is an input authored by whoever produced the repository, and it comprises more than source code:
it also carries configuration (`.selene/project.yml`) as well as file system structure.
Trust determines whether such repository-supplied input may influence Selene beyond having the code read and
analysed as code — for example, by executing commands, by changing how dependencies are acquired, or by causing
Selene to access locations outside the project root.
Trust is decided by the project's root path, which is matched against `trusted_project_path_patterns` in Selene's
...
- `activation_command` is a shell command that a project can request to be run whenever it is activated.
  Without trust gating, merely opening a repository in Selene would execute code chosen by its author,
...
Selene includes several network services:
- the Selene MCP server itself (when run in [HTTP or SSE mode](streamable-http) instead of stdio mode)
- the Selene Dashboard web server
- the Selene JetBrains Plugin server, which runs within the JetBrains IDE (when using the JetBrains language backend)
- the Selene Project Server (only started explicitly for [project querying](query-projects))
...
Selene has two language backends with different security characteristics:
...
For convenience, Selene downloads or installs certain language server dependencies on demand.
...
- hash verification,
- host restriction,
- and isolated Selene-managed installation directories.

### Auto-Downloaded Language Server Dependencies

For language servers that are auto-installed by downloading archives, binaries, VSIX packages, NuGet packages, or other release artifacts, Selene uses a hardened shared download path with the following protections:

- **Pinned versions by default**: default downloads use exact versions instead of floating `latest` or nightly channels.
- **Integrity verification**: downloaded artifacts are checked against pinned SHA256 hashes stored in Selene's source code.
...
- **Managed install locations**: dependencies are installed into Selene-managed directories instead of into the project repository.
...
If any of these checks fail, Selene aborts the installation instead of continuing.

### npm-Based Language Servers

Some language servers are distributed primarily through npm. For those, Selene currently uses pinned package versions and installs them into Selene-managed directories.

By default, Selene uses the **user's normal npm configuration**. We do **not** force a registry override unless one is explicitly configured. If needed, both the package version and the registry can be overridden through `ls_specific_settings`.

For npm-based installs, Selene's current security posture is based on these rules:

- **Exact package versions are pinned by default**.
- **The install location is isolated from the project** and lives in Selene-managed language-server directories.
- **The user's npm configuration is trusted by default**.
- **Repository and user configuration are assumed to be trusted**.

This means Selene protects well against accidental version drift, but npm installs still rely on the npm ecosystem and package-manager execution model. In particular, Selene does **not** currently use lockfile-based `npm ci` installs for bundled language-server dependencies.

### `uvx` and Python Dependency Pinning

Some parts of Selene rely on `uv` / `uvx`.

One important detail is that `uvx` ignores the lockfile when installing directly from a Git repository. Because of that, we pin Selene's Python dependencies exactly in `pyproject.toml` so that installations from Git still resolve to exact dependency versions rather than floating ranges.
