...
Selene provides the functionality of a fully featured agent, and a useful aspect of this is Selene's memory system.
...
Selene differentiates between
  * **project-specific memories**, which are stored in the `.selene/memories/` directory within your project folder, and
  * **global memories**, which are shared across all projects and, by default, are stored in `~/.selene/memories/global/`
...
why Selene ships its own memory layer rather than reusing one.
...
Memories may reference each other. Selene recognizes a reference as a memory name prefixed with
...
- **Renames keep references intact.** When you rename or move a memory with the `rename_memory`
  tool, Selene rewrites every `` `mem:OLD_NAME` `` occurrence across all memories to point to
...
it can be helpful to track global memories with git (i.e. to make `~/.selene/memories/` a git repository)
...
(e.g., `.selene/memories/_archive/2026-03/some-topic.md`).
...
Alternatively, access them via the [Selene Dashboard](060_dashboard), which provides a graphical interface for
viewing, creating, editing, and deleting memories while Selene is running.

(onboarding)=
## Onboarding

By default, Selene performs an **onboarding process** when it encounters a project
...
In further project activations, Selene will check whether onboarding was already
performed by looking for existing project memories and will skip the onboarding
process if memories are found.

### How Onboarding Works

1. When a project is activated, Selene checks whether onboarding was already
   performed (by checking if any memories exist).
2. If no memories are found, Selene triggers the onboarding process, which
   reads key files and directories to understand the project.
3. Before any project memory is written, Selene materializes a project-local
...
To make memory conventions discoverable to both the LLM and the user, Selene seeds
a `memory_maintenance` memory on first onboarding. The seed is copied from a template
shipped with the Selene package and contains the dense agent-notes style, the
...
1. If you already maintain a `global/memory_maintenance` memory, Selene uses that
...
2. Otherwise, if the project already has a `memory_maintenance` memory, it is left
   untouched.
3. Otherwise, the shipped template is written to `.selene/memories/memory_maintenance.md`.
...
Selene also offers memory-related CLI commands.

The following commands have **no MCP tool counterpart** and are intended for human execution:

- `selene memories check` — referential-integrity report. By default reports stale
  `` `mem:NAME` `` references; additional scans (bare occurrences and fuzzy near-misses)
  are opt-in via flags. Run `selene memories check --help` for the full flag list.
- `selene memories auto-prefix-references` — heuristic rewrite of bare occurrences to add
  the `mem:` prefix; supports `--dry-run`.
- `selene memories initialize` will seed the `memory_maintenance` memory for the project.

The remaining commands mirror the MCP tools, you can thus instruct your agent to manage memories with
selene without having a running MCP server. Discover the full surface and per-command flags via:

```shell
selene memories --help
selene memories <subcommand> --help
```
 ...
 * To disable all memory related tools (including onboarding), adding `no-memories` to the `base_modes`
   in Selene's [global configuration](050_configuration).
