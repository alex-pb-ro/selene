# Scala Setup Guide for Selene

This guide explains how to prepare a Scala project so that Selene can provide reliable code intelligence via Metals (Scala LSP) and how to run Scala tests manually.

Selene automatically bootstraps the Metals language server using Coursier when needed. Your project, however, must be importable by a build server (BSP) — typically via Bloop or sbt’s built‑in BSP — so that Metals can compile and index your code.
...
- Coursier command (`cs`) or the legacy `coursier` launcher
  - Selene uses `cs` if available; if only `coursier` exists, it will attempt to install `cs`. If neither is present, install Coursier first.

---
## Quick Start

Start Selene in your project root. Metals asks whether to import a workspace it has not seen before, and Selene answers that prompt for it — the build is imported (for sbt, by running `sbt bloopInstall`), `.bloop/` and `.metals/` are created, and cross-file navigation works from there. The first run therefore takes as long as your build takes to load.
...
4. Once the import completes, start Selene in your project root and use it.
...
4. Start Selene in your project root. Selene will bootstrap Metals (if not already present) and connect to the build server using the configuration exported above.

---
## Using Selene with Scala

- Selene automatically detects Scala files (`*.scala`, `*.sbt`) and will start a Metals process per project when needed.
- On first run, you may see messages like “Bootstrapping metals…” in the Selene logs — this is expected.
...
Metals serves one build per workspace folder, so what it needs is the build roots, not the repository root. Selene detects them: if the repository root is not itself a build root (no `build.sbt`, `build.mill`, `pom.xml`, `.bsp/`, …), it searches up to three levels below for directories that are, and passes those to Metals — one Metals service per build.

Override the detection where it guesses wrong:

```yaml
# ~/.selene/selene_config.yml or .selene/project.yml
...
server writes only as it compiles. Selene waits for all of that, tracking the work-done progress
...
Selene can run alongside other Metals instances (e.g., VS Code with Metals extension) on the same project. This is **fully supported** by Metals via H2 AUTO_SERVER mode.
...
Selene automatically detects and handles stale locks based on your configuration:
