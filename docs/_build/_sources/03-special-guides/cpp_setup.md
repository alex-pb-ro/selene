...
This guide explains how to prepare a C/C++ project so that Selene can provide reliable code intelligence via clangd or ccls language servers.
This is only necessary if you use the language server variant of Selene, for users of the Selene JetBrains plugin no setup is required
...
Selene supports two C/C++ language servers, clangd (default) and ccls.
...
By default, Selene automatically downloads and manages clangd. Since clangd does not properly work with relative paths in `compile_commands.json`,
Selene will detect them and transform them into absolute paths automatically (writing a new `compile_commands.json` file), if needed.

#### Customizing the Compilation Database Location

By default, Selene creates the transformed compilation database at `.selene/compile_commands.json`.
...
language_servers:
  cpp:
    compile_commands_dir: custom/rel/path (defaults to .selene)
```
...
After installing ccls, configure Selene to use it via project settings (in `.selene/project.yml`)
...
[Unreal Engine Setup Guide](unreal_engine_setup_guide_for_selene.md).
