# OCaml Setup Guide for Selene

This guide explains how to set up an OCaml project so that Selene can provide code intelligence via ocaml-lsp-server (ocamllsp).

Unlike some other languages, Selene does not download the OCaml language server automatically. You must install it yourself via opam, as OCaml tooling is compiled from source against your specific environment.
...
2. Create an opam switch with a compatible OCaml version:
   ...
   opam switch create selene-ocaml ocaml-base-compiler.5.2.1
   eval $(opam env)

   # Or for stable OCaml 4.14.x
   opam switch create selene-ocaml ocaml-base-compiler.4.14.2
   eval $(opam env)
   ```
...
When these requirements are met, Selene automatically builds the cross-file index during startup via `dune build @ocaml-index`. Without these versions, references are limited to the current file.

---
## Using Selene with OCaml

- Selene automatically detects OCaml files (`*.ml`, `*.mli`) and Reason files (`*.re`, `*.rei`).
- The language server is started via `opam exec -- ocamllsp`, so your opam environment must be configured.
- Ensure your project builds successfully with `dune build` before using Selene for best results.
