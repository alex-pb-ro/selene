# Groovy Setup Guide for Selene

The Groovy support in Selene is incomplete and requires the user to provide a functioning,
JVM-based Groovy language server as a jar. This intermediate state allows the contributors
of Groovy support to use Selene internally and hopefully to accelerate their open-source
release of a Groovy language server in the future.

If you happen to have a Groovy language server JAR file, you can configure Selene to use it
...
Configure Groovy Language Server by adding settings to your `~/.selene/selene_config.yml`:
...
- `ls_java_home_path`: Custom JAVA_HOME directory for Java installation (optional)
    - When specified, Selene will use this Java installation instead of downloading bundled Java
...
---
## Using Selene with Groovy

- Selene automatically detects Groovy files (`*.groovy`, `*.gvy`) and will start a Groovy Language Server JAR process per project when needed.
...
- **Selene Configuration**: [../02-usage/050_configuration.md](../02-usage/050_configuration.md)
