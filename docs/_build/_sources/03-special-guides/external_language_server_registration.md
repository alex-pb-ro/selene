...
SolidLSP, Selene's language server framework, is designed to be extensible.
Sometimes, you may need to use Selene with a language server for which there can be no built-in adapter,
...
You can either do this
  * programmatically, in your own Python code, if you are launching Selene via a custom script
    where this code can be executed before Selene itself is started, or
  * via a Python package that exposes a `solidlsp.language_server_registration` entry point.
    If the Python package is installed in the same environment as Selene,
    the language server will be automatically registered when Selene starts.
