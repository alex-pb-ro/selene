...
# Connecting Selene MCP Server to ChatGPT via MCPO & Cloudflare Tunnel

This guide explains how to expose a **locally running Selene MCP server** (powered by MCPO) to the internet using **Cloudflare Tunnel**, and how to connect it to **ChatGPT as a Custom GPT with tool access**.
...
## 1. Start the Selene MCP Server via MCPO

Run the following command to launch Selene as http server (assuming port 8000):

```bash
uvx mcpo --port 8000 --api-key <YOUR_SECRET_KEY> -- \
  selene start-mcp-server --context chatgpt --project $(pwd)
```
...
```
https://selene-agent-tunnel.trycloudflare.com
```
...
Depending on your configuration and enabled tools, Selene's MCP server may:
...
In your project’s `.selene/project.yml` or global config, you can disable tools like:
