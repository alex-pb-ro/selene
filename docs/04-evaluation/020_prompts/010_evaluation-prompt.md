...
We use the prompt below to evaluate the added value of Selene's tools against the
...
```
# Evaluate Selene's Tools Against Built-Ins

You have access to Selene's coding tools alongside your built-in tools (Read,
Edit, Write, Glob, Grep, Bash, etc.). I want a thorough, evidence-based
evaluation of **what Selene's tools add on top of the built-ins**, assuming both
...
clear, specific picture of what Selene adds — as well as where it provides no
...
concrete findings. If Selene adds substantial capabilities, name and quantify
...
complementary — that's a given, not the answer. Selene is an augmentation layer,
...
measure what Selene adds where it applies, not what it fails to add where it
doesn't.The answer is a specific list of what Selene contributes (or does not
contribute) to a correct-use workflow relative to built-ins.

Write the report to selene-evaluation.md in the repo root.
...
When a task falls entirely outside Selene's design scope (e.g., reading config
...
that Selene targets the task and performs worse, not that a tool designed for
...
### 1. Headline: what Selene changes

Open with a precise description of the delta Selene provides. Distinguish
between three categories: (a) tasks where Selene adds capability, (b) tasks
where Selene applies but offers no improvement, and (c) tasks outside Selene's
...
- What Selene changes relative to built-ins (positive, neutral, or negative)
- Frequency
- Value per hit
...
### 8. Tasks outside Selene's scope (built-in only)

Identify tasks where built-ins are the natural choice because Selene's tools
don't target them. List these briefly for completeness but do not frame them as
Selene shortcomings — they are outside its scope. Estimate their share of daily
work to contextualize how much of a session Selene's augmentation covers.
