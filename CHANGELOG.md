...
Entries below are inherited project history with names normalized for this checkout. They do not describe releases published by this fork; current privacy behavior is documented in [SECURITY_AUDIT.md](SECURITY_AUDIT.md).
...
* General:
  - Add agent plugin packaging and local marketplaces for GitHub Copilot, Codex and Claude Code,
    with project initialization guidance and a standalone distribution builder.
  ...
  - Fix: Parallel agents auto-registering projects could overwrite each other's changes to the global
    project list in `selene_config.yml`
...
* JetBrains:
  - Fix: Concurrent Selene sessions activating different projects at the same time with
    ...
    command and Selene waits for the plugin server to become reachable before proceeding (#1864)

* Hooks:
  - Fix: Codex's documented hook wiring only routes `remind` through `PreToolUse` on `Bash`, so its
    reset-on-Selene-tool-use branch was unreachable there and reminder counters never cleared after a
    successful Selene call. Add a `selene-hooks reset` command and a `PostToolUse` example matched to
...
* Language Servers:
  ...
  - Fix: the managed Solidity language server could report no diagnostics on macOS when Hardhat could not write
    its global state under ``~/Library``; Selene now gives the child process an isolated home-directory view
  ...
  - Add: Installed Python packages can provide generic external language-server adapters through the
    `selene.language_servers` entry-point group for explicit use in `project.yml`
  ...
  - Fix: `DartLanguageServer._start_server` discarded both `$/analyzerStatus` and
    ...
    return before the workspace scan finished. Selene now waits (bounded by 60s) for either signal to
    report completion, matching the pattern already used for pyright, basedpyright and rust-analyzer
  - Fix: clojure-lsp was not told that Selene sends `workspace/didChangeWatchedFiles`, so changes made
    outside Selene's own edit tools (a git checkout, another editor, a build step) need not invalidate
  ...
  - Fix: Scala cross-file queries waited a fixed 5s after the first file was opened, which on a cold
    ...
    indicate it was incomplete. Selene now declares work-done progress support and waits for the work
  ...
  - Fix: two Selene instances activating the same project concurrently launched their Kotlin LSP
...
* General:
  - Fix: Race conditions in ProjectServer when used by multiple clients in parallel<br>
  ...
  - Fix: cleanup of an independently-started LSP process and its children required enumerating the
    system process table (`psutil`), which can be denied even for processes Selene owns in a
  ...
  - Project activation errors are now reported to the client in Selene's system prompt, instead of failures
...
* Language Servers:
  ...
  - Allow language server priorities to be configured in `selene_config.yml` (for auto-detection during
    project creation)
  ...
  - Fix: `LSPFileBuffer`: a stale content hash could be returned if files are kept open
    and file contents were not read before trying to retrieve the hash value<br>
  ...
  - Fix: On Linux, a language server process spawned in its own session (the default) is no longer
    orphaned when Selene is killed without a chance to shut down cleanly (e.g. SIGKILL, OOM) #1490
  ...
  - `scala`: Fix: Metals asks via `window/showMessageRequest` whether to import a workspace it has not
    seen before, and Selene had no handler, so the request failed with `MethodNotFound` and Metals gave
...
* Dashboard:
  - Fix: Selene PyPI version check triggered by callback on main thread could delay agent startup #1774
  - Improvements in `tray_manager` interface mode:
    ...
    - On Selene shutdown, message the tray manager before lengthy project shutdowns
    - Fix: When dead ports are detected (Selene instance gone), explicitly update the tray menu
      immediately (may not update automatically)
  - Use `tray_manager` interface as new default on macOS

* Hooks:
  - Add `selene-hooks --client=grok`, including Grok-native PreToolUse allow/deny output.
...
* General:
  - Fix: `FileUtils.read_file`'s `charset_normalizer` fallback (used when a file cannot be decoded with
    ...
    reached Selene's in-memory file contents, where the rest of the code assumes LF-normalized text and
...
* Dashboard:
  - The version display now indicates when a newer Selene version is available
...
* General:
  ...
  - Fix: context or mode argument referencing a known name (e.g. `--context anitgravity`) could result in<br>
  ...
  - Fix: a registered project whose root directory was deleted while Selene was already running could break
    `activate_project`/project lookup, raising `FileNotFoundError` in `RegisteredProject.matches_root_path`
  - Update prompts/instructions: Selene instructions manual, modes (editing, interactive)
  ...
  - Consider all LSP-compliant line endings ("\n", "\r\n" and "\r") in `TextUtils`, noting that
    only "\n" appears in files read by `FileUtils.read_file` (Selene's default reading mechanism)
...
* CLI:
  - Fix `--project-from-cwd` hijacking git worktrees nested under a Selene project. `find_project_root`
    now walks up in a single pass so the nearest project boundary wins (either a `.selene/project.yml`
    or a `.git`, including worktree/submodule pointer files), instead of preferring an ancestor's
    `.selene/project.yml` over a closer `.git`. This previously bound CLI agents (Claude Code, Codex,
...
* JetBrains:
  - Add configuration option `jetbrains_launch_command`, allowing Selene to spawn IDE instances automatically
...
* General:
  ...
  - Add `selene-agent` CLI command so that `uvx selene-agent` can be used as entrypoint.
  - Fortls and pyright are now installed on the fly instead of being bundled in the selene-agent package.
...
* General:
  - Fix `onboarding_tool`: Used incorrect path to bootstrap memory (regression in v1.5.0)<br>
...
* Language Servers:
  ...
  - Add **GDScript** (Godot Engine) support. Selene connects over TCP to the Godot editor's built-in LSP server (port 6008, same for Godot 3 and 4) — no separate language server process to install. Godot major version is auto-detected from `config_version` in `project.godot`. Note: Godot's LSP does not implement `workspace/symbol`; first workspace-wide scans fall back to per-file requests and can be slow for large projects (results are cached to disk). See the [GDScript Setup Guide](docs/03-special-guides/godot_gdscript_setup_guide_for_selene.md) for details. Closes #1446.
...
* Memories:
  - Memories can now reference each other using the `mem:<name>` convention. Renames
    propagate to all references automatically. See the [reference convention](docs/02-usage/045_memories.md#referencing-memories-from-other-memories).
  - Onboarding now seeds a `memory_maintenance` memory describing the memory-style and conventions,
    ...
    See the [memory maintenance section](docs/02-usage/045_memories.md#the-memory-maintenance-memory).

* CLI:
  - Add `selene memories` CLI command group: `list`, `read`, `write`, `check` (referential
    integrity report) and `auto-prefix-references` (heuristic rewrite of bare occurrences).
    See the [CLI subcommands](docs/02-usage/045_memories.md#cli-subcommands).
...
* General:
  - Breaking change in mode definitions: Projects (project.yml) can no longer override `base_modes`.
    Instead, they can define `added_modes` to add modes on top of base and default modes.<br>
    See updated [documentation on modes](docs/02-usage/050_configuration.md#modes).
  - Selene's default configuration now uses `interactive` and `editing` as `base_modes` instead of as `default_modes`.
...
* Language Servers:
  - Add **Svelte** support via `svelte-language-server@0.18.0`, installed with npm into Selene-managed language-server resources. The `svelte` language handles `.svelte` Single File Components plus TypeScript/JavaScript files for Svelte projects; use it instead of also enabling `typescript` unless intentionally running multiple language servers.
...
* Hooks:
  ...
  - Reminder hook now also counts and fires on usages of selene's non-symbolic tools.
...
* General:
  ...
  - Usage reporting now also includes the name of the Selene context that is used
  ...
  - Selene's system prompt (a.k.a. the 'Selene Instructions Manual') is now provided lazily.
  ...
  - Add `selene_info` tool for on-demand retrieval of usage information

* CLI:
  - Support `selene --version` CLI command for displaying the current version #1347
  - Extend `prompts` subcommand with `print-prompt-template` and `print-cc-system-prompt-override`, improve `list` subcommand

* Clients:
  - Document workaround to make Claude Code use Selene's tools after recent degradations caused by changes in CC harness and Opus 4.7 release.

* JetBrains:
  - Add `debug` tool: The agent can set breakpoints, inspect variables, evaluate expressions and control execution flow
    by directly interacting with the IDE's debugger, using a REPL-style interface for maximum flexibility.<br>
...
* Language Servers:
  - Fix: clangd capability checks now tolerate valid initialize response shape differences and invalidate cached C++ document symbols when clangd/compile commands context changes #1359<br>
  ...
  - Fix: Elixir/Expert deadlock on startup — Expert's build pipeline requires a `textDocument/didOpen` notification to start; Selene now opens `mix.exs` immediately after `initialized` so Expert begins compiling instead of waiting indefinitely #1397
...
* General:
  - Support environment variable `SELENE_USAGE_REPORTING` (set to `false` to disable usage reporting)
...
* General:
  - **Major**: Add commands for hooks and documentation of recommended setup. Consider setting up the [recommended hooks](docs/02-usage/030_clients.md) !
  - Add `selene init` and `selene setup` commands
  - Rework installation instructions, switching to releases on pypi for distribution. Please update your mcp startup commands!
  - Add minimal usage data collection on startup (only Selene version, language backend, OS, dashboard enabled status; no personally identifiable information)
  - Fix: git commit id in Selene version strings was incorrect
...
* General:
    ...
    * Add support for querying projects other than the currently active one via new tools `QueryProjectTool` and `ListQueryableProjectsTool`.
      The `QueryProjectTool` allows Selene tools to be called on other projects.
    ...
    * Major extensions to the dashboard, which now serves as a central web interface for Selene
        ...
        * View and create memories and edit the selene configuration file
    ...
    * Language server backend:
        ...
        * Allow passing language server-specific settings through `ls_specific_settings` field (in `selene_config.yml`)
    * Add the JetBrains language backend as an alternative to language servers
    * Improve management of Selene projects
        ...
        * The location of Selene's project-specific data folder can now be flexibly configured, allowing, in particular,
...
* Language support:
  ...
  * **Add support for OCaml** via ocaml-lsp-server with cross-file reference support on OCaml 5.2+ (requires opam; see [setup guide](docs/03-special-guides/ocaml_setup_guide_for_selene.md))
  ...
  * **Add partial support for Groovy** requires user-provided Groovy language server JAR (see [setup guide](docs/03-special-guides/groovy_setup_guide_for_selene.md))
  ...
  * **Add support for Scala** via Metals language server (requires some [manual setup](docs/03-special-guides/scala_setup_guide_for_selene.md))
  ...
  * **Rust now uses already installed rustup**: The rust-analyzer is no longer bundled with Selene. Instead, it uses the rust-analyzer from your Rust toolchain managed by rustup. This ensures compatibility with your Rust version and eliminates outdated bundled binaries.
...
We release it for users who install Selene from a tag, since the last tag cannot be installed due to a breaking change in the mcp dependency (see #381).

Since the last release, several new languages were supported, and the Selene CLI and configurability were significantly extended.
...
This is the first release of Selene to pypi. Since the last release, we have greatly improved
...
* **Reduce the use of asyncio to a minimum**, improving stability and reducing the need for workarounds
   ...
   * Switch from fastapi (which uses asyncio) to Flask in the Selene dashboard
...
* Configuration:
   ...
   * Selene config, modes and contexts can now be adjusted from the user's home directory.
...
* **Overhaul and major improvement of editing tools!**
  This represents a very important change in Selene. Symbols can now be addressed by their `name_path` (including nested ones)
...
* Dashboard as web app and possibility to shut down Selene from it (or the old log GUI).
* Possibility to index your project beforehand, accelerating Selene's tools.
...
* Use **process isolation** to fix stability issues and deadlocks (see #170).
  This uses separate process for the MCP server, the Selene agent and the dashboard in order to fix asyncio-related issues.

# 2025-05-24

* Important new feature: **configurability of mode and context**, allowing better integration in a variety of clients.
  See corresponding section in readme - Selene can now be integrated in IDE assistants in a more productive way.
...
* Selene core:
    ...
    * New tool `RestartLanguageServerTool` for restarting the language server (in case of other sources of editing apart from Selene)
...
* Selene core:
    ...
    * Native support for gitignore files for configuring files to be ignored by selene. See also
      in *Language Servers* section below.
    * **Major Feature**: Allow Selene to switch between projects (project activation)
        * Add central Selene configuration in `selene_config.yml`, which
...
* Agno:
    ...
    * Fix Selene GUI log window not capturing logs after initialization
